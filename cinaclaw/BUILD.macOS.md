...
**Important: RVM/Ruby may require OpenSSL v1, while building cinaclaw requires OpenSSL v3. Make sure to switch to the right version of OpenSSL as necessary with `brew link`**
...
Building a cinaclaw package requires cmake 3.9 or greater. The most convenient
...
Dylibbundler is required to make cinaclaw packages:

    brew install dylibbundler

#### QEMU Configuration

QEMU is required to build the cinaclaw tests:
...
```
python3 -m venv ~/cinaclaw-build-env
source ~/cinaclaw-build-env/bin/activate
pip install distlib
```
...
Once it is complete, you will have a cinaclaw.pkg file in the build directory.
...
----------------------
To get console output from cinaclaw while installed, from one shell run:

    sudo launchctl debug system/com.canonical.cinaclawd --stdout --stderr

and then from another shell, restart the cinaclawd daemon with

    sudo launchctl kickstart -k system/com.canonical.cinaclawd

The console output from cinaclawd will appear on the former shell.
