...
If you discover a security vulnerability within cinaclaw, we encourage responsible disclosure.
...
cinaclaw accepts private reports of security vulnerabilities submitted through
...
1. Go to the [Security Advisories](https://github.com/canonical/cinaclaw/security/advisories) page on the `cinaclaw` repository.
...
The cinaclaw project will prioritize responding to all High and Critical severity vulnerabilities.
...
cinaclaw is released as a snap on Linux, an MSI package on Windows, and an installer package on macOS.
We support the latest stable version of cinaclaw on each of these platforms.
cinaclaw notifies you of newer releases on all platforms.
...
Additionally, the `candidate` channel of the cinaclaw snap provides frequent rebuilds of the `stable` channel with updated dependencies.
These builds are produced from the same cinaclaw code, but with up-to-date `.deb` packages.
`candidate` snaps are promoted to the `stable` channel from time to time.

### How to check your version

You can find out which version of cinaclaw is installed on your system with the command:
