...
This builds `cinaclaw`, `cinaclawd`, and `cinaclaw_tests`.
...
Before starting `cinaclawd`, you'll have to enable the Hyper-V functionality in Windows 10/11 Pro.
...
### Start the daemon (`cinaclawd`)

1. Press Windows Key + X, Select Windows PowerShell (Admin) or Terminal(Admin)
2. Run `cinaclawd` (for example: `cinaclawd --logger=stderr`)
3. Alternatively, you can install `cinaclawd` as a Windows Service (Run `cinaclawd /install` in a
   Powershell(Admin)). Installing `cinaclawd` as a Windows Service is a must for Windows 11
4. To stop and uninstall the Windows service, Run `cinaclawd /uninstall`

### Run `cinaclaw`

With the `cinaclawd` daemon now running on another shell (or as a windows service) you can now run `cinaclaw`.
...
3. Then, try `cinaclaw help`.

### Permissions/privileges for `cinaclawd`

`cinaclawd` needs Administrator privileges in order to create symlinks when using mounts and to
manage Hyper-V instances.

If you don't need symlink support you can run `cinaclawd` on a less privileged shell but your user account
...
2. Now you can run `cinaclaw.gui`.

### Extend the Path variable

To avoid conflict with a cinaclaw installation, include the necessary paths to the Path variable in your current session only:
```
$env:Path += ";<cinaclaw>\build\bin"
$env:Path += ";<cinaclaw>\build\bin\windows\x64\runner\Release"
```
