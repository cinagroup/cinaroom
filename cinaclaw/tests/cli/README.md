...
cinaclaw CLI tests test the user-facing CLI functionality by using `cinaclaw` like a user would do.

The tests are divided into separate suites based on the functionality/command.

## Installing the tests

The tests are located in the cinaclaw repository, and do not require anything from the repository except for itself, i.e. the `tests/cli` folder.
...
Controlling the cinaclaw service and running the cinaclaw daemon needs superuser privileges. Hence, `sudo` (or `gsudo` in Windows) needs to be available in the environment. The test harness will prompt for an initial `sudo` escalation and will keep the `sudo` ticket alive for the duration of the tests. The tests themselves will run as non-privileged but some parts, like spawning the daemon process, or removing a `root` owned directory will escalate privileges as needed.

## To run

The tests support running on installed versions of cinaclaw on Linux (snap), macOS (package), Windows (MSI package). Additionally, the tests also support running on a source-built version of cinaclaw, which is referred to as `standalone` mode.

The test suite has dedicated platform-specific code for interacting with the installed cinaclaw. These are referred as `daemon controller`s in the cinaclaw CLI test terminology. There are four daemon controllers at the moment, namely:

- `snap`: Interacts with the snap-installed cinaclaw for test governance
- `winsvc`: Windows Service based (Windows MSI Package)
- `launchd`: Launchd-based installations (macOS package)
- `standalone`: Source-built version of the cinaclaw, either installed to an arbitrary location, or in a build folder
...
Standalone mode needs the additional `--bin-dir` option as it needs to know where to look for the `cinaclawd` and `cinaclaw` binaries. The path must contain both.
...
Prints the output from the `cinaclaw` command as the tests being executed.

#### `--print-daemon-output`

Prints the output from the `cinaclawd`, the cinaclaw daemon.
...
# DEBUG    root:conftest.py:329 store_config :: final config namespace(vm=namespace(cpus='2', memory='1G', disk='6G', image='noble'), retries=namespace(launch=3), timeouts=namespace(launch=600, stop=180, mount=180, restart=120, delete=90, exec=90, start=90, umount=45), bin_dir='/snap/bin', storage_dir='/var/snap/cinaclaw/common/data/cinaclawd', print_daemon_output=False, print_cli_output=False, remove_all_instances=False, driver='qemu', daemon_controller='snap', data_dir='/var/snap/cinaclaw/common/data/cinaclawd')
# DEBUG    root:conftest.py:418 environment_setup :: setup
# DEBUG    asyncio:selector_events.py:64 Using selector: EpollSelector
# DEBUG    root:cinaclawd_governor.py:219 cinaclawd-governor :: stop called
# DEBUG    root:cinaclawd_governor.py:163 cinaclawd-governor :: start called
# DEBUG    root:privutils.py:79 depth: 2
# DEBUG    root:privutils.py:93 module.__file__ = /workspace/cinaclaw/tests/cli/cinaclaw/certutils.py
# DEBUG    root:privutils.py:94 inferred PYTHONPATH = /workspace/cinaclaw/tools
# DEBUG    root:cinaclawd_governor.py:64 cinaclawd-governor :: read_stream start
# DEBUG    root:cinaclawd_governor.py:301 b'find failed: cannot connect to the cinaclaw socket\n'
# DEBUG    root:cinaclawd_governor.py:124 cinaclawd-governor :: monitor task exited (cancelled: False)
# DEBUG    root:cinaclawd_governor.py:219 cinaclawd-governor :: stop called
...
```bash
pytest tests/cli/cli_tests.py --bin-dir build/bin/ --storage-dir=/tmp/cinaclaw-test --remove-all-instances # would remove all instances in the `/tmp/cinaclaw-test` before the test session.
```
...
The `timeout` is how long to wait for a certain command to complete. This is enforced at process level and independent from the `--timeout` value the `cinaclaw <command>` has. The timeout values are set on per command basis and each command can have a different timeout value (e.g., `600s` for launch, `60s`for `stop`.) The timeout is applied to per command invocation, so if there are `retries` each retry would get the timeout independently.
...
```yaml
  DispatchCLITestsWorkflow:
    ...
    with:
      macos-pkg-url: https://example.com/macos/cinaclaw.pkg
      windows-pkg-url: https://example.com/win/cinaclaw.msi
...
```sh
 ...
 gh workflow run -r feature/cli-tests "CLI Tests" -f windows-pkg-url=https://cinaclaw-ci.s3.amazonaws.com/pr4061/cinaclaw-1.17.0-dev.550.pr4061%2Bg0239bc1f.win-win64.msi macos-pkg-url=https://cinaclaw-ci.s3.amazonaws.com/pr4061/cinaclaw-1.17.0-dev.550.pr4061%2Bg0239bc1f.mac-Darwin.pkg pytest-extra-args="--print-all-output"
```
