...
from cli.cinaclaw import cinaclaw, cinaclaw_version_has_feature, skip_if_feature_not_supported
...
class TestFind:
    ...
    def test_find_all(self, show):
        ...
        with cinaclaw("find", "--format=json", show).json() as output:
            assert "images" in output or (
                show == "--only-blueprints")

            if cinaclaw_version_has_feature("blueprints"):
            ...
            if show in ["", "--only-images"]:
                ...
                if cinaclaw_version_has_feature("appliances"):
                    # Check appliances
