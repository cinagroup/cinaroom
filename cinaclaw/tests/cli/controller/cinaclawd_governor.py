...
class MultipassdGovernor:
    """
    Orchestrates the lifecycle of the **cinaclawd** daemon during tests.
    """
    ...
    async def _read_stream(self):
        ...
        logging.debug("cinaclawd-governor :: read_stream start")
        try:
        ...
        logging.debug("cinaclawd-governor :: read_stream finish")
    ...
    async def _monitor(self):
        ...
        try:
            ...
            if returncode is None:
                ...
                logging.warning(
                    "cinaclawd-governor :: monitor task -- unable to determine daemon exit code")
                return
            ...
            if returncode == 1:
                raise TestSessionFailure(
                    f"FATAL: cinaclawd died with code {returncode}!{reasons}"
                )
    ...
    async def on_monitor_exit(self, task):
        logging.debug(
            f"cinaclawd-governor :: monitor task exited (cancelled: {task.cancelled()})"
        )
        if task.cancelled():
            return

        cinaclawd_settings_changed_code = 42
        daemon_exit_code = await self.controller.exit_code()
        needs_restarting = (
            not self.controller.supports_self_autorestart()
            and daemon_exit_code == cinaclawd_settings_changed_code
        )
        self._reset_state()
        if needs_restarting:
            logging.debug("cinaclawd-governor :: daemon auto-restarting")
            self.asyncio_loop.run_fn(
                lambda: asyncio.create_task(self.start_async()))

    async def _ensure_client_certs_are_created(self):
        # Call cinaclaw cli to create the client certs
        async with SilentAsyncSubprocess(
            get_cinaclaw_path(),
            "version",
            env=get_cinaclaw_env(),
        ) as version_proc:
    ...
    async def start_async(self):
        """Start the cinaclawd daemon"""
        logging.debug("cinaclawd-governor :: start called")
        ...
        ready_task = asyncio.create_task(
            self.wait_for_cinaclawd_ready(), name="wait_for_ready_task"
        )
        ...
        if not await ready_task:
            logging.warning(
                "⚠️ cinaclawd not ready, attempting graceful shutdown...")
            await self.controller.stop()
            pytest.exit(
                "Tests cannot proceed, cinaclawd not responding in time.",
                returncode=12,
            )
    ...
    async def stop_async(self):
        """Stop the cinaclawd daemon"""
        logging.debug("cinaclawd-governor :: stop called")
    ...
    def wait_for_restart(self, timeout=60):
        ...
        if self.controller.supports_self_autorestart():
            ...
            logging.debug(
                "cinaclawd-governor :: daemon auto-restart detected")
            # Wait until daemon is ready
            self.asyncio_loop.run(self.wait_for_cinaclawd_ready()).result()
            return
        self.wait_for_shutdown(timeout=timeout)
        self.wait_for_start(timeout=timeout)

    @staticmethod
    async def wait_for_cinaclawd_ready(timeout=60):
        """
