...
"""
Windows Service-based controller for cinaclawd
...
class WindowsServiceMultipassdController:
    """SCM-backed controller for cinaclawd on Windows."""

    def __init__(self, service_name: Optional[str] = "cinaclaw"):
    ...
    async def follow_output(self) -> AsyncIterator[str]:
        ...
        - Uses `wevtutil qe` with XPath filter for provider name containing
          "cinaclaw". Because there is no simple built-in "follow" mode,
