...
def default_storage_dir_for_backend(backend):
    ...
    if backend == "snap":
        return "/var/snap/cinaclaw/common/data/cinaclawd"
    if backend == "launchd":
        return "/var/root/Library/Application Support/cinaclawd"
    if backend == "winsvc":
...
def determine_bin_dir():
    ...
    if cfg.daemon_controller == "snap":
        return "/snap/bin"
    if cfg.daemon_controller == "launchd":
        return "/Library/Application Support/com.canonical.cinaclaw/bin"
    if cfg.daemon_controller == "winsvc":
        # No explicit path, rely on environment PATH
        return None


def get_cinaclaw_env():
    """Return an environment dict for running Multipass with a custom storage root."""
    cinaclaw_env = os.environ.copy()
    if (
        cfg.daemon_controller in ["standalone", "none"]
        or cfg.storage_dir
        != default_storage_dir_for_backend(cfg.daemon_controller)
    ):
        cinaclaw_env["CINACLAWS_STORAGE"] = cfg.storage_dir
    return cinaclaw_env


def get_cinaclaw_path():
    """Resolve the 'cinaclaw' binary."""
    return shutil.which("cinaclaw", path=cfg.bin_dir)


def get_cinaclawd_path():
    """Resolve the 'cinaclawd' binary."""
    return shutil.which("cinaclawd", path=cfg.bin_dir)
