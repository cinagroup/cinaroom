...
def get_client_cert_path():
    ...
    if sys.platform == "win32":
        ...
        if cfg.daemon_controller == "snap":
            data_location = Path.home() / "snap" / "cinaclaw" / "current" / "data"
        else:
            # Not sure about this:
            data_location = Path.home() / ".local" / "share"

    return data_location / "cinaclaw-client-certificate" / "cinaclaw_cert.pem"
...
def authenticate_client_cert(client_cert_path, data_root):
    ...
    under <data_root>/data/authenticated-certs/cinaclaw_client_certs.pem.
    """

    dst_path = Path(data_root) / "authenticated-certs" / "cinaclaw_client_certs.pem"
