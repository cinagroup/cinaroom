...
def validate_output(*args, properties, jq_filter=None):
    with cinaclaw(*args).json() as output:
...
def validate_list_output(name, properties):
    """Validate properties of a specific VM from `cinaclaw list`.

    Fetches all VM data via `cinaclaw list --format=json`, locates the
...
def validate_info_snapshot_output(name, properties):
    """Validate properties of a specific VM from `cinaclaw info --snapshots`.

    Fetches VM data via `cinaclaw info --format=json --snapshots <name>`, locates the
...
def validate_info_output(name, properties):
    """
    Validate 'cinaclaw info' JSON output for a VM.
    """
