...
"""Test helpers for running `cinaclaw` commands and querying instance info."""
...
from .cinaclaw_cmd import cinaclaw


def _retrieve_info_field(name, key):
    """Run `cinaclaw info` for the given instance and return the specified
...
def debug_interactive_shell(name):
    ...
    with cinaclaw("shell", name, interactive=True) as sh:
        sh.interact()


def info(name):
    """
    Get 'cinaclaw info' JSON for instance.
    """
    with cinaclaw("info", "--format=json", f"{name}").json() as output:
...
def exec(name, *args, **kwargs):
    ...
    return cinaclaw("exec", name, "--", *args, **kwargs)
...
def shell(name):
    ...
    if sys.platform == "win32":
        ...
        # On Unix hosts, 'cinaclaw shell' is fine.
        return cinaclaw("shell", name, interactive=True)


def vm_exists(name):
    with cinaclaw("list", "--format=json").json() as list_json:
...
def get_ram_size(name):
    ...
    with cinaclaw("exec", name, "--", "cat", "/proc/meminfo") as result:
...
def get_disk_size(name):
    ...
    with cinaclaw("exec", name, "--", "df", "-m", "--output=size", "/") as result:
        assert result, f"Failed: {result.content} ({result.exitstatus})"
...
def get_core_count(name):
    ...
    with cinaclaw("exec", name, "--", "nproc") as result:
        assert result, f"Failed: {result.content} ({result.exitstatus})"
...
def get_default_interface_name(name):
    ...
    """
    with cinaclaw("exec", name, "--", "ip", "-o", "route", "get", "1") as result:
        assert result, f"Failed: {result.content} ({result.exitstatus})"
...
def get_cinaclaw_version():
    with cinaclaw("version") as version_output:
        match = re.search(r"cinaclaw\s*(\d+\.\d+\.\d+)", version_output.content,re.MULTILINE)
