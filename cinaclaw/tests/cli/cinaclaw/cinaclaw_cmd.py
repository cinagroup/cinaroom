...
from cli.cinaclaw import get_cinaclaw_env, get_cinaclaw_path
from cli.cinaclaw.cmd_output import Output
from cli.utilities import retry, wrap_call_if
from .cinaclaw_spawn import spawn_cinaclaw
...
def restart_epilogue(args, kwargs, result, prologue_result):
    ...
    for vm_name in filtered_args:
        ...
        def attempt_ssh(vm_name):
            with cinaclaw("exec", vm_name, "--", "echo", "ok") as v:
...
def cinaclaw(*args, **kwargs):
    ...
    Args:
        *args: Positional CLI arguments to pass to the `cinaclaw` command.
    ...
    Example:
        >>> out = cinaclaw("list", "--format", "json").jq(".instances")
        >>> with cinaclaw("info", "test") as output:
        ...     print(output.exitstatus)

    Notes:
        - You can access output fields directly: `cinaclaw("version").exitstatus`
    ...
    if retry_count and retry_count > 0:
        ...
        def retry_wrapper():
            ...
            return cinaclaw(*args, **{**kwargs, "retry": 0})
    ...
    cmd_args = [get_cinaclaw_path(), *map(str, args)]
    logging.debug(f"cmd: {cmd_args}")

    if kwargs.get("interactive"):
        return spawn_cinaclaw(args, timeout=timeout, echo=echo, env=env, interactive=True)
    ...
    class RunToCompletion:
        ...
        def __init__(self):
            ...
            try:
                self.pexpect_child = spawn_cinaclaw(
