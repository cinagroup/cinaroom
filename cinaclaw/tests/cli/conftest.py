...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--bin-dir",
        action="store",
        help="Directory to look for the cinaclaw binaries.",
    )

    parser.addoption(
        "--storage-dir",
        action="store",
        help="Storage directory for cinaclaw.",
    )
...
def pytest_runtest_setup(item):
    ...
    cinaclaw_version = get_cinaclaw_version()
    ...
    current = version.parse(cinaclaw_version)
...
def ensure_cinaclaw_binaries_are_present():
    if not get_cinaclaw_path():
        pytest.exit(
            "ERROR: Could not locate the `cinaclaw` binary!",
            returncode=1,
        )

    if cfg.daemon_controller == "standalone" and not get_cinaclawd_path():
        pytest.exit(
            "ERROR: Could not locate the `cinaclawd` binary!",
            returncode=1,
        )
...
def set_driver(controller):
    if cinaclaw("get", "local.driver") != cfg.driver:
        assert cinaclaw("set", f"local.driver={cfg.driver}")
...
def cinaclawd_class_scoped(request):
    with cinaclawd_impl() as daemon:
        request.cls.cinaclawd = daemon
        yield daemon


@pytest.fixture
def windows_privileged_mounts(cinaclawd):
    if sys.platform == "win32":
        # Check if privileged mounts are already enabled
        if "true" in cinaclaw("get", "local.privileged-mounts"):
            return
        assert cinaclaw("set", "local.privileged-mounts=1")
