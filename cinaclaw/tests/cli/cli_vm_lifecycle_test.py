...
from cli.cinaclaw import cinaclaw, state, launch, random_vm_name, get_boot_id


@pytest.mark.lifecycle
@pytest.mark.usefixtures("cinaclawd")
class TestVmLifecycle:
    ...
    def test_lifecycle_ops_on_nonexistent(self):
        ...
        for op in ops:
            with cinaclaw(op, f"{name}", disable_hooks=True) as output:
                assert not output
                assert "does not exist" in output

    def test_stop(self, instance):
        assert state(f"{instance}") == "Running"
        assert cinaclaw("stop", f"{instance}")
    ...
    def test_stop_start(self, instance):
        ...
        assert cinaclaw("stop", f"{instance}")
        assert state(f"{instance}") == "Stopped"
        assert cinaclaw("start", f"{instance}")
        assert state(f"{instance}") == "Running"
    ...
    def test_suspend_resume(self, instance):
        assert state(f"{instance}") == "Running"
        boot_id_before = get_boot_id(instance)
        assert cinaclaw("suspend", f"{instance}")
    ...
    def test_launch_delete_recover_purge(self, instance):
        assert state(f"{instance}") == "Running"
        assert cinaclaw("delete", f"{instance}")
        assert state(f"{instance}") == "Deleted"

        with cinaclaw("start", f"{instance}") as output:
            assert not output
            assert f"Instance '{instance}' is deleted." in output

        assert cinaclaw("recover", f"{instance}")
        assert state(f"{instance}") == "Stopped"
        ...
        assert cinaclaw("delete", f"{instance}", "--purge")
        with cinaclaw("info", f"{instance}") as output:
            assert not output
            assert "does not exist" in output
    ...
    def test_delete_purge(self):
        with (
            ...
            assert cinaclaw("delete", f"{name1}")
            assert state(f"{name1}") == "Deleted"

            assert cinaclaw("delete", f"{name2}")
            assert state(f"{name2}") == "Deleted"

            assert cinaclaw("purge")
    ...
    def test_lifecycle_multiple(self):
        with (
            ...
            assert cinaclaw("stop", name1, name2)
            assert state(name1) == "Stopped" and state(name2) == "Stopped"

            assert cinaclaw("start", name1, name2)
            assert state(name1) == "Running" and state(name2) == "Running"

            assert cinaclaw("suspend", name1, name2)
            ...
            assert cinaclaw("restart", name1, name2)
            assert state(name1) == "Running" and state(name2) == "Running"
            ...
            assert cinaclaw("delete", name1, name2)
            assert state(name1) == "Deleted" and state(name2) == "Deleted"

            assert cinaclaw("recover", name1, name2)
            assert state(name1) == "Stopped" and state(name2) == "Stopped"
            ...
            with cinaclaw("info", name1, name2) as output:
                assert not output
