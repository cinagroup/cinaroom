...
class TestAuthenticate:
    ...
    def test_authenticate(self, cinaclawd):
        assert cinaclaw("set", "local.passphrase=therewillbesecrets")
        ...
        with TempDirectory() as empty_home_dir:
            ...
            if cfg.daemon_controller == "standalone" and sys.platform != "win32":
                ...
                # that respect HOME.
                assert "Please authenticate" in cinaclaw(
                    "list", env={"HOME": str(empty_home_dir)}
                )

            # Invalid password.
            assert "Passphrase is not correct" in cinaclaw(
            ...
            # This will not.
            assert "No instances found." in cinaclaw(
                "list", env={"HOME": str(empty_home_dir)}
            )
