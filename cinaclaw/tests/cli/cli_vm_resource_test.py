...
from cli.utilities import is_within_tolerance
...
class TestVmResource:
    ...
    def test_modify_vm_while_running(self, instance):
        ...
        assert not cinaclaw("set", f"local.{instance}.memory=2G")
        assert not cinaclaw("set", f"local.{instance}.disk=10G")
        assert not cinaclaw("set", f"local.{instance}.cpus=3")
    ...
    def test_modify_vm(self, instance):
        ...
        assert cinaclaw("set", f"local.{instance}.memory=2G")
        assert cinaclaw("set", f"local.{instance}.disk=10G")
        assert cinaclaw("set", f"local.{instance}.cpus=3")
