...
class TestClone:
    ...
    def test_clone_nonexistent_instance(self):
        name = random_vm_name()
        assert not cinaclaw("clone", f"{name}")

    def test_clone_running_instance(self, instance):
        with cinaclaw("clone", f"{instance}") as output:
            assert not output
    ...
    def test_clone_to_self(self, instance):
        ...
        with cinaclaw("clone", f"{instance}", "--name", f"{instance}") as output:
            assert not output
    ...
    def test_clone_instance_with_snapshot(self, instance):
        ...
        assert cinaclaw("delete", f"{instance}-clone1", "--purge")
    ...
    def test_clone_verify_clone_has_different_properties(self, instance):
        ...
        assert cinaclaw("start", expected_clone_name)
