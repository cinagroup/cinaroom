...
@pytest.mark.usefixtures("cinaclawd", "windows_privileged_mounts")
class TestMount:
    ...
    def test_mount(self, instance, mount_type):
        with TempDirectory() as mount_src, src_to_dst(mount_src) as mount_dst:
            ...
            assert cinaclaw("mount", "--type", mount_type, str(mount_src), instance)
            ...
            assert cinaclaw("umount", instance)
            assert mounts(instance) == {}
    ...
    def test_mount_multiple(self, instance, mount_type):
        with (
            ...
            assert cinaclaw("mount", "--type", mount_type, str(mount_src1), instance)
            assert cinaclaw("mount", "--type", mount_type, str(mount_src2), instance)
            assert cinaclaw("start", instance)
    ...
    def test_mount_restart(self, cinaclawd, instance, mount_type, restart_type):
        with TempDirectory() as mount_src, src_to_dst(mount_src) as mount_dst:
            if mount_type == "native":
                assert cinaclaw("stop", instance)
            ...
            if restart_type == "daemon":
                ...
                assert cinaclaw("restart", instance)
            else:
            ...
            from cli.cinaclaw import debug_interactive_shell
