...
from cli.cinaclaw import cinaclaw, path_exists, read_file
...
class TestTransfer:
    ...
    def test_transfer_single_file(self, instance):
        ...
        with TempDirectory() as tmp:
            ...
            # Push
            assert cinaclaw("transfer", str(file), f"{instance}:.")
            assert path_exists(instance, "testfile.txt")

            # Pull
            pull_file = tmp / "testfile_pull.txt"
            assert not pull_file.exists()
            assert cinaclaw("transfer", f"{instance}:testfile.txt", str(pull_file))
    ...
    def test_transfer_single_file_create_parents(self, instance):
        ...
        with TempDirectory() as tmp:
            ...
            # This fails.
            # assert cinaclaw("transfer", "--parents", str(file), f"{instance}:bar/baz/")
            assert cinaclaw(
    ...
    def test_transfer_directory_recursive(self, instance):
        ...
        with TempDirectory() as tmp:
            ...
            # Push
            assert not cinaclaw("transfer", str(tmp), f"{instance}:dest")
            assert cinaclaw("transfer", "--recursive", str(tmp), f"{instance}:dest")
            ...
            assert cinaclaw("transfer", "--recursive", f"{instance}:dest", pull_folder)
