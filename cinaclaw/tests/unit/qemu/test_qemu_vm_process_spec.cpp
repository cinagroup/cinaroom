...
#endif
    const auto expected_uuid = cinaclaw::utils::make_uuid(desc.vm_name);
...
TEST_F(TestQemuVMProcessSpec, apparmorProfileHasCorrectName)
{
    ...
    EXPECT_THAT(spec.apparmor_profile().toStdString(),
                HasSubstr("profile cinaclaw.vm_name.qemu-system-"));
}
...
TEST_F(TestQemuVMProcessSpec, apparmorProfileRunningAsSnapCorrect)
{
    ...
    EXPECT_THAT(spec.apparmor_profile().toStdString(),
                HasSubstr("signal (receive) peer=snap.cinaclaw.cinaclawd"));
    EXPECT_THAT(spec.apparmor_profile().toStdString(),
