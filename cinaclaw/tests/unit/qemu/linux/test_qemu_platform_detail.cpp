...
struct QemuPlatformDetail : public Test
{
          ...
          mock_firewall_config{std::make_unique<mpt::MockFirewallConfig>()}
    {
        ...
        EXPECT_CALL(*mock_utils,
                    run_cmd_for_status(QString("ip"),
                                       QStringList({"addr", "show", cinaclaw_bridge_name}),
                                       _))
    ...
    const QString cinaclaw_bridge_name{"mpqemubr0"};
...
TEST_F(QemuPlatformDetail, ctorSetsUpExpectedVirtualSwitch)
{
    ...
    EXPECT_CALL(
        *mock_utils,
        run_cmd_for_status(
            QString("ip"),
            ElementsAre(QString("link"), QString("set"), cinaclaw_bridge_name, QString("up")),
            _))
        .WillOnce(Return(true));
