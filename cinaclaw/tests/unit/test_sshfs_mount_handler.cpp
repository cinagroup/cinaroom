...
const cinaclaw::logging::Level default_log_level = cinaclaw::logging::Level::debug;
...
TEST_F(SSHFSMountHandlerTest, throwsInstallSshfsNoSnapDirFails)
{
    ...
    REPLACE(ssh_channel_request_exec,
            make_exec_that_fails_for({"[ -e /snap ]", "sudo snap list cinaclaw-sshfs"}, invoked));
...
TEST_F(SSHFSMountHandlerTest, throwsInstallSshfsSnapInstallFails)
{
    ...
    REPLACE(ssh_channel_request_exec,
            make_exec_that_fails_for(
                {"sudo snap list cinaclaw-sshfs", "sudo snap install cinaclaw-sshfs"},
                invoked));
...
TEST_F(SSHFSMountHandlerTest, installSshfsTimeoutLogsInfo)
{
    ...
    auto mocked_ssh_channel_request_exec = [&sleep, &exit_code](ssh_channel, const char* raw_cmd) {
        std::string cmd{raw_cmd};
        exit_code = SSH_OK;
        if (cmd == "sudo snap install cinaclaw-sshfs")
            sleep = true;
        else if (cmd == "sudo snap list cinaclaw-sshfs")
            exit_code = SSH_ERROR;
    ...
    EXPECT_CALL(*logger_scope.mock_logger,
                log(mpl::Level::error,
                    ...
                    AllOf(HasSubstr("Could not install 'cinaclaw-sshfs' in 'stub'"),
                          HasSubstr("timed out"))));
