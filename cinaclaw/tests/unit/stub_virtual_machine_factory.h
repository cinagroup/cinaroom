...
namespace test
{
struct StubVirtualMachineFactory : public cinaclaw::BaseVirtualMachineFactory
{
    ...
    StubVirtualMachineFactory(std::unique_ptr<TempDir> tmp_dir)
        : cinaclaw::BaseVirtualMachineFactory{tmp_dir->path()}, tmp_dir{std::move(tmp_dir)}
    {
    }
    ...
    cinaclaw::FetchType fetch_type() override
    {
        return cinaclaw::FetchType::ImageOnly;
    }

    cinaclaw::VMImage prepare_source_image(const cinaclaw::VMImage& source_image) override
    {
        return source_image;
    }

    void prepare_instance_image(const cinaclaw::VMImage& instance_image,
                                const cinaclaw::VirtualMachineDescription& vm_desc) override
    {
    }
    ...
    cinaclaw::VMImageVault::UPtr create_image_vault(std::vector<VMImageHost*> image_hosts,
