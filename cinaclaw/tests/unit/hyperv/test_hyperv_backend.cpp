...
#include <cinaclaw/memory_size.h>
#include <cinaclaw/platform.h>
#include <cinaclaw/virtual_machine.h>
#include <cinaclaw/virtual_machine_description.h>
...
namespace mpl = cinaclaw::logging;
...
struct HyperVNetworkAccessor
{
    ...
    {
        return mp::HyperVVirtualMachineFactory::get_switches(
            adapters); // private, but we're friends
    }
...
struct HyperVBackend : public Test
{
    ...
    std::vector<RunSpec> standard_ps_run_sequence(const std::vector<RunSpec>& network_runs = {
                                                      default_network_run})
    {
        std::vector<RunSpec> ret = prefix_ctor_runs;
        ret.insert(std::end(ret), std::cbegin(network_runs), std::cend(network_runs));

        auto failing_run_it = std::find_if(std::cbegin(network_runs),
                                           std::cend(network_runs),
                                           [](const auto& run) { return !run.will_return; });
        if (failing_run_it == std::cend(network_runs)) // if the ctor succeeds
        {
            // network runs are executed in the ctor, so if they fail the object is never contructed
            // & no further cmds are executed for either ctor or dtor
...
TEST_F(HyperVBackend, throwsOnFailureToAddExtraInterface)
{
    ...
    MP_EXPECT_THROW_THAT(
        ...
        std::runtime_error,
        Property(&std::runtime_error::what,
                 AllOf(HasSubstr("Could not setup"), HasSubstr(extra_iface.id))));
}
...
struct CheckFineSuite : public HyperVBackend,
...
struct HyperVNetworks : public Test
{
    ...
    std::map<std::string, mp::NetworkInterfaceInfo> network_map_from_vector(
...
{
    inline static constexpr auto bad_line_in_output_format =
        "a,few,,\ngood,lines,,\n{}\naround,a,,\nbad,one,,";
};
...
{
    ps_helper.mock_ps_exec(
        QByteArray::fromStdString(fmt::format(bad_line_in_output_format, GetParam())));
...
struct TestNonExternalSwitchesWithLinks : public HyperVNetworksPS,
                                          public WithParamInterface<std::string>
{
    inline static constexpr auto bad_line_in_output_format =
        TestWrongNumFields::bad_line_in_output_format;
};
...
TEST_P(TestNonExternalSwitchesWithLinks, throwsOnNonExternalSwitchWithLink)
{
    ...
    ps_helper.mock_ps_exec(
        QByteArray::fromStdString(fmt::format(bad_line_in_output_format, switch_line)));
...
INSTANTIATE_TEST_SUITE_P(HyperVNetworksPS,
                         TestNonExternalSwitchesWithLinks,
                         Values("internal", "private"));
...
auto make_required_forbidden_regex_matcher(Str1&& required, Str2&& forbidden)
{
    return Truly([required = make_case_insensitive_regex(std::forward<Str1>(required)),
                  forbidden = make_case_insensitive_regex(std::forward<Str2>(forbidden))](
...
{
    return ElementsAre(
        Field(&mp::NetworkInterfaceInfo::description, std::forward<Matcher>(matcher)));
}
...
TEST_P(TestNonExternalSwitchTypes, recognizesSwitchType)
{
    ...
    const auto matcher = adapt_to_single_description_matcher(
        make_required_forbidden_regex_matcher(type, "external|unknown"));
...
TEST_F(HyperVNetworksPS, recognizesExternalSwitch)
{
    ...
    const auto desc_matcher = Field(&mp::NetworkInterfaceInfo::description,
                                    make_required_forbidden_regex_matcher("external", "unknown"));
...
TEST_F(HyperVNetworksPS, handlesUnknownSwitchTypes)
{
    ...
    const auto matcher = adapt_to_single_description_matcher(
        AllOf(make_required_forbidden_regex_matcher("unknown", "private|internal|external"),
              HasSubstr(type)));
...
TEST_F(HyperVNetworksPS, includesSwitchLinksToKnownAdapters)
{
    ...
    ps_helper.mock_ps_exec(QByteArray::fromStdString(
        fmt::format("switch_{},external,{},", net_c.id, net_c.description)));
...
TEST_P(TestSwitchUnsupportedLinks, excludesSwitchLinksToUnknownAdapters)
{
    ...
    auto switch_matcher = AllOf(Field(&mp::NetworkInterfaceInfo::type, Eq("switch")),
                                Field(&mp::NetworkInterfaceInfo::links, IsEmpty()));
    EXPECT_THAT(backend.networks(),
                IsSupersetOf({switch_matcher, switch_matcher})); // expect two such switches
}
...
TEST_F(HyperVNetworksPS, includesSupportedAdapterInExternalSwitchDescription)
{
    ...
    auto match = [](const auto& expect_link) {
        return Field(&mp::NetworkInterfaceInfo::description,
                     AllOf(make_required_forbidden_regex_matcher("external", "unknown"),
                           HasSubstr(expect_link)));
    };

    ps_helper.mock_ps_exec(
        QByteArray::fromStdString(fmt::format("some switch,external,{},\n", eth.description)));
...
struct TestAdapterAuthorization : public HyperVNetworksPS,
                                  public WithParamInterface<mp::NetworkInterfaceInfo>
{
};
...
TEST_P(TestAdapterAuthorization, requiresNoAuthorizationForKnownAdaptersInSwitches)
{
    ...
    EXPECT_CALL(*mock_platform, get_network_interfaces_info)
        .WillOnce(Return(network_map_from_vector({net})));

    ps_helper.mock_ps_exec(
        QByteArray::fromStdString(fmt::format("switch,external,{},\n", net.description)));
    EXPECT_THAT(backend.networks(),
...
TEST_P(TestAdapterAuthorization, requiresNoAuthorizationForSwitches)
{
    ...
    EXPECT_THAT(std::count_if(networks.cbegin(),
                              networks.cend(),
                              [](const auto& x) { return x.type == "switch"; }),
                4);
...
TEST_F(HyperVNetworksPS, getSwitchesReturnsProvidedInterfaceIds)
{
    ...
    auto id_matcher = [](const auto& expect) {
        return Field(&mp::NetworkInterfaceInfo::id, expect);
    };
...
TEST_F(HyperVNetworksPS, getSwitchesReturnsOnlySwitches)
{
    ps_helper.mock_ps_exec("a,b,,\nc,d,,\nasdf,internal,,\nsdfg,external,dfgh,\nfghj,private,,");
    EXPECT_THAT(mpt::HyperVNetworkAccessor::get_switches({}),
                Each(Field(&mp::NetworkInterfaceInfo::type, "switch")));
}
...
TEST_F(HyperVNetworks, getAdaptersReturnsEthernetAndNoWifi)
{
    ...
    for (const auto& expected_net : {eth1, eth2})
        EXPECT_TRUE(std::any_of(got_nets.begin(),
                                got_nets.end(),
                                [&expected_net, &same_net](const auto& got_net) {
