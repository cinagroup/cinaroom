...
TEST_F(SSLCertProviderFixture, importsExistingCertAndKey)
{
    ...
    const auto key_path = dir.filePath("cinaclaw_cert_key.pem");
    const auto cert_path = dir.filePath("cinaclaw_cert.pem");
...
{
    QDir dir{cert_dir};
    auto key_file = dir.filePath("cinaclaw_cert_key.pem");
    auto cert_file = dir.filePath("cinaclaw_cert.pem");
...
TEST_F(SSLCertProviderFixture, createsDifferentCertsPerServerName)
{
    ...
    // move the cinaclaw_root_cert.pem into the temporary directory so it will be deleted
    // automatically later
