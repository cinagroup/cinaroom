...
#include <cinaclaw/auto_join_thread.h>
#include <cinaclaw/cli/argparser.h>
...
namespace
{
class TestCreate final : public cinaclaw::cmd::Command
{
public:
    using Command::Command;
    cinaclaw::ReturnCodeVariant run(cinaclaw::ArgParser* parser) override
    {
        auto on_success = [](cinaclaw::CreateReply& /*reply*/) -> mp::ReturnCodeVariant {
            return cinaclaw::ReturnCode::Ok;
        };
        auto on_failure = [this](grpc::Status& status) -> mp::ReturnCodeVariant {
            cinaclaw::CreateError create_error;
            ...
            if (errors.size() == 1)
            {
                ...
                if (error == cinaclaw::CreateError::INVALID_DISK_SIZE)
                    cerr << "disk";
                else if (error == cinaclaw::CreateError::INVALID_MEM_SIZE)
                    cerr << "memory";
                else
                    cerr << "?";
            }

            return cinaclaw::ReturnCode::CommandFail;
        };
        ...
        return ret == cinaclaw::ParseCode::Ok ? dispatch(&mp::Rpc::StubInterface::create,
                                                          request,
                                                          on_success,
                                                          on_failure,
                                                          streaming_callback)
...
private:
    cinaclaw::ParseCode parse_args(cinaclaw::ArgParser* parser)
    {
        ...
        if (status == cinaclaw::ParseCode::Ok)
        {
    ...
    cinaclaw::CreateRequest request;
};
...
class TestClient : public cinaclaw::Client
{
public:
    explicit TestClient(cinaclaw::ClientConfig& context) : cinaclaw::Client{context}
    {
                                           ...
                                           std::istream& cin)
{
    ...
    AutoJoinThread t([this, &commands, &cout, &cerr, &cin] {
        ...
        {
            QStringList args = QStringList() << "cinaclaw_test";
    ...
    const std::unordered_map<std::string, mp::VMMount>& mounts)
{
    ...
    cinaclaw::test::fake_vm_properties vm_properties{};
    ...
    const std::string& contents)
{
    ...
    QString filename(temp_dir->path() + "/cinaclawd-vm-instances.json");
