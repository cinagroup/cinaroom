...
auto expect_only_log(cinaclaw::logging::Level lvl, const std::string& substr)
{
...
TEST(PlatformWin, createAliasScriptWorks)
{
    ...
    QFile checked_script(tmp_dir.path() + "/AppData/local/cinaclaw/bin/alias_name.bat");
...
{
    const mpt::TempDir tmp_dir;
    QFile script_file(tmp_dir.path() + "/AppData/local/cinaclaw/bin/alias_name.bat");
