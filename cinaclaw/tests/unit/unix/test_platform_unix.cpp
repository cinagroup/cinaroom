...
TEST_F(TestPlatformUnix, setServerSocketRestrictionsChownFailsThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        ...
        std::runtime_error,
        mpt::match_what(
            StrEq("Could not set ownership of the cinaclaw socket: Operation not permitted")));
}
...
TEST_F(TestPlatformUnix, setServerSocketRestrictionsChmodFailsThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        MP_PLATFORM.Platform::set_server_socket_restrictions(fmt::format("unix:{}", file.name()),
                                                             false),
        std::runtime_error,
        mpt::match_what(StrEq("Could not set permissions for the cinaclaw socket")));
}
...
TEST_F(TestPlatformUnix, cinaclawStorageLocationReturnsExpectedPath)
{
    mpt::SetEnvScope e(mp::cinaclaw_storage_env_var, file.name().toUtf8());
...
TEST_F(TestPlatformUnix, cinaclawStorageLocationNotSetReturnsEmpty)
{
    auto storage_path = MP_PLATFORM.cinaclaw_storage_location();
