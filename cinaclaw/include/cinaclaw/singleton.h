...
template <typename T>
inline constexpr cinaclaw::Singleton<T>::Singleton(
    const cinaclaw::Singleton<T>::PrivatePass&) noexcept
{
}

template <typename T>
std::unique_ptr<std::once_flag> cinaclaw::Singleton<T>::once = std::make_unique<std::once_flag>();

template <typename T>
std::unique_ptr<T> cinaclaw::Singleton<T>::single = nullptr;

template <typename T>
template <typename U, typename>
inline void cinaclaw::Singleton<T>::mock() noexcept(noexcept(U(Base::pass)))
{
    init<U>();
}

template <typename T>
inline T& cinaclaw::Singleton<T>::instance() noexcept(noexcept(T(Base::pass)))
{
    init<T>();
    return *single;
}
...
template <typename T>
inline void cinaclaw::Singleton<T>::reset() noexcept
{
...
template <typename T>
template <typename U>
inline void cinaclaw::Singleton<T>::init() noexcept(noexcept(U(Base::pass)))
{
