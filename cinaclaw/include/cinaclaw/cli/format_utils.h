...
#define MP_FORMAT_UTILS cinaclaw::FormatUtils::instance()
...
template <typename Container>
Container cinaclaw::format::sorted(const Container& items)
{
    if (items.empty())
        return items;
    ...
    std::sort(std::begin(ret), std::end(ret), [&petenv_name](const auto& a, const auto& b) {
        ...
        if constexpr (std::is_same_v<T, cinaclaw::DetailedInfoItem>)
        {
        ...
        else
        {
            ...
            else if constexpr (std::is_same_v<T, cinaclaw::ListVMSnapshot>)
            {
                if (a.name() == b.name())
                {
