...
#include <cinaclaw/callable_traits.h>
...
protected:
                               ...
                               StreamingCallback&& streaming_callback)
    {
        ...
        using Arg0Type =
            typename cinaclaw::callable_traits<SuccessCallable>::template arg<0>::type;
        ...
        else
        {
            ...
            const auto tokens = cinaclaw::utils::split(context.peer(), ":");
            if (tokens[0] == "unix")
            {
                socket_address = tokens[1];
                QLocalSocket cinaclawd_socket;
                cinaclawd_socket.connectToServer(QString::fromStdString(socket_address));
                if (!cinaclawd_socket.waitForConnected() &&
                    cinaclawd_socket.error() == QLocalSocket::SocketAccessError)
                {
                    grpc::Status denied_status{
                        grpc::StatusCode::PERMISSION_DENIED,
                        "cinaclaw socket access denied",
            ...
            grpc::Status access_error_status{
                ...
                "cannot connect to the cinaclaw socket",
                fmt::format("Please ensure cinaclawd is running and '{}' is accessible",
                            socket_address)};
...
private:
    ...
    {
        using SuccessCallableTraits = cinaclaw::callable_traits<SuccessCallable>;
        using FailureCallableTraits = cinaclaw::callable_traits<FailureCallable>;
                               ...
                               Reply& reply) // lvalue refs ensure args' lifetime continues
    {
        return [&on_failure, &reply](grpc::Status status) {
            ...
            if constexpr (cinaclaw::callable_traits<FailureCallable>::num_args == 2)
