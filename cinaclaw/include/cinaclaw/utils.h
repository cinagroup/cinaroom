...
#include <cinaclaw/logging/level.h>
#include <cinaclaw/network_interface_info.h>
...
#define MP_UTILS cinaclaw::Utils::instance()
...
std::string contents_of(const cinaclaw::Path& file_path);
...
bool process_log_on_error(const QString& program,
                          ...
                          cinaclaw::logging::Level level = cinaclaw::logging::Level::debug,
                          const int timeout = 30000);
...
public:
    ...
    virtual std::string contents_of(const cinaclaw::Path& file_path) const;
...
namespace cinaclaw::utils::detail
{
...
template <typename Str, typename Filter>
Str&& cinaclaw::utils::trim_begin(Str&& s, Filter&& filter)
{
...
template <typename Str>
Str&& cinaclaw::utils::trim_begin(Str&& s)
{
    return trim_begin(std::forward<Str>(s), detail::is_space);
}

template <typename Str, typename Filter>
Str&& cinaclaw::utils::trim_end(Str&& s, Filter&& filter)
{
...
template <typename Str>
Str&& cinaclaw::utils::trim_end(Str&& s)
{
    return trim_end(std::forward<Str>(s), detail::is_space);
}

template <typename Str, typename Filter>
Str&& cinaclaw::utils::trim(Str&& s, Filter&& filter)
{
...
template <typename Str>
Str&& cinaclaw::utils::trim(Str&& s)
{
    return trim(std::forward<Str>(s), detail::is_space);
}

inline bool cinaclaw::utils::iequals(std::string_view lhs, std::string_view rhs)
{
...
template <typename OnTimeoutCallable, typename TryAction, typename... Args>
void cinaclaw::utils::try_action_for(OnTimeoutCallable&& on_timeout,
                                      std::chrono::milliseconds timeout,
                                      TryAction&& try_action,
                                      Args&&... args)
{
...
template <typename RegisteredQtEnum>
QString cinaclaw::utils::qenum_to_qstring(RegisteredQtEnum val)
{
    return QVariant::fromValue(val).toString();
}

template <typename RegisteredQtEnum>
std::string cinaclaw::utils::qenum_to_string(RegisteredQtEnum val)
{
