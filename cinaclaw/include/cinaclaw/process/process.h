...
#include <cinaclaw/utils/saturate_cast.h>
...
signals:
    void started();
    void finished(cinaclaw::ProcessState process_state);
...
Q_DECLARE_METATYPE(cinaclaw::ProcessState)

inline bool cinaclaw::Process::wait_for_started(std::chrono::milliseconds ms)
{
    return wait_for_started(saturate_cast<int>(ms.count()));
}

inline bool cinaclaw::Process::wait_for_finished(std::chrono::milliseconds ms)
{
    return wait_for_finished(saturate_cast<int>(ms.count()));
}

inline bool cinaclaw::Process::wait_for_ready_read(std::chrono::milliseconds ms)
{
