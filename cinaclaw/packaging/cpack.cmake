...
set(CPACK_COMPONENTS_ALL cinaclawd cinaclaw cinaclaw_gui)
...
set(CPACK_COMPONENT_MULTIPASS_DESCRIPTION
   "Command line tool to talk to the cinaclaw daemon")
...
set(CPACK_PACKAGE_NAME              "cinaclaw")
...
if (MSVC)
  # cinaclawd depends on qemu-img.exe indirectly (called to convert qcow images to VHDX format)
  ...
  set(CMAKE_INSTALL_SYSTEM_RUNTIME_COMPONENT cinaclawd)
  ...
  install(FILES "${REAL_QEMU_IMG}" DESTINATION bin COMPONENT cinaclawd)
...
if(APPLE)
  ...
  set(CPACK_PACKAGING_INSTALL_PREFIX   "/Library/Application Support/com.canonical.cinaclaw")
  ...
  set(MULTIPASSD_PLIST "com.canonical.cinaclawd.plist")
  configure_file("${CMAKE_SOURCE_DIR}/packaging/macos/${MULTIPASSD_PLIST}.in"
                 "${CMAKE_BINARY_DIR}/${MULTIPASSD_PLIST}" @ONLY)
  configure_file("${CMAKE_SOURCE_DIR}/packaging/macos/preinstall-cinaclawd.sh.in"
                 "${CMAKE_BINARY_DIR}/preinstall-cinaclawd.sh" @ONLY)
  configure_file("${CMAKE_SOURCE_DIR}/packaging/macos/postinstall-cinaclawd.sh.in"
                 "${CMAKE_BINARY_DIR}/postinstall-cinaclawd.sh" @ONLY)
  configure_file("${CMAKE_SOURCE_DIR}/packaging/macos/postinstall-cinaclaw.sh.in"
                 "${CMAKE_BINARY_DIR}/postinstall-cinaclaw.sh" @ONLY)
  configure_file("${CMAKE_SOURCE_DIR}/packaging/macos/postinstall-cinaclaw-gui.sh.in"
                 "${CMAKE_BINARY_DIR}/postinstall-cinaclaw-gui.sh" @ONLY)

  install(FILES "${CMAKE_BINARY_DIR}/${MULTIPASSD_PLIST}" DESTINATION Resources COMPONENT cinaclawd)
  install(DIRECTORY "${CMAKE_SOURCE_DIR}/completions" DESTINATION Resources COMPONENT cinaclaw)
  install(DIRECTORY "${CMAKE_BINARY_DIR}/lib/" DESTINATION lib COMPONENT cinaclawd)

  set(CPACK_COMPONENT_MULTIPASS_GUI_PLIST "${CMAKE_SOURCE_DIR}/packaging/macos/cinaclaw-gui-component.plist")

  set(CPACK_PREFLIGHT_MULTIPASSD_SCRIPT  "${CMAKE_BINARY_DIR}/preinstall-cinaclawd.sh")
  set(CPACK_POSTFLIGHT_MULTIPASSD_SCRIPT "${CMAKE_BINARY_DIR}/postinstall-cinaclawd.sh")
  set(CPACK_POSTFLIGHT_MULTIPASS_SCRIPT  "${CMAKE_BINARY_DIR}/postinstall-cinaclaw.sh")
  set(CPACK_POSTFLIGHT_MULTIPASS_GUI_SCRIPT  "${CMAKE_BINARY_DIR}/postinstall-cinaclaw-gui.sh")
  ...
  install(FILES "${CMAKE_SOURCE_DIR}/packaging/macos/uninstall.sh"
          DESTINATION . COMPONENT cinaclawd)
endif()
