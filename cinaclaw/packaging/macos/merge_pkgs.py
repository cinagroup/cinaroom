...
with tempfile.TemporaryDirectory() as workdir:
    ...
    cinaclaw_gui_component = "cinaclaw_gui"

    for path in x86_work.glob("*.pkg/Payload.unpacked/**/*"):
        if path.is_dir() or cinaclaw_gui_component in str(path.absolute()):
            continue
    ...
    pkgs.mkdir()
    for component in ("cinaclaw", "cinaclawd"):
        pkg = dest_work / f"cinaclaw-{version}-Darwin-{component}.pkg"
        subprocess.check_call(["pkgbuild", "--root", pkg / "Payload.unpacked",
                               "--identifier", f"com.canonical.cinaclaw.{component}",
                               "--scripts", pkg / "Scripts",
                               "--version", version,
                               "--install-location", "/",
                               pkgs / pkg.name])

    # Handle cinaclaw-gui seperately since we need to pass in a component plist
    pkg = dest_work / f"cinaclaw-{version}-Darwin-{cinaclaw_gui_component}.pkg"
    ...
    subprocess.check_call(["plutil", "-replace", "BundleIsRelocatable",
                           "-bool", "false",
                            component_plist])
    subprocess.check_call(["pkgbuild", "--root", pkg / "Payload.unpacked",
                           "--identifier", f"com.canonical.cinaclaw.{cinaclaw_gui_component}",
                           "--scripts", pkg / "Scripts",
