...
function codesign_binaries {
    ...
    find "${DIR}" -type f -name qemu-system-* -print0 | xargs -0L1 \
        codesign -v --timestamp --options runtime --force --strict \
            ...
            --identifier com.canonical.cinaclaw.qemu \
            --sign "${SIGN_APP}"

    # sign cinaclaw with additional entitlements for using the Apple Virtualization framework
    find "${DIR}" -type f -name cinaclawd -print0 | xargs -0L1 \
        codesign -v --timestamp --options runtime --force --strict \
            $( entitlements com.apple.security.virtualization ) \
            --identifier com.canonical.cinaclaw.cinaclawd \
            --sign "${SIGN_APP}"
