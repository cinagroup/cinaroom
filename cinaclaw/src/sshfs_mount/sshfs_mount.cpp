...
auto get_sshfs_exec_and_options(mp::SSHSession& session)
{
    ...
    try
    {
        ...
        auto sshfs_env{MP_UTILS.run_in_ssh_session(session, "snap run cinaclaw-sshfs.env")};
    ...
    {
        mpl::debug(category, "'cinaclaw-sshfs' snap package is not installed: {}", e.what());
    ...
    {
        using namespace cinaclaw::literals;
        ...
        else if (cinaclaw::opaque_semver{fuse_version_str} < "3.0.0"_semver)
        {
