...
mp::ArgParser::ArgParser(const QStringList& arguments,
...
mp::ParseCode mp::ArgParser::parse(const std::optional<mp::AliasDict>& aliases)
{
    ...
    cout << "Error: Unknown command or alias '" << qUtf8Printable(requested_command)
         << "' (try \"cinaclaw help\" or \"cinaclaw aliases\")\n";
...
QString mp::ArgParser::generalHelpText()
{
    ...
    //  2. Remove path prepended to the cinaclaw executable name.
    //  3. remove the useless "Arguments" section listing just "command" (by clearing positional
    //  elements)
    ...
    text = text.replace(QCommandLineParser::tr("Displays this help."),
                        QCommandLineParser::tr("Display this help"));
...
void mp::ArgParser::addPositionalArgument(const QString& name,
