...
#include <cinaclaw/cli/prompters.h>
#include <cinaclaw/constants.h>
#include <cinaclaw/exceptions/cmd_exceptions.h>
...
{
    cinaclaw::LaunchRequest_NetworkOptions net;
...
mp::ParseCode cmd::Launch::parse_args(mp::ArgParser* parser)
{
    ...
    QCommandLineOption networkOption(
        ...
        "`cinaclaw set local.bridged-network`.\n"
...
{
    if (!spinner)
        spinner = std::make_unique<cinaclaw::AnimatedSpinner>(
            cout); // Creating just in time to work around canonical/cinaclaw#2075
        ...
        for (const auto& error : launch_error.error_codes())
        {
            ...
            else if (error == LaunchError::INVALID_NETWORK)
            {
                ...
                error_details =
                    ...
                    "To troubleshoot, see "
                    "https://documentation.ubuntu.com/cinaclaw/stable/how-to-guides/troubleshoot/";
            }
        }
                        ...
                        const QString& mount_target) -> ReturnCodeVariant
{
    ...
    auto ret = run_cmd({"cinaclaw", "mount", mount_source, full_mount_target}, parser, cout, cerr);
    if (ret == ReturnCode::Ok)
...
bool cmd::Launch::ask_bridge_permission(cinaclaw::LaunchReply& reply)
{
