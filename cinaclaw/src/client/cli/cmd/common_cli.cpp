...
#include <cinaclaw/cli/format_utils.h>
...
QString cinaclaw::cmd::describe_common_settings_keys()
{
...
namespace
{
void add_timeout_option(cinaclaw::ArgParser* parser, const QString& extra_description = {})
{
...
void cinaclaw::cmd::add_instance_timeout(cinaclaw::ArgParser* parser)
{
    const auto instance_timeout_str =
        QString(" By default, instance startup and initialization is limited to "
                "%1 minutes each.")
            .arg(std::chrono::duration_cast<std::chrono::minutes>(cinaclaw::default_timeout)
                     .count());

    add_timeout_option(parser, instance_timeout_str);
}

void cinaclaw::cmd::add_timeout(cinaclaw::ArgParser* parser)
{
    add_timeout_option(parser);
}

int cinaclaw::cmd::parse_timeout(const cinaclaw::ArgParser* parser)
{
...
std::unique_ptr<cinaclaw::utils::Timer> cinaclaw::cmd::make_timer(int timeout,
                                                                    AnimatedSpinner* spinner,
                                                                    std::ostream& cerr,
                                                                    const std::string& msg)
{
    auto timer = std::make_unique<cinaclaw::utils::Timer>(std::chrono::seconds(timeout),
                                                           [spinner, &cerr, msg]() {
                                                               if (spinner)
                                                                   spinner->stop();
                                                               cerr << msg << std::endl;
