...
#include <cinaclaw/ssh/ssh_client.h>
...
mp::ReturnCodeVariant cmd::Shell::run(mp::ArgParser* parser)
{
    ...
    auto on_failure = [this, &instance_name, parser](grpc::Status& status) -> ReturnCodeVariant {
        ...
        if (status.error_code() == grpc::StatusCode::NOT_FOUND &&
            ...
            retry_args.append({"cinaclaw", "launch", "--name", petenv_name});
        else if (status.error_code() == grpc::StatusCode::ABORTED)
            retry_args.append({"cinaclaw", "start", QString::fromStdString(instance_name)});
        else
