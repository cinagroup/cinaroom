...
#include <cinaclaw/cli/formatter.h>
...
mp::ReturnCodeVariant cmd::Version::run(mp::ArgParser* parser)
{
    ...
    auto on_success = [this](mp::VersionReply& reply) -> ReturnCodeVariant {
        cout << chosen_formatter->format(reply, cinaclaw::version_string);
...
{
    return QStringLiteral("Display version information about the cinaclaw command\n"
                          "and daemon.");
}
