...
#include <cinaclaw/ssh/throw_on_error.h>
...
cinaclaw::SSHSession::SSHSession(cinaclaw::SSHSession&& other)
    : SSHSession(std::move(other), std::unique_lock{other.mut})
{
}

cinaclaw::SSHSession::SSHSession(cinaclaw::SSHSession&& other, std::unique_lock<std::mutex>)
    : session{std::move(other.session)}, mut{}
{
}

cinaclaw::SSHSession& cinaclaw::SSHSession::operator=(cinaclaw::SSHSession&& other)
{
    if (this != &other)
    {
...
cinaclaw::SSHSession::~SSHSession()
{
...
bool cinaclaw::SSHSession::is_connected() const
{
    std::unique_lock lock{mut};
