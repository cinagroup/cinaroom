...
#include <cinaclaw/exceptions/autostart_setup_exception.h>
#include <cinaclaw/exceptions/settings_exceptions.h>
#include <cinaclaw/exceptions/snap_environment_exception.h>
#include <cinaclaw/file_ops.h>
#include <cinaclaw/format.h>
#include <cinaclaw/logging/log.h>
#include <cinaclaw/platform.h>
#include <cinaclaw/process/qemuimg_process_spec.h>
#include <cinaclaw/settings/settings.h>
#include <cinaclaw/snap_utils.h>
...
int get_net_type(const QDir& net_dir) // types defined in if_arp.h
{
    ...
    auto snap_hint =
        mpu::in_cinaclaw_snap() ? " Is the 'network-observe' snap interface connected?" : "";
    mpl::warn(category, "Could not read {}.{}", type_file.fileName(), snap_hint);
...
QString get_net_devtype(const QDir& net_dir)
{
    ...
    mpl::warn(category, "Could not read {}", uevent_file.fileName());
    return {};
}
...
std::unique_ptr<QFile> cinaclaw::platform::detail::find_os_release()
{
...
std::pair<QString, QString> cinaclaw::platform::detail::parse_os_release(
...
std::string cinaclaw::platform::detail::read_os_release()
{
                                                 ...
                                                 const mp::AliasDefinition& def) const
{
    ...
    std::string cinaclaw_exec =
        mpu::in_cinaclaw_snap()
            ? "exec /usr/bin/snap run cinaclaw"
...
std::string mp::platform::default_server_address()
{
    ...
    return "unix:" + base_dir + "/cinaclaw_socket";
}
...
std::string cinaclaw::platform::host_version()
{
    return mpu::in_cinaclaw_snap()
               ? cinaclaw::platform::detail::read_os_release()
...
std::filesystem::path mp::platform::Platform::get_root_cert_dir() const
{
    ...
    const auto base_dir = utils::in_cinaclaw_snap()
