...
#include <cinaclaw/platform_unix.h>
#include <cinaclaw/snap_utils.h>
#include <cinaclaw/timer.h>
                                                            ...
                                                            const bool restricted) const
{
    ...
    if (chown(socket_path.c_str(), 0, gid) == -1)
        throw std::runtime_error(
            fmt::format("Could not set ownership of the cinaclaw socket: {}", strerror(errno)));

    if (!set_permissions(socket_path, mode))
        throw std::runtime_error(fmt::format("Could not set permissions for the cinaclaw socket"));
}

QString mp::platform::Platform::cinaclaw_storage_location() const
{
    return mp::utils::get_cinaclaw_storage();
}
