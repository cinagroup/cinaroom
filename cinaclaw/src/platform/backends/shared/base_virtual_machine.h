...
inline int cinaclaw::BaseVirtualMachine::get_num_snapshots() const
{
    return static_cast<int>(snapshots.size());
}

inline int cinaclaw::BaseVirtualMachine::get_snapshot_count() const
{
    const std::unique_lock lock{snapshot_mutex};
    return snapshot_count;
}

inline QDir cinaclaw::BaseVirtualMachine::instance_directory() const
{
    return instance_dir;
}

inline const std::string& cinaclaw::BaseVirtualMachine::get_name() const
{
    return vm_name;
}

inline void cinaclaw::BaseVirtualMachine::save_error_msg(std::string error) noexcept
{
    saved_error_msg = std::move(error);
}

inline void cinaclaw::BaseVirtualMachine::refresh_start()
{
