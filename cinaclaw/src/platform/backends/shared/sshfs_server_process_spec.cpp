...
profile %1 flags=(attach_disconnected) {
    ...
    # Allow cinaclawd send sshfs_server signals
    ...
    QString root_dir; // sshfs_server is a cinaclaw utility, is located relative to the cinaclawd
                      // binary if not in a snap. If snapped, is located relative to $SNAP
    QString signal_peer; // if snap confined, specify only cinaclawd can kill dnsmasq

    try
    {
        root_dir = mpu::snap_dir();
        signal_peer = "snap.cinaclaw.cinaclawd";
    }
