...
AppleVZVirtualMachine::~AppleVZVirtualMachine()
{
    ...
    if (vm_handle)
    {
        cinaclaw::top_catch_all(vm_name, [this]() {
...
void AppleVZVirtualMachine::shutdown(ShutdownPolicy shutdown_policy)
{
    ...
    cinaclaw::utils::try_action_for(on_timeout, std::chrono::seconds{180}, [this]() {
        ...
        case VirtualMachine::State::off:
            drop_ssh_session();
            vm_handle.reset();
            return cinaclaw::utils::TimeoutAction::done;
        default:
            return cinaclaw::utils::TimeoutAction::retry;
        }
    });
}
...
{
    if (!management_ip)
        management_ip = cinaclaw::backend::get_neighbour_ip(desc.default_mac_address);
...
void AppleVZVirtualMachine::resize_disk(const MemorySize& new_size)
{
    ...
    cinaclaw::backend::resize_instance_image(new_size, desc.image.image_path);
...
void AppleVZVirtualMachine::fetch_ip(std::chrono::milliseconds timeout)
{
    ...
    auto action = [this] {
        ...
        return ((management_ip = cinaclaw::backend::get_neighbour_ip(desc.default_mac_address)))
                   ? cinaclaw::utils::TimeoutAction::done
                   : cinaclaw::utils::TimeoutAction::retry;
    };
    ...
    cinaclaw::utils::try_action_for(on_timeout, timeout, action);
}
