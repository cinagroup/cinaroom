...
signals:
    void latest_release_found(const cinaclaw::NewReleaseInfo& release);
...
{
    qRegisterMetaType<cinaclaw::NewReleaseInfo>(); // necessary to allow custom type be passed in
                                                    // signal/slot
...
void mp::NewReleaseMonitor::latest_release_found(const NewReleaseInfo& latest_release)
{
    try
    {
        const cinaclaw::opaque_semver current{current_version};
        const cinaclaw::opaque_semver latest{latest_release.version};
