...
#include <cinaclaw/settings/custom_setting_spec.h>
#include <cinaclaw/settings/settings.h>
#include <cinaclaw/standard_paths.h>
...
Json::Value create_primary_profile()
{
    ...
    primary_profile["commandline"] = "cinaclaw shell";
    ...
    primary_profile["icon"] =
        QDir{QCoreApplication::applicationDirPath()}.filePath("cinaclaw_wt.ico").toStdString();
...
QString program_data_cinaclaw_path()
{
...
QString mp::platform::Platform::daemon_config_home() const // temporary
{
    ...
    if (QFile::exists(ret))
    {
        return ret; // should be something like
                    // "C:/Windows/system32/config/systemprofile/AppData/Local/cinaclawd"
    }
    else
    {
        return MP_PLATFORM.cinaclaw_storage_location();
    }
}
                                                 ...
                                                 const mp::AliasDefinition& def) const
{
    ...
    std::string cinaclaw_exec = QCoreApplication::applicationFilePath().toStdString();

    std::string script = "@\"" + cinaclaw_exec + "\" " + alias + " -- %*\n";
...
std::string mp::platform::Platform::alias_path_message() const
{
    return fmt::format(
        ...
        "without prefixing with `cinaclaw`. For now, you can just do:\n\n"
        "In PowerShell:\n$ENV:PATH=\"$ENV:PATH;{0}\"\n\n"
        "Or in Command Prompt:\nPATH=%PATH%;{0}\n",
        get_alias_scripts_folder().absolutePath());
}

QString mp::platform::Platform::cinaclaw_storage_location() const
{
    auto storage_location = mp::utils::get_cinaclaw_storage();

    // If CINACLAWS_STORAGE env var is set, use that
    if (!storage_location.isEmpty())
    {
        return storage_location;
    }

    auto program_data_path = program_data_cinaclaw_path();
    ...
    // %SYSTEMROOT%\system32\config\AppData\Roaming\cinaclawd doesn't exist, use
    // %PROGRAMDATA%\Multipass
    if (QFile::exists(program_data_path) ||
        !QFile::exists(QDir{systemprofile_roaming_path}.absoluteFilePath("cinaclawd")))
    {
        return program_data_path;
    }

    // If %SYSTEMROOT%\system32\config\AppData\Roaming\cinaclawd exists, return empty and let the
...
std::filesystem::path mp::platform::Platform::get_root_cert_dir() const
{
    ...
    // Windows doesn't use `daemon_name` for the data directory (see `program_data_cinaclaw_path`)
