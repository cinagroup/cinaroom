...
#include <cinaclaw/alias_definition.h>
#include <cinaclaw/cloud_init_iso.h>
#include <cinaclaw/constants.h>
#include <cinaclaw/exceptions/create_image_exception.h>
#include <cinaclaw/exceptions/exitless_sshprocess_exceptions.h>
#include <cinaclaw/exceptions/ghost_instance_exception.h>
#include <cinaclaw/exceptions/image_vault_exceptions.h>
#include <cinaclaw/exceptions/invalid_memory_size_exception.h>
#include <cinaclaw/exceptions/not_implemented_on_this_backend_exception.h>
#include <cinaclaw/exceptions/snapshot_exceptions.h>
#include <cinaclaw/exceptions/sshfs_missing_error.h>
#include <cinaclaw/exceptions/start_exception.h>
#include <cinaclaw/exceptions/virtual_machine_state_exceptions.h>
#include <cinaclaw/image_host/vm_image_host.h>
#include <cinaclaw/ip_address.h>
#include <cinaclaw/json_utils.h>
#include <cinaclaw/logging/client_logger.h>
#include <cinaclaw/logging/log.h>
#include <cinaclaw/name_generator.h>
#include <cinaclaw/network_interface.h>
#include <cinaclaw/platform.h>
#include <cinaclaw/query.h>
#include <cinaclaw/settings/bool_setting_spec.h>
#include <cinaclaw/settings/settings.h>
#include <cinaclaw/snapshot.h>
#include <cinaclaw/ssh/ssh_session.h>
#include <cinaclaw/sshfs_mount/sshfs_mount_handler.h>
#include <cinaclaw/top_catch_all.h>
#include <cinaclaw/version.h>
...
#include <cinaclaw/yaml_node_utils.h>
...
constexpr auto instance_db_name = "cinaclawd-vm-instances.json";
...
constexpr auto sshfs_error_template =
    ...
    "\n\nPlease install the 'cinaclaw-sshfs' snap manually inside the instance.";
constexpr auto invalid_network_template =
    "Invalid network '{}' set as bridged interface, use `cinaclaw set "
    "{}=<name>` to correct. See `cinaclaw networks` for valid names.";
                                   ...
                                   const mp::CreateRequest* request)
{
    ...
    if (request->image() != "fedora")
    {
        ...
        auto pollinate_user_agent_string =
            fmt::format("cinaclaw/version/{} # written by Multipass\n", cinaclaw::version_string);
        pollinate_user_agent_string +=
            fmt::format("cinaclaw/driver/{} # written by Multipass\n", backend_version_string);
        pollinate_user_agent_string += fmt::format("cinaclaw/host/{} # written by Multipass\n",
                                                   cinaclaw::platform::host_version());
        pollinate_user_agent_string += fmt::format("cinaclaw/alias/{}{} # written by Multipass\n",
...
std::string get_bridged_interface_name()
{
    ...
    if (bridged_id == "")
    {
        throw std::runtime_error(
            fmt::format("You have to `cinaclaw set {}=<name>` to use the \"bridged\" shortcut.",
    ...
    mp::LaunchError& option_errors)
{
    ...
    for (const auto& net : request->network_options())
    {
        ...
        if (dont_allow_auto && net.mode() == cinaclaw::LaunchRequest_NetworkOptions_Mode_AUTO)
        {
        ...
        if (const auto& mac = QString::fromStdString(net.mac_address()).toLower().toStdString();
            ...
            interfaces.push_back(mp::NetworkInterface{
                net_id,
                mac,
                net.mode() != cinaclaw::LaunchRequest_NetworkOptions_Mode_MANUAL});
        else
        {
    ...
    for (auto& entry : vm_instance_specs)
    {
        ...
        if (spec.state == e_state::running)
        {
            ...
            case e_state::starting:
            {
                ...
                cinaclaw::top_catch_all(name, [this, &name, &lock] {
                    lock.unlock();
                    on_restart(name);
                });
            }
...
try
{
    ...
    if (!MP_SETTINGS.get_as<bool>(mp::mounts_key))
        return status_promise->set_value(grpc::Status(
            ...
            "See https://canonical.com/cinaclaw/docs/set-command#local.privileged-mounts for "
            "information\n"
                         ...
                         std::promise<grpc::Status>* status_promise)
{
    ...
    VersionReply reply;
    reply.set_version(cinaclaw::version_string);
...
try
{
    ...
    if (stored_hash.isNull() || stored_hash.isEmpty())
    {
        return status_promise->set_value(grpc::Status(
            ...
            "'cinaclaw set local.passphrase'. Note that only the user who installs Multipass is "
    ...
    if (vm.state == VirtualMachine::State::delayed_shutdown &&
        ...
        return grpc::Status{
            ...
            fmt::format("\"{}\" is scheduled to shut down in less than a minute, use "
                        "'cinaclaw stop --cancel {}' to cancel the shutdown.",
                        name,
                        name),
            ""};
