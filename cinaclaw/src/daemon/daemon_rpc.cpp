                 ...
                 mp::Rpc::Service* service)
{
    ...
    if (server == nullptr)
    {
        auto detail = check_is_server_running(server_address)
                          ? " A cinaclaw daemon is already running there."
                          : "";
        throw std::runtime_error(
            fmt::format("Failed to start cinaclaw gRPC service at {}.{}", server_address, detail));
    }
                                                                 ...
                                                                 const std::string& client_cert)
{
    ...
    else if (!client_cert_store->verify_cert(client_cert))
    {
        return grpc::Status{
            ...
            "Please authenticate before proceeding (e.g. via 'cinaclaw authenticate'). Note that "
            "you first need an authenticated user to set and provide you with a trusted passphrase "
            "(e.g. via 'cinaclaw set local.passphrase')."};
    }
