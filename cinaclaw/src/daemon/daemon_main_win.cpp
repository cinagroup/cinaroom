...
void create_client_cert_if_necessary()
{
    ...
    const QString cinaclawd_data_dir_path{
        storage_path.isEmpty()
            ? QString("%1\\config\\systemprofile\\AppData\\Roaming\\cinaclawd\\").arg(infoBuf)
            : QString("%1\\data").arg(storage_path)};

    mp::ClientCertStore cert_store{cinaclawd_data_dir_path};
