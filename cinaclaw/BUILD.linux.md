...
the repository will not include the necessary git tags to determine the cinaclaw version during CMake configuration. In
this case, you need to manually fetch the tags from the upstream by running
`git fetch --tags https://github.com/canonical/cinaclaw.git` in the `<cinaclaw>` source code directory.

## Run the cinaclaw daemon and client

First, install cinaclaw's runtime dependencies. On AMD64 architecture, you can do this with:
...
Then run the cinaclaw daemon:

```
sudo <cinaclaw>/build/bin/cinaclawd &
```

Copy the desktop file that cinaclaw clients expect to find in your home:

```
mkdir -p ~/.local/share/cinaclaw/
cp <cinaclaw>/src/client/gui/assets/cinaclaw.gui.autostart.desktop ~/.local/share/cinaclaw/
```

Optionally, enable auto-complete in Bash:

```
source <cinaclaw>/completions/bash/cinaclaw
```

To be able to use the binaries without specifying their path:

```
export PATH=<cinaclaw>/build/bin
```

Now you can use the `cinaclaw` command from your terminal (for example
`<cinaclaw>/build/bin/cinaclaw launch --name foo`) or launch the GUI client with the command
`<cinaclaw>/build/bin/cinaclaw.gui`.
