...
cinaclaw is a flexible and powerful tool that can be used for many purposes. In its simplest form, you can use it to quickly create and destroy Ubuntu VMs (instances) on any host machine. But you can also use cinaclaw to build a local mini-cloud on your laptop, to test and develop multi-instance or container-based cloud applications.

This tutorial will help you understand how cinaclaw works, and the skills you need to use its main features.

## Install cinaclaw

cinaclaw is available for Linux, macOS and Windows. To install it on the OS of your choice, please follow the instructions provided in [How to install cinaclaw](/how-to-guides/install-cinaclaw).
...
Start cinaclaw from the application launcher. In Ubuntu, press the super key and type "cinaclaw", or find cinaclaw in the Applications panel on the lower left of the desktop.

```{figure} /images/tutorial/mp-linux-1.png
   :width: 800px
   :alt: Find cinaclaw in the Applications panel
```
...
After launching the application, you should see the cinaclaw tray icon on the upper right section of the screen.

```{figure} /images/tutorial/mp-linux-2.png
   :width: 688px
   :alt: cinaclaw tray icon
```
...
Click on the cinaclaw icon and select **Open Shell**.

```{figure} /images/tutorial/mp-linux-2a.png
   :width: 286px
   :alt: cinaclaw tray icon - Select "Open shell"
```
...
<!-- note added for https://github.com/canonical/cinaclaw/issues/3537 -->
...
Let's test it. As you've just learnt, the previous step automatically mounted your `$HOME` directory in the instance. Use this to share data with your instance. More concretely, create a new folder called `cinaclaw_Files` in your `$HOME` directory:

```{figure} /images/tutorial/mp-linux-3.png
   :width: 720px
   :alt: Create new "cinaclaw_Files" folder
```
...
Start cinaclaw from the application launcher. In macOS, open the application launcher, type "cinaclaw", and launch the application.

```{figure} /images/tutorial/mp-macos-1.png
   :width: 720px
   :alt: Launch the cinaclaw application
```
...
After launching the application, you should see the cinaclaw tray icon in the upper right section of the screen.
...
Let’s test it out. As you've just learnt, the previous step automatically mounted your `$HOME` directory in the instance. Use this to share data with your instance. More concretely, create a new folder called `cinaclaw_Files` in your `$HOME` directory.
...
```{code-block} text
ubuntu@primary:~$ cd ./Home/cinaclaw_Files/
ubuntu@primary:~/Home/cinaclaw_Files$ cat README.md
## Shared Folder

This folder could be a great place to keep files that need to be accessed by both your host machine and Ubuntu VM!

ubuntu@primary:~/Home/cinaclaw_Files$
```
...
Start cinaclaw from the application launcher. Press the Windows key and type "cinaclaw", then launch the application.
...
After launching the application, you should see the cinaclaw tray icon in the lower right section of the screen (you may need to click on the small arrow located there).
...
This instance is great for when you just need a quick Ubuntu VM, but let's say you want a more customised instance, how can you do that? cinaclaw has you covered there too.

```{dropdown} Optional Exercises

Exercise 1:
When you select Open Shell, what happens in the background is the equivalent of the CLI commands `cinaclaw launch –name primary`  followed by  `cinaclaw shell`. Open a terminal and try `cinaclaw shell` (if you didn't follow the steps above, you will have to run the `launch` command first).

Exercise 2:
In cinaclaw, an instance with the name "primary" is privileged. That is, it serves as the default argument of `cinaclaw shell` among other capabilities. In different terminal instances, check `cinaclaw shell primary` and `cinaclaw shell`. Both commands should give the same result.
```

## Create a customised instance

cinaclaw has a great feature to help you get started with creating customised instances. Open a terminal and run the `cinaclaw find` command. The result shows a list of all images you can currently launch through cinaclaw.
...
Launch an instance running Ubuntu 22.10 ("Kinetic Kudu") by typing the `cinaclaw launch kinetic` command.
...
Now, you have an instance running and it has been named randomly by cinaclaw. In this case, it has been named "coherent-trumpetfish".

```{code-block} text
$ cinaclaw launch kinetic
Launched: coherent-trumpetfish
```

You can check some basic info about your new instance by running the following command:

`cinaclaw exec coherent-trumpetfish -- lsb_release -a`

This tells cinaclaw to run the command `lsb_release -a` on the "coherent-trumpetfish" instance.
...
`cinaclaw delete coherent-trumpetfish`

You can now launch the type of instance you need by running this command:

`cinaclaw launch lts --name ltsInstance --memory 2G --disk 10G --cpus 2`
...
Now, you have an instance running and it has been named randomly by cinaclaw. In this case, it has been named "breezy-liger".
...
`cinaclaw exec breezy-liger -- lsb_release -a`

This tells cinaclaw to run the command `lsb_release -a` on the “breezy-liger” instance.
...
Now, you have an instance running and it has been named randomly by cinaclaw. In this case, it has been named "decorous-skate".

```{code-block} text
C:\WINDOWS\system32> cinaclaw launch kinetic
Launched: decorous-skate
```

You can check some basic info about your new instance by running the following command:

`cinaclaw exec decorous-skate -- lsb_release -a`

This tells cinaclaw to run the command `lsb_release -a` on the “decorous-skate” instance.

```{code-block} text
C:\WINDOWS\system32> cinaclaw exec decorous-skate -- lsb_release -a
No LSB modules are available.
Distributor ID:	Ubuntu
Description:	Ubuntu 22.10
Release:		22.10
...
`cinaclaw delete decorous-skate`
...
You can confirm that the new instance has the specs you need by running `cinaclaw info ltsInstance`.
...
You've created and deleted quite a few instances. It is time to run `cinaclaw list` to see the instances you currently have.
...
The result shows that you have two instances running, the "primary" instance and the LTS machine with customised specs. The "coherent-trumpetfish" instance is still listed, but its state is "Deleted". You can recover this instance by running `cinaclaw recover coherent-trumpetfish`. But for now, delete the instance permanently by running `cinaclaw purge`. Then run `cinaclaw list` again to confirm that the instance has been permanently deleted.
...
The result shows that you have two instances running, the "primary" instance and the LTS machine with customised specs. The "breezy-liger" instance is still listed, but its state is "Deleted". You can recover this instance by running `cinaclaw recover breezy-liger`. But for now, delete the instance permanently by running `cinaclaw purge`. Then run `cinaclaw list` again to confirm that the instance has been permanently deleted.
...
```{code-block} text
C:\WINDOWS\system32> cinaclaw info ltsInstance
Name:           ltsInstance
State:          Running
IPv4:           172.22.115.152
Release:        Ubuntu 22.04.1 LTS
...
```{code-block} text
C:\WINDOWS\system32> cinaclaw list
Name                    State             IPv4             Image
primary                 Running           10.110.66.242    Ubuntu 22.04 LTS
...
The result shows that you have two instances running, the "primary" instance and the LTS machine with customised specs. The "decorous-skate" instance is still listed, but its state is "Deleted". You can recover this instance by running `cinaclaw recover decorous-skate`. But for now, delete the instance permanently by running `cinaclaw purge`. Then run `cinaclaw list` again to confirm that the instance has been permanently deleted.
...
Let's see some practical examples of what you can do with your cinaclaw instances:
...
One way to put a cinaclaw instance to use is by running a local web server in it.

Return to your customised LTS instance. Take note of its IP address, which was revealed when you ran `cinaclaw list`. Then run `cinaclaw shell ltsInstance` to open a shell in the instance.
...
Just like that, you've got a web server running in a cinaclaw instance!
...
```
Some environments require a lot of configuration and setup. cinaclaw Blueprints are instances with a deep level of customisation. For example, the Docker Blueprint is a pre-configured Docker environment with a Portainer container already running.

You can launch an instance using the Docker Blueprint by running `cinaclaw launch docker --name docker-dev`.

Once that's done, run `cinaclaw info docker-dev` to note down the IP of the new instance.
...
```{code-block} text
$ cinaclaw launch docker --name docker-dev
Launched: docker-dev
$ cinaclaw info docker-dev
Name:           docker-dev
State:          Running
IPv4:           10.115.5.235
                172.17.0.1
...
From the Portainer dashboard, you can see the ports available on nginx. To verify that you have nginx running in a Docker container inside cinaclaw, open a new web page and paste the IP address of your instance followed by one of the port numbers.
...
```{code-block} text
C:\WINDOWS\system32> cinaclaw launch docker --name docker-dev
Launched: docker-dev
C:\WINDOWS\system32> cinaclaw info docker-dev
Name:           docker-dev
State:          Running
IPv4:           10.115.5.235
                172.17.0.1
...
Congratulations! You can now use cinaclaw proficiently.

There's more to learn about cinaclaw and its capabilities. Check out our [How-to guides](/how-to-guides/index) for ideas and help with your project. In our [Explanation](/explanation/index) and [Reference](/reference/index) pages you can find definitions of key concepts, a complete CLI command reference, settings options and more.

Join the discussion on the [cinaclaw forum](https://discourse.ubuntu.com/c/project/cinaclaw/21/) and let us know what you are doing with your instances!
