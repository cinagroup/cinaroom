...
Where `<instance-name>` is the name of a cinaclaw instance and `<snapshot-name>` is the current name of one of its snapshots.
...
`cinaclaw set local.relative-lion.snaphot2.comment="Got it! 😏"`
