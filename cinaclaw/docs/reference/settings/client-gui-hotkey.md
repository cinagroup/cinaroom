...
```{caution}
This setting has been removed from the CLI starting from cinaclaw version 1.14.
...
- Some combinations may be grabbed by the OS before they reach cinaclaw (e.g. `meta+a` may open the Applications, `ctrl+alt+f3` may move ttys).
```

## Examples

  * `cinaclaw set client.gui.hotkey="Ctrl+Print Screen"`.
  * `cinaclaw set client.gui.hotkey="⌃⇧Y"`.
  * `cinaclaw set client.gui.hotkey=option+space`.
  * `cinaclaw set client.gui.hotkey=""`
