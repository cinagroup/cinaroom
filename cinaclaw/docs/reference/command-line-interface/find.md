...
The `cinaclaw find` command without any argument lists the images cinaclaw can use to run instances with [`launch`](/reference/command-line-interface/launch) on your system and associated version information. For example:
...
Launching an image using its name or its alias gives the same result. For example, `cinaclaw launch 23.10`  and  `cinaclaw launch mantic` will launch the same image.
...
The option `--show-unsupported` includes old Ubuntu images, which were available at some point but are not supported anymore. This means that some features of cinaclaw might now work on these images and no user support is given. However, they are still available for testing.

The command also supports searching through available images. For example, `cinaclaw find mantic`  returns:
...
The full `cinaclaw help find` output explains the available options:

```{code-block} text
Usage: cinaclaw find [options] [<remote:>][<string>]
