...
The `cinaclaw start` command without any argument will start the {ref}`primary-instance` (and also create it, if it doesn't exist). You can also pass one or more instance names or the `--all` option to start more instances at the same time.

For example, the command `cinaclaw start` will produce the following output:
...
Only instances in `Stopped` or `Suspended` status can be started. Running instances can be restarted with [`cinaclaw restart`](/reference/command-line-interface/restart), stopped with [`cinaclaw stop`](/reference/command-line-interface/stop), and suspended with [`cinaclaw suspend`](/reference/command-line-interface/suspend).

---

The full `cinaclaw help start` output explains the available options:

```{code-block} text
Usage: cinaclaw start [options] [<name> ...]
