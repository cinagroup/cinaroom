...
The `cinaclaw unalias` command removes a previously defined alias.

```{code-block} text
cinaclaw unalias name
```
...
```{code-block} text
cinaclaw unalias name1 name2 name3
```
...
The full `cinaclaw help unalias` output explains the available options:

```{code-block} text
Usage: cinaclaw unalias [options] <name> [<name> ...]
Remove aliases
