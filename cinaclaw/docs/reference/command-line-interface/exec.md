...
> See also: [cinaclaw `exec` and shells](/explanation/cinaclaw-exec-and-shells), [How to use instance command aliases](/how-to-guides/manage-instances/use-instance-command-aliases)

The `exec` command runs the given commands inside the instance. The first argument is the instance to run the commands on, `--` optionally separates the `cinaclaw` options from the rest - the command to run itself:

```{code-block} text
cinaclaw exec primary -- uname -r
```
...
```{code-block} text
cinaclaw exec primary -- lsb_release -a | grep ^Codename:
```
...
The first one is `--working-directory <dir>`, which tells cinaclaw that the command must be run in the folder `<dir>`. For example:

```{code-block} text
cinaclaw exec arriving-pipefish --working-directory /home -- ls -a
```
...
The full `cinaclaw help exec` output explains the available options:

```{code-block} text
Usage: cinaclaw exec [options] <name> [--] <command>
