...
The `alias` command makes cinaclaw create a persistent alias to run commands on a given instance. Its syntax is the following:

```{code-block} text
cinaclaw alias instance:command [name]
```
...
The full `cinaclaw help alias` output explains the available options:

```{code-block} text
Usage: cinaclaw alias [options] <definition> [<name>]
