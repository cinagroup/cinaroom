...
The `prefer` command, introduced in cinaclaw release 1.11.0,  is used to switch alias contexts. It accepts a single argument, specifying the name of the context to switch to.

For example, the command `cinaclaw prefer secondary` makes `secondary` the current context. If the given context does not exist, cinaclaw creates it.
...
The full `cinaclaw help prefer` output explains the available options:

```{code-block} text
Usage: cinaclaw prefer [options] <name>
