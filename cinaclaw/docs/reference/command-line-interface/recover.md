...
The `cinaclaw recover` command will revive an instance that was previously removed with `cinaclaw delete`. For this to be possible, the instance cannot have been purged with `cinaclaw purge` nor with `cinaclaw delete --purge`.

Use the `--all` option to recover all deleted instances at once:

```{code-block} text
cinaclaw recover --all
```
...
```{code-block} text
Usage: cinaclaw recover [options] <name> [<name> ...]
