...
The `cinaclaw shell` command will open a shell prompt on an instance. Without any arguments, it will open the shell prompt of the {ref}`primary-instance` (and also create it, if it doesn't exist). You can also pass the name of an existing instance. If the instance is not running, it will be started automatically.

If you run `cinaclaw shell` you'll find yourself in the *primary* instance:
...
```{code-block} text
cinaclaw shell <instance name>
```

---

The full `cinaclaw help shell` output explains the available options:

```{code-block} text
Usage: cinaclaw shell [options] [<name>]
