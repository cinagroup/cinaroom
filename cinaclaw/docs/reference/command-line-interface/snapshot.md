...
The `cinaclaw snapshot` command takes a snapshot of an instance; for example:

```{code-block} text
cinaclaw snapshot maximal-stag
```
...
The full `cinaclaw help snapshot` output explains the available options:

```{code-block} text
Usage: cinaclaw snapshot [options] instance
