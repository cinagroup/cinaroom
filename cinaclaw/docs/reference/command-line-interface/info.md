...
The `cinaclaw info` command shows properties of instances and snapshots.

For example, `cinaclaw info calm-squirrel`:
...
Mounts:         /home/user => Home
                ...
                /home/user/cinaclaw => mp
                    UID map: 1000:default
                    GID map: 1001:default
```

or, in case of a snapshot, `cinaclaw info relative-lion.snapshot3`:
...
With no positional arguments, `cinaclaw info` displays information on all the instances. If you add the `--snapshots` flag and no positional arguments, the command displays information on all the snapshots of all instances.
...
For example, the command `cinaclaw info --format yaml calm-squirrel` produces the following output:
...
Here you can find a comprehensive list of the details that `cinaclaw info` provides for instances and snapshots, respectively.
...
| `IPv4` | A list of IPs through which a machine on the same network as the instance can reach the instance. Adding IP addresses besides the one automatically generated by cinaclaw is described in {ref}`create-an-instance-with-multiple-network-interfaces`. |
...
The full `cinaclaw help info` output explains the available options:
```{code-block} text
Usage: cinaclaw info [options] <instance>[.snapshot] [<instance>[.snapshot] ...]
