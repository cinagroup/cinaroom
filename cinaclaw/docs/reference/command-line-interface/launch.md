...
The `cinaclaw launch` command without any argument will create and start a new instance based on the default image, using a random generated name; for example:
...
```{code-block} text
cinaclaw shell relishing-lionfish
```
...
It's also possible to provide a full URL to the image (use `file://` for an image available on the host running `cinaclawd`).
...
Passing `--bridged` and `--network bridged` are shortcuts to `--network <name>`, where `<name>` is configured via `cinaclaw set local.bridged-interface`.
...
Use the `--timeout` option to change how long cinaclaw waits for the machine to boot and initialise.

---

The full `cinaclaw help launch` output explains the available options:

```{code-block} text
Usage: cinaclaw launch [options] [[<remote:>]<image> | <url>]
