...
The `cinaclaw restore` command restores an instance to the state that it was in when the given snapshot was taken.

For example, when you run the command:

```{code-block} text
cinaclaw restore relative-lion.snapshot2
```
...
```{code-block} text
cinaclaw info relative-lion.snapshot3 | grep Comment
```
...
The full `cinaclaw help restore` output explains the available options:

```{code-block} text
Usage: cinaclaw restore [options] <instance>.<snapshot>
