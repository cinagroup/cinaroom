...
The `cinaclaw wait-ready` command blocks execution until the cinaclaw daemon is fully initialized and ready to accept requests. This is particularly useful in batch operations and automated environments such as CI/CD pipelines, where subsequent cinaclaw commands may fail if the daemon has not yet finished starting up.
...
```bash
cinaclaw wait-ready --timeout 30
```

---

The full `cinaclaw help wait-ready` output explains the available options:

```{code-block} text

Usage: cinaclaw wait-ready [options]
Wait for the cinaclaw daemon to be ready. This command will block until the
...
```bash
cinaclaw wait-ready && cinaclaw launch
```
