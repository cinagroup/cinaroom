...
The `cinaclaw stop` command without any argument will stop the {ref}`primary-instance`
...
`Stopped` instances can be started again with [`cinaclaw start`](/reference/command-line-interface/start).

You can use the `--force` option (*available starting from cinaclaw version 1.14*) to stop the instance when it is in a non-responsive, unknown or suspended state. It can also be used with the `--all` option to stop all instances forcefully.

---

The full `cinaclaw help stop` output explains the available options:

```{code-block} text
Usage: cinaclaw stop [options] [<name> ...]
