...
The `cinaclaw delete` command deletes the instances or snapshots that are specified as arguments.

You can provide multiple arguments in the same delete command, including both instances and snapshots; for example:

```{code-block} text
cinaclaw delete --purge legal-takin calm-squirrel.snapshot2
```

Deleted instances are marked as such and removed from use, but you can still recover them using the `cinaclaw recover` command, unless you used the `-p`/`--purge` option to delete them permanently.
...
```{caution}
When you delete a [snapshot](/explanation/snapshot), or when you delete an instance using the [GUI client](/reference/gui-client), cinaclaw removes them permanently (even if you didn't use the `--purge` option) and they cannot be recovered.
...
The output of `cinaclaw help delete` explains the available options:

```{code-block} text
Usage: cinaclaw delete [options] <instance>[.snapshot] [<instance>[.snapshot] ...]
