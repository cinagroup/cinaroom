...
The `cinaclaw transfer` command copies files between host and instance, without the need of mounting a folder on the instance.

For example, to copy a local file `local_file.txt` to the default home folder of the instance `good-prawn`, use the command:

```{code-block} text
cinaclaw transfer local_file.txt good-prawn:.
```
...
```{code-block} text
cinaclaw transfer ample-pigeon:remote_file.txt .
```
...
```{code-block} text
cinaclaw transfer local_file.txt ample-pigeon:non/existent/path/remote_file.txt
```
...
```{code-block} text
cinaclaw transfer --parents local_file.txt ample-pigeon:non/existent/path/remote_file.txt
```

You can also copy an entire directory tree using the `--recursive` option:

```{code-block} text
cinaclaw transfer --recursive ample-pigeon:dir .
```
...
The full `cinaclaw help transfer` output explains the available options:
```{code-block} text
Usage: cinaclaw transfer [options] <source> [<source> ...] <destination>
