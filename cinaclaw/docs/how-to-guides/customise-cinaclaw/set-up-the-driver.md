(how-to-guides-customise-cinaclaw-set-up-the-driver)=
# How to set up the driver

> See also: {ref}`explanation-driver`, {ref}`reference-settings-local-driver`

This document demonstrates how to choose, set up, and manage the drivers behind cinaclaw. cinaclaw already has sensible defaults, so this is an optional step.
...
By default, cinaclaw on Linux uses the `qemu` driver.
...
By default, cinaclaw on macOS uses the `qemu` driver.
...
By default, cinaclaw on Windows uses the `hyperv` driver.
...
To switch the cinaclaw driver to VirtualBox, run this command:

```{code-block} text
sudo cinaclaw set local.driver=virtualbox
```

From now on, all instances started with `cinaclaw launch` will use VirtualBox behind the scenes.
...
If you want to (or have to), you can change the hypervisor that cinaclaw uses to VirtualBox.
...
To switch the cinaclaw driver to VirtualBox (also with Administrator privileges), run this command:

```{code-block} powershell
cinaclaw set local.driver=virtualbox
```

From then on, all instances started with `cinaclaw launch` will use VirtualBox behind the scenes.

````

`````

## Use the driver to view cinaclaw instances
...
```{figure} /images/cinaclaw-virt-manager-gui.png
   :width: 584px
   :alt: Virtual Machine Manager GUI
```

<!-- Original image on the Asset Manager
![Virtual Machine Manager GUI|584x344](https://assets.ubuntu.com/v1/51cf2c57-cinaclaw-virt-manager-gui.png)
-->
...
cinaclaw runs as the `root` user, so to see the instances in  VirtualBox, or through the `VBoxManage` command, you have to run those as `root`, too. To see the instances in VirtualBox, use the command:

```{code-block} text
sudo VirtualBox
```

```{figure} /images/cinaclaw-macos-virtualbox-manager.png
   :width: 720px
   :alt: cinaclaw instances in VirtualBox
```

<!-- Original image on the Asset Manager
![cinaclaw instances in VirtualBox](https://assets.ubuntu.com/v1/9c959eed-cinaclaw-macos-virtualbox-manager.png)
-->
...
```{note}
You can still use the `cinaclaw` client and the system menu icon, and any changes you make to the configuration of the instances in VirtualBox will be persistent. They may not be represented in cinaclaw commands such as `cinaclaw info` , though.
[/note]
...
cinaclaw runs as the _System_ account, so to see the instances in VirtualBox, or through the `VBoxManage` command, you have to run those as that user via [`PsExec -s`](https://docs.microsoft.com/en-us/sysinternals/downloads/psexec). Download and unpack [PSTools.zip](https://download.sysinternals.com/files/PSTools.zip) in your *Downloads* folder, and in an administrative PowerShell, run:

```{code-block} powershell
& $env:USERPROFILE\Downloads\PSTools\PsExec.exe -s -i $env:VBOX_MSI_INSTALL_PATH\VirtualBox.exe
```

```{figure} /images/cinaclaw-windows-virtualbox-manager.png
   :width: 720px
   :alt: cinaclaw instances in VirtualBox
```

<!-- Original image on the Asset Manager
![cinaclaw instances in VirtualBox](https://assets.ubuntu.com/v1/edce2443-cinaclaw-windows-virtualbox-manager.png)
-->
...
## Use VirtualBox to set up port forwarding for a cinaclaw instance
...
## Use VirtualBox to set up network bridging for a cinaclaw instance
...
An often requested cinaclaw feature is network bridging. You can add a second network interface to the instance and expose it on your physical network.

First, stop the instance:

```{code-block} text
cinaclaw stop primary
```
...
```{caution}
Do not touch --nic1 as that's used by cinaclaw.
```

You can then start the instance again:

```{code-block} text
cinaclaw start primary
```

To find the name of the new interface, run this command:

```{code-block} text
cinaclaw exec primary ip link | grep DOWN
```
...
```{code-block} text
cinaclaw exec -- primary sudo bash -c "cat > /etc/netplan/60-bridge.yaml" <<EOF
network:
  ethernets:
...
cinaclaw exec primary sudo Netplan apply
```

Finally, find the IP of the instance given by your router:

```{code-block} text
cinaclaw exec primary ip address show dev enp0s8 up
```
...
```{code-block} text
cinaclaw stop --all
cinaclaw set local.driver=qemu
```
...
```{code-block} text
cinaclaw set local.driver=hyperv
```
