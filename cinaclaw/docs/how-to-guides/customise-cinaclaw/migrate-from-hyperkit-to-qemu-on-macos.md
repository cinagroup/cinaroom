(how-to-guides-customise-cinaclaw-migrate-from-hyperkit-to-qemu-on-macos)=
# Migrate from Hyperkit to QEMU on macOS

> See also: [`set`](reference-command-line-interface-set), [local.driver](reference-settings-local-driver), [Driver](explanation-driver), [How to set up the driver](how-to-guides-customise-cinaclaw-set-up-the-driver)

As of cinaclaw 1.12, the Hyperkit driver is being deprecated. New installs will start with the QEMU driver set by default, but existing installs will retain the previous driver setting. cinaclaw will warn Hyperkit users of the deprecation and ask them to move to QEMU. To facilitate that, cinaclaw 1.12 will migrate Hyperkit instances to QEMU.
...
```{code-block} text
cinaclaw set local.driver=hyperkit
cinaclaw delete [-p] <instance> [...]
cinaclaw set local.driver=qemu
```
...
You can choose a convenient time to do any of this and you can set the driver to Hyperkit and move back and forth as many times as you want. Apart from the deprecation warning, functionality remains the same until the driver is removed entirely. When that happens, it will no longer be possible to migrate cinaclaw (unless you downgrade to version 1.12).

## Demo

Here is a video demonstration of the migration:

[![Hyperkit Migration in cinaclaw](https://asciinema.org/a/556203.svg)](https://asciinema.org/a/556203)
