...
<!-- this topic originates from Ricardo's suggestions here: https://github.com/canonical/cinaclaw/discussions/3660#discussioncomment-10548808 -->

This topic addresses common issues when launching or starting instances, such as *timeouts or "unknown state" errors*.

These problems can occur for a few different reasons. Since cinaclaw relies on instances having an IP address on the default interface to establish an SSH connection, they are often (but not always) linked to IP assignment or connectivity issues.
...
4. The VM obtained an IP address, but:
    1. cinaclaw can't find it.
    2. cinaclaw finds an IP that doesn't match the one that was assigned to the instance.
5. SSH doesn't function properly in the VM, or cinaclaw is blocked from accessing it.
...
1. If the `cinaclaw launch` command fails with the message "Downloaded image hash does not match", see {ref}`launch-start-issues-stale-network-cache`.
...
3. *(Linux/macOS, QEMU driver)* Inspect the cinaclaw logs and look for a message mentioning `NIC_RX_FILTER_CHANGED`. This message indicates that the network interface has been initialised.
...
4. *(Linux/macOS, QEMU driver)* Check DHCP traffic from your host to the instance, to find out if there are requests and replies. Adapt and run the following command *right after starting/launching* the instance:
    ...
    * If you see `NIC_RX_FILTER_CHANGED`, you should also see DHCP requests. If you don't, see {ref}`launch-start-issues-vm-boot-failure` and please [let us know](https://github.com/canonical/cinaclaw/issues/new/choose).
...
5. Look for messages regarding SSH in cinaclaw's logs. The instance may have obtained an IP and/or be properly connected, but still refuse cinaclaw when it tries to SSH into it.

6. Look for the message in the CLI or GUI spinner. Once it reads "Waiting for initialisation to complete", cinaclaw will have succeeded SSH-ing into the instance but remain waiting for cloud-init to finish.
...
- *(Linux/macOS, QEMU driver)* Relaunch QEMU manually:
    1. Look for the `qemu-system-*` command line corresponding to the failing VM in cinaclaw logs.
    ...
    ```{code-block} text
    /Library/Application\ Support/com.canonical.cinaclaw/bin/qemu-system-aarch64 -machine virt,gic-version=3 -accel hvf -drive file=/Library/Application\ Support/com.canonical.cinaclaw/bin/../Resources/qemu/edk2-aarch64-code.fd,if=pflash,format=raw,readonly=on -cpu host -nic vmnet-shared,model=virtio-net-pci,mac=52:54:00:e2:30:dd -device virtio-scsi-pci,id=scsi0 -drive file=/var/root/Library/Application\ Support/cinaclawd/qemu/vault/instances/superior-chihuahua/ubuntu-22.04-server-cloudimg-arm64.img,if=none,format=qcow2,discard=unmap,id=hda -device scsi-hd,drive=hda,bus=scsi0.0 -smp 2 -m 4096M -qmp stdio -cdrom /var/root/Library/Application\ Support/cinaclawd/qemu/vault/instances/superior-chihuahua/cloud-init-config.iso
    ```
...
- *(macOS/Windows, VirtualBox driver)* Observe the output in the VirtualBox GUI:
    ...
    2. Start or launch the instance with `cinaclaw start|launch`.
...
- Run `qemu-img check -r` on the image.
    * `qemu-img`, shipped with cinaclaw, can also be used to check and repair disk images.
    * See {ref}`launch-start-issues-locate-cinaclaw-binaries` below.
    * See {ref}`launch-start-issues-locate-cinaclaw-images` below.
    * For example:
    ```{code-block} text
    /Library/Application\ Support/com.canonical.cinaclaw/bin/qemu-img check -r /var/root/Library/Application\ Support/cinaclawd/qemu/vault/instances/<instance>/<img>
    ```
...
The macOS firewall is known to cause `vmnet` to malfunction, because it blocks Apple's own `bootp` from giving out IPs. The effect of this problem on cinaclaw is tracked in [this issue](https://github.com/canonical/cinaclaw/issues/2387), which we internally call the *dreaded firewall issue*.
...
If SSH doesn't function properly in the VM, or cinaclaw is blocked from accessing it, your instance may need to be reconfigured or repaired.

* If the default user is not `ubuntu`, cinaclaw cannot connect. If you used a custom cloud-init config file, make sure that the default user is `ubuntu`.
...
* Run the instance VM directly. This will require a username and password to log in. The username is the default user, `ubuntu`, and the password is what was set in cloud-init if you used a custom cloud-init config. If you do not have a password you can modify the instance's `cloud-init-config.iso` file to change it. One way to do so is as follows.
  ...
  2. Make a new instance by running `cinaclaw launch --cloud-init config.yaml`, the contents `config.yaml` are shown below.
...
- When launching a new instance, once cinaclaw obtains an SSH session to the instance, it will wait for cloud-init to complete. During this phase, the CLI/GUI spinner reads "Waiting for initialisation to complete".
...
- So if you wait for a little while longer, your instance may eventually finish setting up. When it does, it will have this file: `/var/lib/cloud/instance/boot-finished`.
    * Consider passing a longer timeout to the `launch` command. For example, `cinaclaw launch --timeout 1000` sets the launch timeout to 1000 seconds.

* You can use `cinaclaw shell` to get a shell in the instance when cinaclaw is waiting for cloud-init to finish. To diagnose problems, inspect cloud-init's logs in `/var/log/cloud-init*log`.
...
When successful, you should see cinaclaw's instances in VirtualBox

(launch-start-issues-stale-network-cache)=
### Stale network cache

This can be caused by a known Qt bug (see issue [#1714](https://github.com/canonical/cinaclaw/issues/1714) on our GitHub).

A workaround to resolve this issue is to run the command `cinaclaw find --force-update`, which forces downloading the image information from the network. As a result, if the download is successful, the `network-cache` will be overwritten.

Alternatively, try deleting the `network-cache` folder and restart the cinaclaw service:

* *(on Linux)*
   ```{code-block} text
   sudo snap stop cinaclaw
   sudo rm -rf /var/snap/cinaclaw/common/cache/cinaclawd/network-cache/
   sudo snap start cinaclaw
   ```
* *(on macOS)*
   ```{code-block} text
   sudo launchctl unload /Library/LaunchDaemons/com.canonical.cinaclawd.plist
   sudo rm -rf /System/Volumes/Data/private/var/root/Library/Caches/cinaclawd/network-cache
   sudo launchctl load /Library/LaunchDaemons/com.canonical.cinaclawd.plist
   ```
* *(on Windows)*
   Remove `C:\ProgramData\cinaclaw\cache\network-cache` and restart the cinaclaw service.

(launch-start-issues-reading-data-from-an-image)=
### Reading data from a QCOW2 image

The images that cinaclaw uses with the QEMU driver follow a standard format — QCOW2 — which other tools can read.

One example is [`qemu-nbd`](https://manpages.ubuntu.com/manpages/questing/en/man8/qemu-nbd.8.html), which allows mounting the image. This tool is not shipped with cinaclaw, so you would need to install it manually.
...
(launch-start-issues-locate-cinaclaw-binaries)=
### Locating cinaclaw binaries
You may need to locate where cinaclaw is installed. There are several ways to do so, depending on your platform:
* *(on Linux)*
  * Run the command `which cinaclaw` or `whereis cinaclaw`.
  * By default, cinaclaw is installed in the `/snap/bin` folder.

* *(on Windows)*
  * Run the command `where.exe cinaclaw`.
  * Right-click a shortcut to cinaclaw in your files or Start menu and select "Open file location".
  * By default, cinaclaw is installed in the `C:\Program Files\cinaclaw\bin` folder.

* *(on macOS)*
  * Run the command `readlink -f $(which cinaclaw)`
  * By default, cinaclaw is installed in the `/Library/Application\ Support/com.canonical.cinaclaw/bin/` folder.

(launch-start-issues-locate-cinaclaw-images)=
### Locating cinaclaw images
You may need to locate where cinaclaw is storing instances. The location changes depending on your platform:
* *(Linux)* `/root/.local/share/cinaclawd/vault/instances/<instance/<img>`

* *(Windows)* `C:\ProgramData\cinaclaw\data\vault\instances\<instance>\<img>`

* *(macOS)* `/var/root/Library/Application\ Support/cinaclawd/qemu/vault/instances/<instance>/<img>`
