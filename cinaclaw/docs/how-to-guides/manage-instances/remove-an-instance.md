...
```{code-block} text
cinaclaw delete keen-yak
```

Now, if you run `cinaclaw list` to list the instances, you will see that it is actually just marked for deletion (or to put it in other words, moved to the recycle bin):
...
```{code-block} text
cinaclaw recover keen-yak
```

If you try `cinaclaw list` again, you'll see that the instance is no longer marked for deletion:
...
```{code-block} text
cinaclaw delete --purge keen-yak
```
