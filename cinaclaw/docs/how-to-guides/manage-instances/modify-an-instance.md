...
This document shows further ways to customise an instance outside of the [`launch`](reference-command-line-interface-launch) command using the cinaclaw [settings](reference-settings-index).
...
```{code-block} text
cinaclaw stop handsome-ling
cinaclaw set local.handsome-ling.cpus=4
cinaclaw set local.handsome-ling.disk=60G
cinaclaw set local.handsome-ling.memory=7G
```
...
You can view these properties using the `get` command, without the need to stop instances. For example, `cinaclaw get local.handsome-ling.cpus` returns the configured number of CPUs, which in our example is "4".
...
On the other hand, it's always possible to fetch properties for all instances. Use `cinaclaw get --keys` to obtain their settings keys.
```
...
```{code-block} text
cinaclaw set client.primary-name=<instance name>
```
...
This instance is picked up automatically by `cinaclaw start`. When you run this command, the primary instance also automatically mounts the user's home directory into a directory called `Home`:
...
Run `cinaclaw stop` to stop the primary instance, and then launch another instance named "second":

```{code-block} text
cinaclaw launch --name second
```

Now, change the primary instance to the existing "second" instance:

```{code-block} text
cinaclaw set client.primary-name=second
```

From now on, the "second" instance will be used as the *primary* instance, so for example it will be used by default when you run the command `cinaclaw suspend`.

When listing instances, the primary one is displayed first. For example, if you run `cinaclaw list` now, you'll see:
