...
```{code-block} text
cinaclaw mount <local path> <instance name>
```
...
```{code-block} text
cinaclaw mount $HOME keen-yak
```

You can check the result running `cinaclaw info keen-yak`:
...
```{code-block} text
cinaclaw mount $HOME keen-yak:/some/path
```
...
```{code-block} text
cinaclaw launch --mount /local/path:/instance/path
```
...
```{code-block} text
cinaclaw transfer keen-yak:/etc/crontab keen-yak:/etc/fstab /home/michal
```
...
```{code-block} text
cinaclaw transfer /etc/crontab /etc/fstab keen-yak:/home/michal
```
