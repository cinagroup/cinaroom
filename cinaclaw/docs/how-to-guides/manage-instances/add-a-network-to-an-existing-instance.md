...
This guide explains how to bridge an existing cinaclaw instance with the available networks.

```{caution}
This feature is available starting from cinaclaw version 1.14.
```

First, you need to select a cinaclaw-wide preferred network to bridge with (you can always change it later). To do so, list all available networks using the [`cinaclaw networks`](reference-command-line-interface-networks) command. The output will be similar to the following:
...
```{code-block} text
cinaclaw set local.bridged-network=eth0
```
...
```{code-block} text
cinaclaw stop ultimate-grosbeak
```

You can now ask cinaclaw to bridge your preferred network using the [`local.<instance-name>.bridged`](reference-settings-local-instance-name-bridged) setting:
...
```{code-block} text
cinaclaw set local.bridged-network=eth1
cinaclaw set local.ultimate-grosbeak.bridged=true
```
...
```{code-block} text
cinaclaw get local.ultimate-grosbeak.bridged
```
...
```{code-block} text
cinaclaw start ultimate-grosbeak
```
