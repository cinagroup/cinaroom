...
To open a shell prompt on an existing instance (e.g. `loving-duck`), run the command `cinaclaw shell loving-duck`. The output will be similar to the following:
...
If no argument is given to the `shell` command, cinaclaw will open a shell prompt on the primary instance (and also create it, if it doesn't exist).
...
To run a single command inside an instance, you don't need to open a shell. The command can be directly called from the host using `cinaclaw exec`. For example, the command `cinaclaw exec loving-duck -- pwd` returns:
...
An existing instance that is in `Stopped` or `Suspended` status can be started with the `cinaclaw start` command; for example:

```{code-block} text
cinaclaw start loving-duck
```

You can start multiple instances at once, specifying the instance names in the command line:

```{code-block} text
cinaclaw start loving-duck devoted-lionfish sensible-shark
```
...
If no options are specified, the `cinaclaw start` command starts the primary instance, creating it if needed.
...
```{code-block} text
cinaclaw suspend loving-duck
```

You can suspend multiple instances at once, specifying the instance names in the command line:

```{code-block} text
cinaclaw suspend loving-duck devoted-lionfish sensible-shark
```

To suspend all running instances at once, use the `--all` option:

```{code-block} text
cinaclaw suspend --all
```

If no options are specified, the `cinaclaw suspend` command suspends the primary instance, if it exists and is running.
...
If no options are specified, the `cinaclaw stop` command stops the primary instance, if it exists and is running.

### Stop an instance forcefully

If the `cinaclaw stop` command doesn’t work, you can use the `--force` argument to force the instance to shut down immediately. This is particularly useful when the virtual machine is in a non-responsive, unknown or suspended state.
...
```{note}
This command is also available on the cinaclaw GUI.
```
