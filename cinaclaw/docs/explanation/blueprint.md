...
In cinaclaw, a **blueprint** is a recipe to create a customised cinaclaw [instance](/explanation/instance).
...
instances:
  <name>:                  # * equal to the blueprint name
    image: <base image>    # a valid image alias, see `cinaclaw find` for available values
    limits:
...
Blueprints currently integrated into cinaclaw can be found with the [`find`](/reference/command-line-interface/find) command.

For more information on creating a blueprint for inclusion into cinaclaw, please refer to the [GitHub project](https://github.com/canonical/cinaclaw-blueprints).
