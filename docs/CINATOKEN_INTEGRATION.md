...
- ⏳ CinaSeek 需集成 CinaToken OAuth 实现 SSO
...
```
cinaseek.vms
cinaseek.vm_snapshots
cinaseek.vm_logs
cinaseek.vm_metrics
cinaseek.mounts
cinaseek.openclaw_configs
cinaseek.openclaw_logs
cinaseek.remote_access
cinaseek.remote_logs
cinaseek.ip_whitelists
cinaseek.system_settings
```

#### 4. API 路径调整

**原路径：** `/api/v1/auth/login`
**新路径：** `/api/cinaseek/v1/...`（移除认证相关接口）

**保留接口：**
- `GET /api/cinaseek/v1/vm/list` - 虚拟机列表
- `POST /api/cinaseek/v1/vm/create` - 创建虚拟机
- `POST /api/cinaseek/v1/vm/operate` - 虚拟机操作
- `GET /api/cinaseek/v1/mount/list` - 挂载列表
- `POST /api/cinaseek/v1/openclaw/deploy` - OpenClaw 部署
- `GET /api/cinaseek/v1/remote/status` - 远程访问状态
- ...（共 40 个业务接口）
...
dsn := "host=43.156.66.122 port=5432 user=cinaseek password=xxx dbname=cinatoken sslmode=require"
// 使用 cinaseek schema
db.Exec("SET search_path TO cinaseek, public")
```
