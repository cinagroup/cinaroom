...
with tempfile.TemporaryDirectory() as workdir:
    ...
    cinaseek_gui_component = "cinaseek_gui"

    for path in x86_work.glob("*.pkg/Payload.unpacked/**/*"):
        if path.is_dir() or cinaseek_gui_component in str(path.absolute()):
            continue
    ...
    pkgs.mkdir()
    for component in ("cinaseek", "cinaseekd"):
        pkg = dest_work / f"cinaseek-{version}-Darwin-{component}.pkg"
        subprocess.check_call(["pkgbuild", "--root", pkg / "Payload.unpacked",
                               "--identifier", f"com.canonical.cinaseek.{component}",
                               "--scripts", pkg / "Scripts",
                               "--version", version,
                               "--install-location", "/",
                               pkgs / pkg.name])

    # Handle cinaseek-gui seperately since we need to pass in a component plist
    pkg = dest_work / f"cinaseek-{version}-Darwin-{cinaseek_gui_component}.pkg"
    ...
    subprocess.check_call(["plutil", "-replace", "BundleIsRelocatable",
                           "-bool", "false",
                            component_plist])
    subprocess.check_call(["pkgbuild", "--root", pkg / "Payload.unpacked",
                           "--identifier", f"com.canonical.cinaseek.{cinaseek_gui_component}",
                           "--scripts", pkg / "Scripts",
