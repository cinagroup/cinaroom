...
function codesign_binaries {
    ...
    find "${DIR}" -type f -name qemu-system-* -print0 | xargs -0L1 \
        codesign -v --timestamp --options runtime --force --strict \
            ...
            --identifier com.canonical.cinaseek.qemu \
            --sign "${SIGN_APP}"

    # sign cinaseek with additional entitlements for using the Apple Virtualization framework
    find "${DIR}" -type f -name cinaseekd -print0 | xargs -0L1 \
        codesign -v --timestamp --options runtime --force --strict \
            $( entitlements com.apple.security.virtualization ) \
            --identifier com.canonical.cinaseek.cinaseekd \
            --sign "${SIGN_APP}"
