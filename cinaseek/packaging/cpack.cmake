...
set(CPACK_COMPONENTS_ALL cinaseekd cinaseek cinaseek_gui)
...
set(CPACK_COMPONENT_MULTIPASS_DESCRIPTION
   "Command line tool to talk to the cinaseek daemon")
...
set(CPACK_PACKAGE_NAME              "cinaseek")
...
if (MSVC)
  # cinaseekd depends on qemu-img.exe indirectly (called to convert qcow images to VHDX format)
  ...
  set(CMAKE_INSTALL_SYSTEM_RUNTIME_COMPONENT cinaseekd)
  ...
  install(FILES "${REAL_QEMU_IMG}" DESTINATION bin COMPONENT cinaseekd)
...
if(APPLE)
  ...
  set(CPACK_PACKAGING_INSTALL_PREFIX   "/Library/Application Support/com.canonical.cinaseek")
  ...
  set(MULTIPASSD_PLIST "com.canonical.cinaseekd.plist")
  configure_file("${CMAKE_SOURCE_DIR}/packaging/macos/${MULTIPASSD_PLIST}.in"
                 "${CMAKE_BINARY_DIR}/${MULTIPASSD_PLIST}" @ONLY)
  configure_file("${CMAKE_SOURCE_DIR}/packaging/macos/preinstall-cinaseekd.sh.in"
                 "${CMAKE_BINARY_DIR}/preinstall-cinaseekd.sh" @ONLY)
  configure_file("${CMAKE_SOURCE_DIR}/packaging/macos/postinstall-cinaseekd.sh.in"
                 "${CMAKE_BINARY_DIR}/postinstall-cinaseekd.sh" @ONLY)
  configure_file("${CMAKE_SOURCE_DIR}/packaging/macos/postinstall-cinaseek.sh.in"
                 "${CMAKE_BINARY_DIR}/postinstall-cinaseek.sh" @ONLY)
  configure_file("${CMAKE_SOURCE_DIR}/packaging/macos/postinstall-cinaseek-gui.sh.in"
                 "${CMAKE_BINARY_DIR}/postinstall-cinaseek-gui.sh" @ONLY)

  install(FILES "${CMAKE_BINARY_DIR}/${MULTIPASSD_PLIST}" DESTINATION Resources COMPONENT cinaseekd)
  install(DIRECTORY "${CMAKE_SOURCE_DIR}/completions" DESTINATION Resources COMPONENT cinaseek)
  install(DIRECTORY "${CMAKE_BINARY_DIR}/lib/" DESTINATION lib COMPONENT cinaseekd)

  set(CPACK_COMPONENT_MULTIPASS_GUI_PLIST "${CMAKE_SOURCE_DIR}/packaging/macos/cinaseek-gui-component.plist")

  set(CPACK_PREFLIGHT_MULTIPASSD_SCRIPT  "${CMAKE_BINARY_DIR}/preinstall-cinaseekd.sh")
  set(CPACK_POSTFLIGHT_MULTIPASSD_SCRIPT "${CMAKE_BINARY_DIR}/postinstall-cinaseekd.sh")
  set(CPACK_POSTFLIGHT_MULTIPASS_SCRIPT  "${CMAKE_BINARY_DIR}/postinstall-cinaseek.sh")
  set(CPACK_POSTFLIGHT_MULTIPASS_GUI_SCRIPT  "${CMAKE_BINARY_DIR}/postinstall-cinaseek-gui.sh")
  ...
  install(FILES "${CMAKE_SOURCE_DIR}/packaging/macos/uninstall.sh"
          DESTINATION . COMPONENT cinaseekd)
endif()
