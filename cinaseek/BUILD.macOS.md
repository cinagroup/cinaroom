...
**Important: RVM/Ruby may require OpenSSL v1, while building cinaseek requires OpenSSL v3. Make sure to switch to the right version of OpenSSL as necessary with `brew link`**
...
Building a cinaseek package requires cmake 3.9 or greater. The most convenient
...
Dylibbundler is required to make cinaseek packages:

    brew install dylibbundler

#### QEMU Configuration

QEMU is required to build the cinaseek tests:
...
```
python3 -m venv ~/cinaseek-build-env
source ~/cinaseek-build-env/bin/activate
pip install distlib
```
...
Once it is complete, you will have a cinaseek.pkg file in the build directory.
...
----------------------
To get console output from cinaseek while installed, from one shell run:

    sudo launchctl debug system/com.canonical.cinaseekd --stdout --stderr

and then from another shell, restart the cinaseekd daemon with

    sudo launchctl kickstart -k system/com.canonical.cinaseekd

The console output from cinaseekd will appear on the former shell.
