...
```{code-block} text
cinaseek mount <local path> <instance name>
```
...
```{code-block} text
cinaseek mount $HOME keen-yak
```

You can check the result running `cinaseek info keen-yak`:
...
```{code-block} text
cinaseek mount $HOME keen-yak:/some/path
```
...
```{code-block} text
cinaseek launch --mount /local/path:/instance/path
```
...
```{code-block} text
cinaseek transfer keen-yak:/etc/crontab keen-yak:/etc/fstab /home/michal
```
...
```{code-block} text
cinaseek transfer /etc/crontab /etc/fstab keen-yak:/home/michal
```
