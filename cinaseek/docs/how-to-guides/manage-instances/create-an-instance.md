...
This document demonstrates various ways to create an instance with cinaseek. While every method is a one-liner involving the command `cinaseek launch`, each showcases a different option that you can use to get exactly the instance that you want.

## Create an instance

> See also: [`launch`](reference-command-line-interface-launch), [`info`](reference-command-line-interface-info)

To create an instance with cinaseek, run the command `cinaseek launch`. This launches a new instance, which is randomly named; for example:
...
In particular, when we run `cinaseek info keen-yak`, we find out that it is an Ubuntu LTS release, namely 18.04, with 1GB RAM, 1 CPU, 5GB of disk:
...
To find out which images are available, run `cinaseek find`. Here's a sample output:
...
To launch an instance with a specific image, include the image name or alias in the command, for example `cinaseek launch jammy`:
...
`cinaseek info tenacious-mink` confirms that we've launched an instance of the selected image.
...
To launch an instance with a specific name, add the `--name` option to the command line; for example `cinaseek launch kinetic --name helpful-duck`:
...
```{code-block} text
cinaseek launch --cpus 4 --disk 20G --memory 8G
```
...
```{code-block} text
cinaseek launch kinetic --name primary
```
...
cinaseek can create instances with additional network interfaces using the `cinaseek launch` command with the `--network` option. This is complemented by the [`networks`](reference-command-line-interface-networks) command, that you can use to find available host networks to bridge with.
...
* on Linux, with QEMU (*from cinaseek 1.15 onward*)
...
```{code-block} text
cinaseek launch --network en0 --network name=bridge0,mode=manual
```
...
```{code-block} text
cinaseek exec upbeat-whipsnake -- ip -br address show scope global
```
...
- `enp0s3` - The default interface, that the instance can use to reach the outside world and that cinaseek uses to communicate with the instance.
...
For example, if the command `cinaseek exec upbeat-whipsnake -- ip route` returns the following routing table:
...
```{code-block} text
cinaseek exec upbeat-whipsnake -- ip route get 91.189.88.181
```
...
On Linux, when trying to connect an instance network to an ethernet device on the host, cinaseek will offer to create the required bridge.

First, run the `cinaseek networks` command; for example:
...
Then, select an ethernet device and launch a new instance requesting to connect to it, for example via `cinaseek launch --network eth0`. The output will be:

```{code-block} text
cinaseek needs to create a bridge to connect to eth0.
This will temporarily disrupt connectivity on that interface.

Do you want to continue (yes/no)?
```

However, cinaseek requires `NetworkManager` to achieve this. On installations that do not have `NetworkManager` installed (e.g. Ubuntu Server), the user can still create a bridge by other means and pass that to cinaseek. For instance, this configuration snippet achieves that with Netplan:
...
That goes somewhere in `/etc/netplan/` (e.g. `/etc/netplan/50-custom.yaml`). After a successful `netplan try` or `netplan apply`, cinaseek will show the new bridge with the `networks` command and instances can be connected to it:

```{code-block} text
cinaseek launch --network mybridge
```
...
```{code-block} text
cinaseek launch --cloud-init systemd-resolved.yaml
```
