...
To open a shell prompt on an existing instance (e.g. `loving-duck`), run the command `cinaseek shell loving-duck`. The output will be similar to the following:
...
If no argument is given to the `shell` command, cinaseek will open a shell prompt on the primary instance (and also create it, if it doesn't exist).
...
To run a single command inside an instance, you don't need to open a shell. The command can be directly called from the host using `cinaseek exec`. For example, the command `cinaseek exec loving-duck -- pwd` returns:
...
An existing instance that is in `Stopped` or `Suspended` status can be started with the `cinaseek start` command; for example:

```{code-block} text
cinaseek start loving-duck
```

You can start multiple instances at once, specifying the instance names in the command line:

```{code-block} text
cinaseek start loving-duck devoted-lionfish sensible-shark
```
...
If no options are specified, the `cinaseek start` command starts the primary instance, creating it if needed.
...
```{code-block} text
cinaseek suspend loving-duck
```

You can suspend multiple instances at once, specifying the instance names in the command line:

```{code-block} text
cinaseek suspend loving-duck devoted-lionfish sensible-shark
```

To suspend all running instances at once, use the `--all` option:

```{code-block} text
cinaseek suspend --all
```

If no options are specified, the `cinaseek suspend` command suspends the primary instance, if it exists and is running.
...
If no options are specified, the `cinaseek stop` command stops the primary instance, if it exists and is running.

### Stop an instance forcefully

If the `cinaseek stop` command doesn’t work, you can use the `--force` argument to force the instance to shut down immediately. This is particularly useful when the virtual machine is in a non-responsive, unknown or suspended state.
...
```{note}
This command is also available on the cinaseek GUI.
```
