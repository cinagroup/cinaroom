...
```{code-block} text
cinaseek delete keen-yak
```

Now, if you run `cinaseek list` to list the instances, you will see that it is actually just marked for deletion (or to put it in other words, moved to the recycle bin):
...
```{code-block} text
cinaseek recover keen-yak
```

If you try `cinaseek list` again, you'll see that the instance is no longer marked for deletion:
...
```{code-block} text
cinaseek delete --purge keen-yak
```
