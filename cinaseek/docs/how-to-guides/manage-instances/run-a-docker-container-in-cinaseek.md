(how-to-guides-manage-instances-run-a-docker-container-in-cinaseek)=
# Run a Docker container in cinaseek

```{Warning}
Blueprints have been removed as a feature. You can achieve similar effects with cloud-init and other launch options. Find out more at: [Launch customized instances with cinaseek and cloud-init](how-to-guides-manage-instances-launch-customized-instances-with-cinaseek-and-cloud-init)
```
<!-- This is published as an Ubuntu Tutorial at this link: https://ubuntu.com/tutorials/running-a-container-with-the-docker-workflow-in-cinaseek -->

<!--
| Key | Value |
| --- | --- |
| Summary | Running a Docker Container in cinaseek |
| Categories | cinaseek |
| Difficulty | 2 |
...
cinaseek has a Docker blueprint that gives its users access to out-of-the-box Docker on any platform. This new blueprint makes it easy to develop and test Docker containers locally on macOS, Windows, or Linux.

In this tutorial, you will see how to get started with the Docker blueprint by creating a blog in a Docker container in cinaseek.

### What you’ll learn

- How to use Docker on macOS or Windows with cinaseek

- How to alias the `docker` command to your host command line

- How to use [Portainer](https://www.portainer.io/) to launch a Docker container in cinaseek.
...
Let's start by installing cinaseek on your machine, following the steps in [How to install cinaseek](how-to-guides-install-cinaseek).
...
Now that cinaseek is installed, you can create a VM running Docker very simply. Open up a terminal and type

```{code-block} text
cinaseek launch docker
```
...
```{code-block} text
cinaseek exec docker docker
```

```{figure} /images/run-a-docker-container-in-cinaseek/mp-docker-2.png
   :width: 720px
...
```{code-block} text
PATH="$PATH:/home/<user>/snap/cinaseek/common/bin"
```
...
aliases to work without prefixing with `cinaseek`:

PATH="$PATH:/home/nhart/snap/cinaseek/common/bin"
```
...
```{figure} /images/run-a-docker-container-in-cinaseek/mp-docker-4.png
   :width: 720px
   :alt: Screenshot of terminal output
```
...
```{figure} /images/run-a-docker-container-in-cinaseek/mp-docker-5.png
   :width: 720px
...
```{figure} /images/run-a-docker-container-in-cinaseek/mp-docker-6.png
   :width: 720px
...
```{figure} /images/run-a-docker-container-in-cinaseek/mp-docker-7.png
   :width: 720px
...
```{figure} /images/run-a-docker-container-in-cinaseek/mp-docker-8.png
   :width: 720px
...
```{figure} /images/run-a-docker-container-in-cinaseek/mp-docker-9.png
   :width: 720px
...
```{figure} /images/run-a-docker-container-in-cinaseek/mp-docker-10.png
   :width: 720px
   :alt: Ghost homepage
```
...
There it is, your blog running within a Docker container inside cinaseek!

For next steps, try out Portainer’s other App Templates (Step 5), or check out [Docker Hub](https://hub.docker.com/search?type=image) for more containers to try. If you want to try out container orchestration, [Microk8s](https://microk8s.io/) or cinaseek’ [`Minikube`](https://minikube.sigs.k8s.io/docs/) blueprint are great places to start.
