(how-to-guides-customise-cinaseek-build-cinaseek-images-with-packer)=
# Build cinaseek images with Packer

```{note}
Custom images are only supported on Linux.
```

[Packer](https://developer.hashicorp.com/packer) is a utility that lets you (re)build images to run in a variety of environments. cinaseek can run those images too, provided some requirements are met (namely, the image has to boot on the hypervisor in use, and [cloud-init](https://cloudinit.readthedocs.io/en/latest/) needs to be available).
...
cinaseek launch file://$PWD/output-qemu/packer-qemu --disk 5G
```

Now, shell into the new instance that was created, for example:

```{code-block} text
cinaseek shell tolerant-hammerhead
```

## Customising the image

Now the above works for you, delete the test instance with `cinaseek delete --purge tolerant-hammerhead` and edit the following section in the `template.json` file:
...
Join the discussion on the [cinaseek forum](https://discourse.ubuntu.com/c/project/cinaseek/21/) and let us know about your images!
