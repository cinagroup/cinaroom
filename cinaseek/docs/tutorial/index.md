...
cinaseek is a flexible and powerful tool that can be used for many purposes. In its simplest form, you can use it to quickly create and destroy Ubuntu VMs (instances) on any host machine. But you can also use cinaseek to build a local mini-cloud on your laptop, to test and develop multi-instance or container-based cloud applications.

This tutorial will help you understand how cinaseek works, and the skills you need to use its main features.

## Install cinaseek

cinaseek is available for Linux, macOS and Windows. To install it on the OS of your choice, please follow the instructions provided in [How to install cinaseek](/how-to-guides/install-cinaseek).
...
Start cinaseek from the application launcher. In Ubuntu, press the super key and type "cinaseek", or find cinaseek in the Applications panel on the lower left of the desktop.

```{figure} /images/tutorial/mp-linux-1.png
   :width: 800px
   :alt: Find cinaseek in the Applications panel
```
...
After launching the application, you should see the cinaseek tray icon on the upper right section of the screen.

```{figure} /images/tutorial/mp-linux-2.png
   :width: 688px
   :alt: cinaseek tray icon
```
...
Click on the cinaseek icon and select **Open Shell**.

```{figure} /images/tutorial/mp-linux-2a.png
   :width: 286px
   :alt: cinaseek tray icon - Select "Open shell"
```
...
<!-- note added for https://github.com/canonical/cinaseek/issues/3537 -->
...
Let's test it. As you've just learnt, the previous step automatically mounted your `$HOME` directory in the instance. Use this to share data with your instance. More concretely, create a new folder called `cinaseek_Files` in your `$HOME` directory:

```{figure} /images/tutorial/mp-linux-3.png
   :width: 720px
   :alt: Create new "cinaseek_Files" folder
```
...
Start cinaseek from the application launcher. In macOS, open the application launcher, type "cinaseek", and launch the application.

```{figure} /images/tutorial/mp-macos-1.png
   :width: 720px
   :alt: Launch the cinaseek application
```
...
After launching the application, you should see the cinaseek tray icon in the upper right section of the screen.
...
Let’s test it out. As you've just learnt, the previous step automatically mounted your `$HOME` directory in the instance. Use this to share data with your instance. More concretely, create a new folder called `cinaseek_Files` in your `$HOME` directory.
...
```{code-block} text
ubuntu@primary:~$ cd ./Home/cinaseek_Files/
ubuntu@primary:~/Home/cinaseek_Files$ cat README.md
## Shared Folder

This folder could be a great place to keep files that need to be accessed by both your host machine and Ubuntu VM!

ubuntu@primary:~/Home/cinaseek_Files$
```
...
Start cinaseek from the application launcher. Press the Windows key and type "cinaseek", then launch the application.
...
After launching the application, you should see the cinaseek tray icon in the lower right section of the screen (you may need to click on the small arrow located there).
...
This instance is great for when you just need a quick Ubuntu VM, but let's say you want a more customised instance, how can you do that? cinaseek has you covered there too.

```{dropdown} Optional Exercises

Exercise 1:
When you select Open Shell, what happens in the background is the equivalent of the CLI commands `cinaseek launch –name primary`  followed by  `cinaseek shell`. Open a terminal and try `cinaseek shell` (if you didn't follow the steps above, you will have to run the `launch` command first).

Exercise 2:
In cinaseek, an instance with the name "primary" is privileged. That is, it serves as the default argument of `cinaseek shell` among other capabilities. In different terminal instances, check `cinaseek shell primary` and `cinaseek shell`. Both commands should give the same result.
```

## Create a customised instance

cinaseek has a great feature to help you get started with creating customised instances. Open a terminal and run the `cinaseek find` command. The result shows a list of all images you can currently launch through cinaseek.
...
Launch an instance running Ubuntu 22.10 ("Kinetic Kudu") by typing the `cinaseek launch kinetic` command.
...
Now, you have an instance running and it has been named randomly by cinaseek. In this case, it has been named "coherent-trumpetfish".

```{code-block} text
$ cinaseek launch kinetic
Launched: coherent-trumpetfish
```

You can check some basic info about your new instance by running the following command:

`cinaseek exec coherent-trumpetfish -- lsb_release -a`

This tells cinaseek to run the command `lsb_release -a` on the "coherent-trumpetfish" instance.
...
`cinaseek delete coherent-trumpetfish`

You can now launch the type of instance you need by running this command:

`cinaseek launch lts --name ltsInstance --memory 2G --disk 10G --cpus 2`
...
Now, you have an instance running and it has been named randomly by cinaseek. In this case, it has been named "breezy-liger".
...
`cinaseek exec breezy-liger -- lsb_release -a`

This tells cinaseek to run the command `lsb_release -a` on the “breezy-liger” instance.
...
Now, you have an instance running and it has been named randomly by cinaseek. In this case, it has been named "decorous-skate".

```{code-block} text
C:\WINDOWS\system32> cinaseek launch kinetic
Launched: decorous-skate
```

You can check some basic info about your new instance by running the following command:

`cinaseek exec decorous-skate -- lsb_release -a`

This tells cinaseek to run the command `lsb_release -a` on the “decorous-skate” instance.

```{code-block} text
C:\WINDOWS\system32> cinaseek exec decorous-skate -- lsb_release -a
No LSB modules are available.
Distributor ID:	Ubuntu
Description:	Ubuntu 22.10
Release:		22.10
...
`cinaseek delete decorous-skate`
...
You can confirm that the new instance has the specs you need by running `cinaseek info ltsInstance`.
...
You've created and deleted quite a few instances. It is time to run `cinaseek list` to see the instances you currently have.
...
The result shows that you have two instances running, the "primary" instance and the LTS machine with customised specs. The "coherent-trumpetfish" instance is still listed, but its state is "Deleted". You can recover this instance by running `cinaseek recover coherent-trumpetfish`. But for now, delete the instance permanently by running `cinaseek purge`. Then run `cinaseek list` again to confirm that the instance has been permanently deleted.
...
The result shows that you have two instances running, the "primary" instance and the LTS machine with customised specs. The "breezy-liger" instance is still listed, but its state is "Deleted". You can recover this instance by running `cinaseek recover breezy-liger`. But for now, delete the instance permanently by running `cinaseek purge`. Then run `cinaseek list` again to confirm that the instance has been permanently deleted.
...
```{code-block} text
C:\WINDOWS\system32> cinaseek info ltsInstance
Name:           ltsInstance
State:          Running
IPv4:           172.22.115.152
Release:        Ubuntu 22.04.1 LTS
...
```{code-block} text
C:\WINDOWS\system32> cinaseek list
Name                    State             IPv4             Image
primary                 Running           10.110.66.242    Ubuntu 22.04 LTS
...
The result shows that you have two instances running, the "primary" instance and the LTS machine with customised specs. The "decorous-skate" instance is still listed, but its state is "Deleted". You can recover this instance by running `cinaseek recover decorous-skate`. But for now, delete the instance permanently by running `cinaseek purge`. Then run `cinaseek list` again to confirm that the instance has been permanently deleted.
...
Let's see some practical examples of what you can do with your cinaseek instances:
...
One way to put a cinaseek instance to use is by running a local web server in it.

Return to your customised LTS instance. Take note of its IP address, which was revealed when you ran `cinaseek list`. Then run `cinaseek shell ltsInstance` to open a shell in the instance.
...
Just like that, you've got a web server running in a cinaseek instance!
...
```
Some environments require a lot of configuration and setup. cinaseek Blueprints are instances with a deep level of customisation. For example, the Docker Blueprint is a pre-configured Docker environment with a Portainer container already running.

You can launch an instance using the Docker Blueprint by running `cinaseek launch docker --name docker-dev`.

Once that's done, run `cinaseek info docker-dev` to note down the IP of the new instance.
...
```{code-block} text
$ cinaseek launch docker --name docker-dev
Launched: docker-dev
$ cinaseek info docker-dev
Name:           docker-dev
State:          Running
IPv4:           10.115.5.235
                172.17.0.1
...
From the Portainer dashboard, you can see the ports available on nginx. To verify that you have nginx running in a Docker container inside cinaseek, open a new web page and paste the IP address of your instance followed by one of the port numbers.
...
```{code-block} text
C:\WINDOWS\system32> cinaseek launch docker --name docker-dev
Launched: docker-dev
C:\WINDOWS\system32> cinaseek info docker-dev
Name:           docker-dev
State:          Running
IPv4:           10.115.5.235
                172.17.0.1
...
Congratulations! You can now use cinaseek proficiently.

There's more to learn about cinaseek and its capabilities. Check out our [How-to guides](/how-to-guides/index) for ideas and help with your project. In our [Explanation](/explanation/index) and [Reference](/reference/index) pages you can find definitions of key concepts, a complete CLI command reference, settings options and more.

Join the discussion on the [cinaseek forum](https://discourse.ubuntu.com/c/project/cinaseek/21/) and let us know what you are doing with your instances!
