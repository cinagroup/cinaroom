...
```{caution}
This setting has been removed from the CLI starting from cinaseek version 1.14.
...
- Some combinations may be grabbed by the OS before they reach cinaseek (e.g. `meta+a` may open the Applications, `ctrl+alt+f3` may move ttys).
```

## Examples

  * `cinaseek set client.gui.hotkey="Ctrl+Print Screen"`.
  * `cinaseek set client.gui.hotkey="⌃⇧Y"`.
  * `cinaseek set client.gui.hotkey=option+space`.
  * `cinaseek set client.gui.hotkey=""`
