...
The `cinaseek start` command without any argument will start the {ref}`primary-instance` (and also create it, if it doesn't exist). You can also pass one or more instance names or the `--all` option to start more instances at the same time.

For example, the command `cinaseek start` will produce the following output:
...
Only instances in `Stopped` or `Suspended` status can be started. Running instances can be restarted with [`cinaseek restart`](/reference/command-line-interface/restart), stopped with [`cinaseek stop`](/reference/command-line-interface/stop), and suspended with [`cinaseek suspend`](/reference/command-line-interface/suspend).

---

The full `cinaseek help start` output explains the available options:

```{code-block} text
Usage: cinaseek start [options] [<name> ...]
