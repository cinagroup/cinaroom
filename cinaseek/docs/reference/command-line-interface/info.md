...
The `cinaseek info` command shows properties of instances and snapshots.

For example, `cinaseek info calm-squirrel`:
...
Mounts:         /home/user => Home
                ...
                /home/user/cinaseek => mp
                    UID map: 1000:default
                    GID map: 1001:default
```

or, in case of a snapshot, `cinaseek info relative-lion.snapshot3`:
...
With no positional arguments, `cinaseek info` displays information on all the instances. If you add the `--snapshots` flag and no positional arguments, the command displays information on all the snapshots of all instances.
...
For example, the command `cinaseek info --format yaml calm-squirrel` produces the following output:
...
Here you can find a comprehensive list of the details that `cinaseek info` provides for instances and snapshots, respectively.
...
| `IPv4` | A list of IPs through which a machine on the same network as the instance can reach the instance. Adding IP addresses besides the one automatically generated by cinaseek is described in {ref}`create-an-instance-with-multiple-network-interfaces`. |
...
The full `cinaseek help info` output explains the available options:
```{code-block} text
Usage: cinaseek info [options] <instance>[.snapshot] [<instance>[.snapshot] ...]
