...
The `cinaseek restore` command restores an instance to the state that it was in when the given snapshot was taken.

For example, when you run the command:

```{code-block} text
cinaseek restore relative-lion.snapshot2
```
...
```{code-block} text
cinaseek info relative-lion.snapshot3 | grep Comment
```
...
The full `cinaseek help restore` output explains the available options:

```{code-block} text
Usage: cinaseek restore [options] <instance>.<snapshot>
