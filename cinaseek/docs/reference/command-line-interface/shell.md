...
The `cinaseek shell` command will open a shell prompt on an instance. Without any arguments, it will open the shell prompt of the {ref}`primary-instance` (and also create it, if it doesn't exist). You can also pass the name of an existing instance. If the instance is not running, it will be started automatically.

If you run `cinaseek shell` you'll find yourself in the *primary* instance:
...
```{code-block} text
cinaseek shell <instance name>
```

---

The full `cinaseek help shell` output explains the available options:

```{code-block} text
Usage: cinaseek shell [options] [<name>]
