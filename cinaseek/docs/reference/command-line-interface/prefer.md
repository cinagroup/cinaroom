...
The `prefer` command, introduced in cinaseek release 1.11.0,  is used to switch alias contexts. It accepts a single argument, specifying the name of the context to switch to.

For example, the command `cinaseek prefer secondary` makes `secondary` the current context. If the given context does not exist, cinaseek creates it.
...
The full `cinaseek help prefer` output explains the available options:

```{code-block} text
Usage: cinaseek prefer [options] <name>
