...
The `cinaseek snapshot` command takes a snapshot of an instance; for example:

```{code-block} text
cinaseek snapshot maximal-stag
```
...
The full `cinaseek help snapshot` output explains the available options:

```{code-block} text
Usage: cinaseek snapshot [options] instance
