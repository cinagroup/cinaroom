...
The `cinaseek stop` command without any argument will stop the {ref}`primary-instance`
...
`Stopped` instances can be started again with [`cinaseek start`](/reference/command-line-interface/start).

You can use the `--force` option (*available starting from cinaseek version 1.14*) to stop the instance when it is in a non-responsive, unknown or suspended state. It can also be used with the `--all` option to stop all instances forcefully.

---

The full `cinaseek help stop` output explains the available options:

```{code-block} text
Usage: cinaseek stop [options] [<name> ...]
