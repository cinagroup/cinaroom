...
> See also: [Driver (backend)](/explanation/driver), [How to set up the driver](/how-to-guides/customise-cinaseek/set-up-the-driver)

The `cinaseek networks` command lists network interfaces that cinaseek can connect instances to. The result depends both on the platform and the driver in use.

At this time, `cinaseek networks` can only find interfaces in the following scenarios:

- on Linux, with QEMU
- on Windows, with both Hyper-V and VirtualBox
- on macOS, with the QEMU and VirtualBox drivers

For example, on Windows with Hyper-V the `cinaseek networks` command returns:
...
Another example, running the command `cinaseek networks --format yaml` on macOS with VirtualBox returns:
...
The `cinaseek help networks` command explains the available options:

```{code-block} text
Usage: cinaseek networks [options]
