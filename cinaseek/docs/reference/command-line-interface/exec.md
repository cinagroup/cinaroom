...
> See also: [cinaseek `exec` and shells](/explanation/cinaseek-exec-and-shells), [How to use instance command aliases](/how-to-guides/manage-instances/use-instance-command-aliases)

The `exec` command runs the given commands inside the instance. The first argument is the instance to run the commands on, `--` optionally separates the `cinaseek` options from the rest - the command to run itself:

```{code-block} text
cinaseek exec primary -- uname -r
```
...
```{code-block} text
cinaseek exec primary -- lsb_release -a | grep ^Codename:
```
...
The first one is `--working-directory <dir>`, which tells cinaseek that the command must be run in the folder `<dir>`. For example:

```{code-block} text
cinaseek exec arriving-pipefish --working-directory /home -- ls -a
```
...
The full `cinaseek help exec` output explains the available options:

```{code-block} text
Usage: cinaseek exec [options] <name> [--] <command>
