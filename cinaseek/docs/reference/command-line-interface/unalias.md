...
The `cinaseek unalias` command removes a previously defined alias.

```{code-block} text
cinaseek unalias name
```
...
```{code-block} text
cinaseek unalias name1 name2 name3
```
...
The full `cinaseek help unalias` output explains the available options:

```{code-block} text
Usage: cinaseek unalias [options] <name> [<name> ...]
Remove aliases
