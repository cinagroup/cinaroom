...
The `alias` command makes cinaseek create a persistent alias to run commands on a given instance. Its syntax is the following:

```{code-block} text
cinaseek alias instance:command [name]
```
...
The full `cinaseek help alias` output explains the available options:

```{code-block} text
Usage: cinaseek alias [options] <definition> [<name>]
