...
The `cinaseek wait-ready` command blocks execution until the cinaseek daemon is fully initialized and ready to accept requests. This is particularly useful in batch operations and automated environments such as CI/CD pipelines, where subsequent cinaseek commands may fail if the daemon has not yet finished starting up.
...
```bash
cinaseek wait-ready --timeout 30
```

---

The full `cinaseek help wait-ready` output explains the available options:

```{code-block} text

Usage: cinaseek wait-ready [options]
Wait for the cinaseek daemon to be ready. This command will block until the
...
```bash
cinaseek wait-ready && cinaseek launch
```
