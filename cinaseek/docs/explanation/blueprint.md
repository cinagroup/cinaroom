...
In cinaseek, a **blueprint** is a recipe to create a customised cinaseek [instance](/explanation/instance).
...
instances:
  <name>:                  # * equal to the blueprint name
    image: <base image>    # a valid image alias, see `cinaseek find` for available values
    limits:
...
Blueprints currently integrated into cinaseek can be found with the [`find`](/reference/command-line-interface/find) command.

For more information on creating a blueprint for inclusion into cinaseek, please refer to the [GitHub project](https://github.com/canonical/cinaseek-blueprints).
