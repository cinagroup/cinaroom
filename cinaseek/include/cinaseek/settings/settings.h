...
#define MP_SETTINGS cinaseek::Settings::instance()
...
public:
    ...
    virtual SettingsHandler* register_handler(
        std::unique_ptr<SettingsHandler> handler);             // no nulls please
    ...
    /**
     * Get the value of the setting specified by @c key, as returned by the first registered handler
     * that handles it.
     * @param key The key identifying the requested setting.
     * @return A string representation of the value of the specified setting, according to the
     * corresponding SettingsHandler's interpretation.
    ...
    /**
     * Set the value of the setting specified by @c key to @val, according to the interpretation of
     * first registered handler that handles the respective setting.
     ...
     * @return The value that the setting converts to, when interpreted as type @p T. Follows the
     * conversion rules used by @c QVariant. If the current @em value cannot be converted, a default
     * @p T value is returned.
     * @throws UnsupportedSettingValueType<T> If QVariant cannot convert from a @c QString to type
     * @c T. Note that this is only thrown when the type itself can't be converted to. The actual
     * value may still fail to convert, in which case a default @c T value is returned.
     */
    template <typename T>
...
template <typename T>
T cinaseek::Settings::get_as(const QString& key) const
{
    auto var = QVariant{get(key)};
    if (var.canConvert<T>()) // Note: this only checks if the types themselves are convertible, not
                             // the values
        return var.value<T>();
