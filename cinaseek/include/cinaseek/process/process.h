...
#include <cinaseek/utils/saturate_cast.h>
...
signals:
    void started();
    void finished(cinaseek::ProcessState process_state);
...
Q_DECLARE_METATYPE(cinaseek::ProcessState)

inline bool cinaseek::Process::wait_for_started(std::chrono::milliseconds ms)
{
    return wait_for_started(saturate_cast<int>(ms.count()));
}

inline bool cinaseek::Process::wait_for_finished(std::chrono::milliseconds ms)
{
    return wait_for_finished(saturate_cast<int>(ms.count()));
}

inline bool cinaseek::Process::wait_for_ready_read(std::chrono::milliseconds ms)
{
