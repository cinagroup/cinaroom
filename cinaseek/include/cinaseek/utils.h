...
#include <cinaseek/logging/level.h>
#include <cinaseek/network_interface_info.h>
...
#define MP_UTILS cinaseek::Utils::instance()
...
std::string contents_of(const cinaseek::Path& file_path);
...
bool process_log_on_error(const QString& program,
                          ...
                          cinaseek::logging::Level level = cinaseek::logging::Level::debug,
                          const int timeout = 30000);
...
public:
    ...
    virtual std::string contents_of(const cinaseek::Path& file_path) const;
...
namespace cinaseek::utils::detail
{
...
template <typename Str, typename Filter>
Str&& cinaseek::utils::trim_begin(Str&& s, Filter&& filter)
{
...
template <typename Str>
Str&& cinaseek::utils::trim_begin(Str&& s)
{
    return trim_begin(std::forward<Str>(s), detail::is_space);
}

template <typename Str, typename Filter>
Str&& cinaseek::utils::trim_end(Str&& s, Filter&& filter)
{
...
template <typename Str>
Str&& cinaseek::utils::trim_end(Str&& s)
{
    return trim_end(std::forward<Str>(s), detail::is_space);
}

template <typename Str, typename Filter>
Str&& cinaseek::utils::trim(Str&& s, Filter&& filter)
{
...
template <typename Str>
Str&& cinaseek::utils::trim(Str&& s)
{
    return trim(std::forward<Str>(s), detail::is_space);
}

inline bool cinaseek::utils::iequals(std::string_view lhs, std::string_view rhs)
{
...
template <typename OnTimeoutCallable, typename TryAction, typename... Args>
void cinaseek::utils::try_action_for(OnTimeoutCallable&& on_timeout,
                                      std::chrono::milliseconds timeout,
                                      TryAction&& try_action,
                                      Args&&... args)
{
...
template <typename RegisteredQtEnum>
QString cinaseek::utils::qenum_to_qstring(RegisteredQtEnum val)
{
    return QVariant::fromValue(val).toString();
}

template <typename RegisteredQtEnum>
std::string cinaseek::utils::qenum_to_string(RegisteredQtEnum val)
{
