...
#include <cinaseek/callable_traits.h>
#include <cinaseek/cli/return_codes.h>
...
protected:
                               ...
                               StreamingCallback&& streaming_callback)
    {
        ...
        using Arg0Type =
            typename cinaseek::callable_traits<SuccessCallable>::template arg<0>::type;
        ...
        else
        {
            ...
            const auto tokens = cinaseek::utils::split(context.peer(), ":");
            if (tokens[0] == "unix")
            {
                socket_address = tokens[1];
                QLocalSocket cinaseekd_socket;
                cinaseekd_socket.connectToServer(QString::fromStdString(socket_address));
                if (!cinaseekd_socket.waitForConnected() &&
                    cinaseekd_socket.error() == QLocalSocket::SocketAccessError)
                {
                    grpc::Status denied_status{
                        grpc::StatusCode::PERMISSION_DENIED,
                        "cinaseek socket access denied",
            ...
            grpc::Status access_error_status{
                ...
                "cannot connect to the cinaseek socket",
                fmt::format("Please ensure cinaseekd is running and '{}' is accessible",
                            socket_address)};
...
private:
    ...
    {
        using SuccessCallableTraits = cinaseek::callable_traits<SuccessCallable>;
        using FailureCallableTraits = cinaseek::callable_traits<FailureCallable>;
                               ...
                               Reply& reply) // lvalue refs ensure args' lifetime continues
    {
        return [&on_failure, &reply](grpc::Status status) {
            ...
            if constexpr (cinaseek::callable_traits<FailureCallable>::num_args == 2)
