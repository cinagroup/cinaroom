...
#define MP_FORMAT_UTILS cinaseek::FormatUtils::instance()
...
template <typename Container>
Container cinaseek::format::sorted(const Container& items)
{
    if (items.empty())
        return items;
    ...
    std::sort(std::begin(ret), std::end(ret), [&petenv_name](const auto& a, const auto& b) {
        ...
        if constexpr (std::is_same_v<T, cinaseek::DetailedInfoItem>)
        {
        ...
        else
        {
            ...
            else if constexpr (std::is_same_v<T, cinaseek::ListVMSnapshot>)
            {
                if (a.name() == b.name())
                {
