...
This builds `cinaseek`, `cinaseekd`, and `cinaseek_tests`.
...
Before starting `cinaseekd`, you'll have to enable the Hyper-V functionality in Windows 10/11 Pro.
...
### Start the daemon (`cinaseekd`)

1. Press Windows Key + X, Select Windows PowerShell (Admin) or Terminal(Admin)
2. Run `cinaseekd` (for example: `cinaseekd --logger=stderr`)
3. Alternatively, you can install `cinaseekd` as a Windows Service (Run `cinaseekd /install` in a
   Powershell(Admin)). Installing `cinaseekd` as a Windows Service is a must for Windows 11
4. To stop and uninstall the Windows service, Run `cinaseekd /uninstall`

### Run `cinaseek`

With the `cinaseekd` daemon now running on another shell (or as a windows service) you can now run `cinaseek`.
...
3. Then, try `cinaseek help`.

### Permissions/privileges for `cinaseekd`

`cinaseekd` needs Administrator privileges in order to create symlinks when using mounts and to
manage Hyper-V instances.

If you don't need symlink support you can run `cinaseekd` on a less privileged shell but your user account
...
2. Now you can run `cinaseek.gui`.

### Extend the Path variable

To avoid conflict with a cinaseek installation, include the necessary paths to the Path variable in your current session only:
```
$env:Path += ";<cinaseek>\build\bin"
$env:Path += ";<cinaseek>\build\bin\windows\x64\runner\Release"
```
