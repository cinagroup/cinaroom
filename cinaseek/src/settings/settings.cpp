...
{
    assert(handler &&
           "can't have null settings handler"); // TODO use a `not_null` type (e.g. gsl::not_null)
...
std::set<QString> cinaseek::Settings::keys() const
{
    std::set<QString> ret{};
    if (!handlers.empty())
    {
        ...
        {
            assert(*it && "can't have null settings handler"); // TODO use a `not_null` type (e.g.
                                                               // gsl::not_null)
            ret.merge((*it)->keys());
        }
    }
