...
#include <cinaseek/ssh/throw_on_error.h>
...
cinaseek::SSHSession::SSHSession(cinaseek::SSHSession&& other)
    : SSHSession(std::move(other), std::unique_lock{other.mut})
{
}

cinaseek::SSHSession::SSHSession(cinaseek::SSHSession&& other, std::unique_lock<std::mutex>)
    : session{std::move(other.session)}, mut{}
{
}

cinaseek::SSHSession& cinaseek::SSHSession::operator=(cinaseek::SSHSession&& other)
{
    if (this != &other)
    {
...
cinaseek::SSHSession::~SSHSession()
{
...
bool cinaseek::SSHSession::is_connected() const
{
    std::unique_lock lock{mut};
