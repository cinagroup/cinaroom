...
auto get_sshfs_exec_and_options(mp::SSHSession& session)
{
    ...
    try
    {
        ...
        auto sshfs_env{MP_UTILS.run_in_ssh_session(session, "snap run cinaseek-sshfs.env")};
    ...
    {
        mpl::debug(category, "'cinaseek-sshfs' snap package is not installed: {}", e.what());
    ...
    {
        using namespace cinaseek::literals;
        ...
        else if (cinaseek::opaque_semver{fuse_version_str} < "3.0.0"_semver)
        {
