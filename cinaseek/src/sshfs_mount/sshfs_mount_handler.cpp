...
bool has_sshfs(const std::string& name, mp::SSHSession& session)
{
    ...
    // Check if cinaseek-sshfs is already installed
    if (session.exec("sudo snap list cinaseek-sshfs").exit_code(std::chrono::seconds(15)) == 0)
    {
        mpl::debug(category, "The cinaseek-sshfs snap is already installed on '{}'", name);
        return true;
    }
...
try
{
    mpl::info(category, "Installing the cinaseek-sshfs snap in '{}'", name);
    auto proc = session.exec("sudo snap install cinaseek-sshfs");
    if (proc.exit_code(timeout) != 0)
    {
        auto error_msg = proc.read_std_error();
        mpl::error(category, "Failed to install 'cinaseek-sshfs': {}", mpu::trim_end(error_msg));
        throw mp::SSHFSMissingError();
    }
}
catch (const mp::ExitlessSSHProcessException& e)
{
    mpl::error(category, "Could not install 'cinaseek-sshfs' in '{}': {}", name, e.what());
    throw mp::SSHFSMissingError();
}
} // namespace
