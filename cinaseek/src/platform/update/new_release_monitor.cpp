...
signals:
    void latest_release_found(const cinaseek::NewReleaseInfo& release);
...
{
    qRegisterMetaType<cinaseek::NewReleaseInfo>(); // necessary to allow custom type be passed in
                                                    // signal/slot
...
void mp::NewReleaseMonitor::latest_release_found(const NewReleaseInfo& latest_release)
{
    try
    {
        const cinaseek::opaque_semver current{current_version};
        const cinaseek::opaque_semver latest{latest_release.version};
