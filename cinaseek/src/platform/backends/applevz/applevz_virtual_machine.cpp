...
AppleVZVirtualMachine::~AppleVZVirtualMachine()
{
    ...
    if (vm_handle)
    {
        cinaseek::top_catch_all(vm_name, [this]() {
...
void AppleVZVirtualMachine::shutdown(ShutdownPolicy shutdown_policy)
{
    ...
    cinaseek::utils::try_action_for(on_timeout, std::chrono::seconds{180}, [this]() {
        ...
        case VirtualMachine::State::off:
            drop_ssh_session();
            vm_handle.reset();
            return cinaseek::utils::TimeoutAction::done;
        default:
            return cinaseek::utils::TimeoutAction::retry;
        }
    });
}
...
{
    if (!management_ip)
        management_ip = cinaseek::backend::get_neighbour_ip(desc.default_mac_address);
...
void AppleVZVirtualMachine::resize_disk(const MemorySize& new_size)
{
    ...
    cinaseek::backend::resize_instance_image(new_size, desc.image.image_path);
...
void AppleVZVirtualMachine::fetch_ip(std::chrono::milliseconds timeout)
{
    ...
    auto action = [this] {
        ...
        return ((management_ip = cinaseek::backend::get_neighbour_ip(desc.default_mac_address)))
                   ? cinaseek::utils::TimeoutAction::done
                   : cinaseek::utils::TimeoutAction::retry;
    };
    ...
    cinaseek::utils::try_action_for(on_timeout, timeout, action);
}
