...
#include <cinaseek/snapshot_description.h>
#include <cinaseek/vm_mount.h>
...
inline std::string cinaseek::BaseSnapshot::get_name() const
{
    const std::unique_lock lock{mutex};
    return desc.name;
}

inline std::string cinaseek::BaseSnapshot::get_comment() const
{
    const std::unique_lock lock{mutex};
    return desc.comment;
}

inline std::string cinaseek::BaseSnapshot::get_cloud_init_instance_id() const noexcept
{
    return desc.cloud_init_instance_id;
}

inline int cinaseek::BaseSnapshot::get_index() const noexcept
{
    return desc.index;
}

inline QDateTime cinaseek::BaseSnapshot::get_creation_timestamp() const noexcept
{
    return desc.creation_timestamp;
}

inline std::string cinaseek::BaseSnapshot::get_parents_name() const
{
...
inline int cinaseek::BaseSnapshot::get_parents_index() const
{
    const std::unique_lock lock{mutex};
    return parent ? parent->get_index() : 0; // this doesn't lock
}

inline auto cinaseek::BaseSnapshot::get_parent() const -> std::shared_ptr<const Snapshot>
{
    const std::unique_lock lock{mutex};
    return parent;
}

inline auto cinaseek::BaseSnapshot::get_parent() -> std::shared_ptr<Snapshot>
{
    return std::const_pointer_cast<Snapshot>(std::as_const(*this).get_parent());
}

inline int cinaseek::BaseSnapshot::get_num_cores() const noexcept
{
    return desc.num_cores;
}

inline auto cinaseek::BaseSnapshot::get_mem_size() const noexcept -> MemorySize
{
    return desc.mem_size;
}

inline auto cinaseek::BaseSnapshot::get_disk_space() const noexcept -> MemorySize
{
    return desc.disk_space;
}

inline auto cinaseek::BaseSnapshot::get_extra_interfaces() const noexcept
    -> std::vector<NetworkInterface>
{
    return desc.extra_interfaces;
}

inline auto cinaseek::BaseSnapshot::get_state() const noexcept -> VirtualMachine::State
{
    return desc.state;
}

inline auto cinaseek::BaseSnapshot::get_mounts() const noexcept
    -> const std::unordered_map<std::string, VMMount>&
{
    return desc.mounts;
}

inline const boost::json::object& cinaseek::BaseSnapshot::get_metadata() const noexcept
{
    return desc.metadata;
}

inline void cinaseek::BaseSnapshot::set_name(const std::string& n)
{
    const std::unique_lock lock{mutex};
...
inline void cinaseek::BaseSnapshot::set_comment(const std::string& c)
{
    const std::unique_lock lock{mutex};
    assert(captured && "precondition: only captured snapshots can be edited");
...
inline void cinaseek::BaseSnapshot::set_parent(std::shared_ptr<Snapshot> p)
{
    const std::unique_lock lock{mutex};
    assert(captured && "precondition: only captured snapshots can be edited");
...
inline void cinaseek::BaseSnapshot::capture()
{
    const std::unique_lock lock{mutex};
...
inline void cinaseek::BaseSnapshot::apply()
{
    const std::unique_lock lock{mutex};
    apply_impl();
...
inline const QString& cinaseek::BaseSnapshot::get_id() const noexcept
{
    return id;
}
