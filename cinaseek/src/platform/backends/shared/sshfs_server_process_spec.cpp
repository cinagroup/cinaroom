...
profile %1 flags=(attach_disconnected) {
    ...
    # Allow cinaseekd send sshfs_server signals
    ...
    QString root_dir; // sshfs_server is a cinaseek utility, is located relative to the cinaseekd
                      // binary if not in a snap. If snapped, is located relative to $SNAP
    QString signal_peer; // if snap confined, specify only cinaseekd can kill dnsmasq

    try
    {
        root_dir = mpu::snap_dir();
        signal_peer = "snap.cinaseek.cinaseekd";
    }
