...
int get_net_type(const QDir& net_dir) // types defined in if_arp.h
{
    ...
    auto snap_hint =
        mpu::in_cinaseek_snap() ? " Is the 'network-observe' snap interface connected?" : "";
...
std::unique_ptr<QFile> cinaseek::platform::detail::find_os_release()
{
...
std::pair<QString, QString> cinaseek::platform::detail::parse_os_release(
...
std::string cinaseek::platform::detail::read_os_release()
{
                                                 ...
                                                 const mp::AliasDefinition& def) const
{
    ...
    std::string cinaseek_exec =
        mpu::in_cinaseek_snap()
            ? "exec /usr/bin/snap run cinaseek"
...
std::string mp::platform::default_server_address()
{
    ...
    return "unix:" + base_dir + "/cinaseek_socket";
}
...
std::string cinaseek::platform::host_version()
{
    return mpu::in_cinaseek_snap()
               ? cinaseek::platform::detail::read_os_release()
...
std::filesystem::path mp::platform::Platform::get_root_cert_dir() const
{
    ...
    const auto base_dir = utils::in_cinaseek_snap()
