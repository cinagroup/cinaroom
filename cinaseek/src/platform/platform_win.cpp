...
#include <cinaseek/settings/custom_setting_spec.h>
#include <cinaseek/settings/settings.h>
#include <cinaseek/standard_paths.h>
...
Json::Value create_primary_profile()
{
    ...
    primary_profile["commandline"] = "cinaseek shell";
    ...
    primary_profile["icon"] =
        QDir{QCoreApplication::applicationDirPath()}.filePath("cinaseek_wt.ico").toStdString();
...
QString program_data_cinaseek_path()
{
...
QString mp::platform::Platform::daemon_config_home() const // temporary
{
    ...
    if (QFile::exists(ret))
    {
        return ret; // should be something like
                    // "C:/Windows/system32/config/systemprofile/AppData/Local/cinaseekd"
    }
    else
    {
        return MP_PLATFORM.cinaseek_storage_location();
    }
}
                                                 ...
                                                 const mp::AliasDefinition& def) const
{
    ...
    std::string cinaseek_exec = QCoreApplication::applicationFilePath().toStdString();

    std::string script = "@\"" + cinaseek_exec + "\" " + alias + " -- %*\n";
...
std::string mp::platform::Platform::alias_path_message() const
{
    return fmt::format(
        ...
        "without prefixing with `cinaseek`. For now, you can just do:\n\n"
        "In PowerShell:\n$ENV:PATH=\"$ENV:PATH;{0}\"\n\n"
        "Or in Command Prompt:\nPATH=%PATH%;{0}\n",
        get_alias_scripts_folder().absolutePath());
}

QString mp::platform::Platform::cinaseek_storage_location() const
{
    auto storage_location = mp::utils::get_cinaseek_storage();

    // If CINASEEK_STORAGE env var is set, use that
    if (!storage_location.isEmpty())
    {
        return storage_location;
    }

    auto program_data_path = program_data_cinaseek_path();
    ...
    // %SYSTEMROOT%\system32\config\AppData\Roaming\cinaseekd doesn't exist, use
    // %PROGRAMDATA%\Multipass
    if (QFile::exists(program_data_path) ||
        !QFile::exists(QDir{systemprofile_roaming_path}.absoluteFilePath("cinaseekd")))
    {
        return program_data_path;
    }

    // If %SYSTEMROOT%\system32\config\AppData\Roaming\cinaseekd exists, return empty and let the
...
std::filesystem::path mp::platform::Platform::get_root_cert_dir() const
{
    ...
    // Windows doesn't use `daemon_name` for the data directory (see `program_data_cinaseek_path`)
