...
#include <cinaseek/platform_unix.h>
                                                            ...
                                                            const bool restricted) const
{
    ...
    if (chown(socket_path.c_str(), 0, gid) == -1)
        throw std::runtime_error(
            fmt::format("Could not set ownership of the cinaseek socket: {}", strerror(errno)));

    if (!set_permissions(socket_path, mode))
        throw std::runtime_error(fmt::format("Could not set permissions for the cinaseek socket"));
}

QString mp::platform::Platform::cinaseek_storage_location() const
{
    return mp::utils::get_cinaseek_storage();
}
