...
#include <cinaseek/alias_definition.h>
#include <cinaseek/cloud_init_iso.h>
#include <cinaseek/constants.h>
#include <cinaseek/exceptions/create_image_exception.h>
#include <cinaseek/exceptions/exitless_sshprocess_exceptions.h>
#include <cinaseek/exceptions/ghost_instance_exception.h>
#include <cinaseek/exceptions/image_vault_exceptions.h>
#include <cinaseek/exceptions/invalid_memory_size_exception.h>
#include <cinaseek/exceptions/not_implemented_on_this_backend_exception.h>
#include <cinaseek/exceptions/snapshot_exceptions.h>
#include <cinaseek/exceptions/sshfs_missing_error.h>
#include <cinaseek/exceptions/start_exception.h>
#include <cinaseek/exceptions/virtual_machine_state_exceptions.h>
#include <cinaseek/image_host/vm_image_host.h>
#include <cinaseek/ip_address.h>
#include <cinaseek/json_utils.h>
#include <cinaseek/logging/client_logger.h>
...
#include <cinaseek/query.h>
#include <cinaseek/settings/bool_setting_spec.h>
#include <cinaseek/settings/settings.h>
#include <cinaseek/snapshot.h>
#include <cinaseek/ssh/ssh_session.h>
#include <cinaseek/sshfs_mount/sshfs_mount_handler.h>
#include <cinaseek/top_catch_all.h>
#include <cinaseek/version.h>
...
#include <cinaseek/yaml_node_utils.h>
...
constexpr auto instance_db_name = "cinaseekd-vm-instances.json";
...
constexpr auto sshfs_error_template =
    ...
    "\n\nPlease install the 'cinaseek-sshfs' snap manually inside the instance.";
constexpr auto invalid_network_template =
    "Invalid network '{}' set as bridged interface, use `cinaseek set "
    "{}=<name>` to correct. See `cinaseek networks` for valid names.";
                                   ...
                                   const mp::CreateRequest* request)
{
    ...
    if (request->image() != "fedora")
    {
        ...
        auto pollinate_user_agent_string =
            fmt::format("cinaseek/version/{} # written by Multipass\n", cinaseek::version_string);
        pollinate_user_agent_string +=
            fmt::format("cinaseek/driver/{} # written by Multipass\n", backend_version_string);
        pollinate_user_agent_string += fmt::format("cinaseek/host/{} # written by Multipass\n",
                                                   cinaseek::platform::host_version());
        pollinate_user_agent_string += fmt::format("cinaseek/alias/{}{} # written by Multipass\n",
...
std::string get_bridged_interface_name()
{
    ...
    if (bridged_id == "")
    {
        throw std::runtime_error(
            fmt::format("You have to `cinaseek set {}=<name>` to use the \"bridged\" shortcut.",
    ...
    mp::LaunchError& option_errors)
{
    ...
    for (const auto& net : request->network_options())
    {
        ...
        if (dont_allow_auto && net.mode() == cinaseek::LaunchRequest_NetworkOptions_Mode_AUTO)
        {
        ...
        if (const auto& mac = QString::fromStdString(net.mac_address()).toLower().toStdString();
            ...
            interfaces.push_back(mp::NetworkInterface{
                net_id,
                mac,
                net.mode() != cinaseek::LaunchRequest_NetworkOptions_Mode_MANUAL});
        else
        {
    ...
    for (auto& entry : vm_instance_specs)
    {
        ...
        if (spec.state == e_state::running)
        {
            ...
            case e_state::starting:
            {
                ...
                cinaseek::top_catch_all(name, [this, &name, &lock] {
                    lock.unlock();
                    on_restart(name);
                });
            }
...
try
{
    ...
    if (!MP_SETTINGS.get_as<bool>(mp::mounts_key))
        return status_promise->set_value(grpc::Status(
            ...
            "See https://canonical.com/cinaseek/docs/set-command#local.privileged-mounts for "
            "information\n"
                         ...
                         std::promise<grpc::Status>* status_promise)
{
    ...
    VersionReply reply;
    reply.set_version(cinaseek::version_string);
...
try
{
    ...
    if (stored_hash.isNull() || stored_hash.isEmpty())
    {
        return status_promise->set_value(grpc::Status(
            ...
            "'cinaseek set local.passphrase'. Note that only the user who installs Multipass is "
    ...
    if (vm.state == VirtualMachine::State::delayed_shutdown &&
        ...
        return grpc::Status{
            ...
            fmt::format("\"{}\" is scheduled to shut down in less than a minute, use "
                        "'cinaseek stop --cancel {}' to cancel the shutdown.",
                        name,
                        name),
            ""};
