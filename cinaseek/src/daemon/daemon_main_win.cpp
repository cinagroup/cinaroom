...
void create_client_cert_if_necessary()
{
    ...
    const QString cinaseekd_data_dir_path{
        storage_path.isEmpty()
            ? QString("%1\\config\\systemprofile\\AppData\\Roaming\\cinaseekd\\").arg(infoBuf)
            : QString("%1\\data").arg(storage_path)};

    mp::ClientCertStore cert_store{cinaseekd_data_dir_path};
