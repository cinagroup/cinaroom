...
#include <cinaseek/exceptions/download_exception.h>
#include <cinaseek/exceptions/image_not_found_exception.h>
#include <cinaseek/exceptions/manifest_exceptions.h>
#include <cinaseek/exceptions/unsupported_image_exception.h>
#include <cinaseek/image_host/ubuntu_image_host.h>
#include <cinaseek/platform.h>
#include <cinaseek/query.h>
#include <cinaseek/settings/settings.h>
#include <cinaseek/simple_streams_index.h>
...
mp::VMImageInfo mp::UbuntuVMImageHost::info_for_full_hash_impl(const std::string& full_hash)
{
    for (const auto& manifest : manifests)
    {
        ...
        {
            if (cinaseek::utils::iequals(product.id.toStdString(), full_hash))
            {
                return product;
            }
        }
...
cinaseek::UbuntuVMImageRemote::UbuntuVMImageRemote(
...
bool cinaseek::UbuntuVMImageRemote::default_image_mutator(VMImageInfo& image)
{
    return true;
}
