...
#include <cinaseek/cli/prompters.h>
#include <cinaseek/constants.h>
#include <cinaseek/exceptions/cmd_exceptions.h>
...
{
    cinaseek::LaunchRequest_NetworkOptions net;
...
mp::ParseCode cmd::Launch::parse_args(mp::ArgParser* parser)
{
    ...
    QCommandLineOption networkOption(
        ...
        "`cinaseek set local.bridged-network`.\n"
...
{
    if (!spinner)
        spinner = std::make_unique<cinaseek::AnimatedSpinner>(
            cout); // Creating just in time to work around canonical/cinaseek#2075
        ...
        for (const auto& error : launch_error.error_codes())
        {
            ...
            else if (error == LaunchError::INVALID_NETWORK)
            {
                ...
                error_details =
                    ...
                    "To troubleshoot, see "
                    "https://documentation.ubuntu.com/cinaseek/stable/how-to-guides/troubleshoot/";
            }
        }
                        ...
                        const QString& mount_target) -> ReturnCodeVariant
{
    ...
    auto ret = run_cmd({"cinaseek", "mount", mount_source, full_mount_target}, parser, cout, cerr);
    if (ret == ReturnCode::Ok)
...
bool cmd::Launch::ask_bridge_permission(cinaseek::LaunchReply& reply)
{
