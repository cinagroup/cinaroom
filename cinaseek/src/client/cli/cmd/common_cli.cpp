...
#include <cinaseek/cli/format_utils.h>
...
QString cinaseek::cmd::describe_common_settings_keys()
{
...
namespace
{
void add_timeout_option(cinaseek::ArgParser* parser, const QString& extra_description = {})
{
...
void cinaseek::cmd::add_instance_timeout(cinaseek::ArgParser* parser)
{
    const auto instance_timeout_str =
        QString(" By default, instance startup and initialization is limited to "
                "%1 minutes each.")
            .arg(std::chrono::duration_cast<std::chrono::minutes>(cinaseek::default_timeout)
                     .count());

    add_timeout_option(parser, instance_timeout_str);
}

void cinaseek::cmd::add_timeout(cinaseek::ArgParser* parser)
{
    add_timeout_option(parser);
}

int cinaseek::cmd::parse_timeout(const cinaseek::ArgParser* parser)
{
...
std::unique_ptr<cinaseek::utils::Timer> cinaseek::cmd::make_timer(int timeout,
                                                                    AnimatedSpinner* spinner,
                                                                    std::ostream& cerr,
                                                                    const std::string& msg)
{
    auto timer = std::make_unique<cinaseek::utils::Timer>(std::chrono::seconds(timeout),
                                                           [spinner, &cerr, msg]() {
                                                               if (spinner)
                                                                   spinner->stop();
                                                               cerr << msg << std::endl;
