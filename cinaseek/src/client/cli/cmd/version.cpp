...
#include <cinaseek/cli/formatter.h>
...
mp::ReturnCodeVariant cmd::Version::run(mp::ArgParser* parser)
{
    ...
    auto on_success = [this](mp::VersionReply& reply) -> ReturnCodeVariant {
        cout << chosen_formatter->format(reply, cinaseek::version_string);
...
{
    return QStringLiteral("Display version information about the cinaseek command\n"
                          "and daemon.");
}
