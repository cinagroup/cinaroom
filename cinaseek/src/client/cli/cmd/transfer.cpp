...
#include <cinaseek/cli/client_platform.h>
#include <cinaseek/file_ops.h>
#include <cinaseek/ssh/sftp_utils.h>
...
mp::ParseCode cmd::Transfer::parse_args(mp::ArgParser* parser)
{
    ...
    if (const auto ret_opt =
            parse_streaming(full_sources, full_target, split_sources, split_target);
        ret_opt)
...
std::vector<std::pair<std::string, fs::path>> cmd::Transfer::args_to_instance_and_path(
    const QStringList& args)
{
    ...
    {
        // this is needed so that Windows absolute paths are not split at the colon following the
        // drive letter
        ...
        const auto instance_name =
            source_components.size() == 1 ? "" : source_components.first().toStdString();
        const auto file_path =
            source_components.size() == 1 ? source_components.first() : entry.section(':', 1);
        if (!instance_name.empty())
            request.add_instance_name()->append(instance_name);
        instance_entry_args.emplace_back(instance_name,
                                         file_path.isEmpty() ? "." : file_path.toStdString());
    }
    return instance_entry_args;
}

std::optional<mp::ParseCode> cinaseek::cmd::Transfer::parse_streaming(
...
{
    const auto source_streaming =
        std::count(full_sources.begin(), full_sources.end(), streaming_symbol);
...
mp::ParseCode cmd::Transfer::parse_non_streaming(
    std::vector<std::pair<std::string, fs::path>>& split_sources,
    std::pair<std::string, fs::path>& split_target)
{
    if (split_target.first.empty())
    ...
    std::transform(split_sources.begin(),
                   split_sources.end(),
