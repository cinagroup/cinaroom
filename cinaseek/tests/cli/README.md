...
cinaseek CLI tests test the user-facing CLI functionality by using `cinaseek` like a user would do.

The tests are divided into separate suites based on the functionality/command.

## Installing the tests

The tests are located in the cinaseek repository, and do not require anything from the repository except for itself, i.e. the `tests/cli` folder.
...
One of the python dependencies in Windows is a Rust library. Therefore, the Rust toolchain needs to be present to satisfy the Python deps in Windows:
...
Controlling the cinaseek service and running the cinaseek daemon needs superuser privileges. Hence, `sudo` (or `gsudo` in Windows) needs to be available in the environment. The test harness will prompt for an initial `sudo` escalation and will keep the `sudo` ticket alive for the duration of the tests. The tests themselves will run as non-privileged but some parts, like spawning the daemon process, or removing a `root` owned directory will escalate privileges as needed.

## To run

The tests support running on installed versions of cinaseek on Linux (snap), macOS (package), Windows (MSI package). Additionally, the tests also support running on a source-built version of cinaseek, which is referred to as `standalone` mode.

The test suite has dedicated platform-specific code for interacting with the installed cinaseek. These are referred as `daemon controller`s in the cinaseek CLI test terminology. There are four daemon controllers at the moment, namely:

- `snap`: Interacts with the snap-installed cinaseek for test governance
- `winsvc`: Windows Service based (Windows MSI Package)
- `launchd`: Launchd-based installations (macOS package)
- `standalone`: Source-built version of the cinaseek, either installed to an arbitrary location, or in a build folder
...
```bash
pytest tests/cli # or pass `--daemon-controller=snap` explicitly
```

### macOS, launchd

```bash
pytest tests/cli # or pass `--daemon-controller=launchd` explicitly
```

### Windows, Windows Service

```bash
pytest tests/cli # or pass `--daemon-controller=winsvc` explicitly
```

### Standalone (all platforms)

Standalone mode needs the additional `--bin-dir` option as it needs to know where to look for the `cinaseekd` and `cinaseek` binaries. The path must contain both.
...
Prints the output from the `cinaseek` command as the tests being executed.

#### `--print-daemon-output`

Prints the output from the `cinaseekd`, the cinaseek daemon.
...
```bash
pytest tests/cli/ --log-level=DEBUG # INFO, TRACE, WARN..

# DEBUG    root:conftest.py:329 store_config :: final config namespace(vm=namespace(cpus='2', memory='1G', disk='6G', image='noble'), retries=namespace(launch=3), timeouts=namespace(launch=600, stop=180, mount=180, restart=120, delete=90, exec=90, start=90, umount=45), bin_dir='/snap/bin', storage_dir='/var/snap/cinaseek/common/data/cinaseekd', print_daemon_output=False, print_cli_output=False, remove_all_instances=False, driver='qemu', daemon_controller='snap', data_dir='/var/snap/cinaseek/common/data/cinaseekd')
# DEBUG    root:conftest.py:418 environment_setup :: setup
# DEBUG    asyncio:selector_events.py:64 Using selector: EpollSelector
# DEBUG    root:cinaseekd_governor.py:219 cinaseekd-governor :: stop called
# DEBUG    root:cinaseekd_governor.py:163 cinaseekd-governor :: start called
# DEBUG    root:privutils.py:79 depth: 2
# DEBUG    root:privutils.py:93 module.__file__ = /workspace/cinaseek/tests/cli/cinaseek/certutils.py
# DEBUG    root:privutils.py:94 inferred PYTHONPATH = /workspace/cinaseek/tools
# DEBUG    root:cinaseekd_governor.py:64 cinaseekd-governor :: read_stream start
# DEBUG    root:cinaseekd_governor.py:301 b'find failed: cannot connect to the cinaseek socket\n'
# DEBUG    root:cinaseekd_governor.py:124 cinaseekd-governor :: monitor task exited (cancelled: False)
# DEBUG    root:cinaseekd_governor.py:219 cinaseekd-governor :: stop called
...
```bash
pytest tests/cli/cli_tests.py --bin-dir build/bin/ --storage-dir=/tmp/cinaseek-test --remove-all-instances # would remove all instances in the `/tmp/cinaseek-test` before the test session.
```
...
The `timeout` is how long to wait for a certain command to complete. This is enforced at process level and independent from the `--timeout` value the `cinaseek <command>` has. The timeout values are set on per command basis and each command can have a different timeout value (e.g., `600s` for launch, `60s`for `stop`.) The timeout is applied to per command invocation, so if there are `retries` each retry would get the timeout independently.
...
```bash
pytest tests/cli/ --cmd-timeouts launch=300 start=180 alias=10
```
...
``` bash
pytest tests/cli/ --cmd-retries launch=5 exec=2
```
...
```yaml
  DispatchCLITestsWorkflow:
    ...
    with:
      macos-pkg-url: https://example.com/macos/cinaseek.pkg
      windows-pkg-url: https://example.com/win/cinaseek.msi
...
```sh
 ...
 gh workflow run -r feature/cli-tests "CLI Tests" -f windows-pkg-url=https://cinaseek-ci.s3.amazonaws.com/pr4061/cinaseek-1.17.0-dev.550.pr4061%2Bg0239bc1f.win-win64.msi macos-pkg-url=https://cinaseek-ci.s3.amazonaws.com/pr4061/cinaseek-1.17.0-dev.550.pr4061%2Bg0239bc1f.mac-Darwin.pkg pytest-extra-args="--print-all-output"
```
