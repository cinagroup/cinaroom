...
class TestVmResource:
    ...
    def test_modify_vm_while_running(self, instance):
        ...
        assert not cinaseek("set", f"local.{instance}.memory=2G")
        assert not cinaseek("set", f"local.{instance}.disk=10G")
        assert not cinaseek("set", f"local.{instance}.cpus=3")
    ...
    def test_modify_vm(self, instance):
        ...
        assert cinaseek("set", f"local.{instance}.memory=2G")
        assert cinaseek("set", f"local.{instance}.disk=10G")
        assert cinaseek("set", f"local.{instance}.cpus=3")
