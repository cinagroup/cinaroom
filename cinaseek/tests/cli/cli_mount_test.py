...
from cli.utilities import TempDirectory, retry
...
@pytest.mark.usefixtures("cinaseekd", "windows_privileged_mounts")
class TestMount:
    ...
    def test_mount(self, instance, mount_type):
        with TempDirectory() as mount_src, src_to_dst(mount_src) as mount_dst:
            ...
            assert cinaseek("mount", "--type", mount_type, str(mount_src), instance)
            ...
            assert cinaseek("umount", instance)
            assert mounts(instance) == {}
    ...
    def test_mount_multiple(self, instance, mount_type):
        with (
            ...
            assert cinaseek("mount", "--type", mount_type, str(mount_src1), instance)
            assert cinaseek("mount", "--type", mount_type, str(mount_src2), instance)
            assert cinaseek("start", instance)
    ...
    def test_mount_restart(self, cinaseekd, instance, mount_type, restart_type):
        with TempDirectory() as mount_src, src_to_dst(mount_src) as mount_dst:
            if mount_type == "native":
                assert cinaseek("stop", instance)
            ...
            if restart_type == "daemon":
                ...
                assert cinaseek("restart", instance)
            else:
            ...
            from cli.cinaseek import debug_interactive_shell
