...
from cli.cinaseek import cinaseek, path_exists, read_file
...
class TestTransfer:
    ...
    def test_transfer_single_file(self, instance):
        ...
        with TempDirectory() as tmp:
            ...
            # Push
            assert cinaseek("transfer", str(file), f"{instance}:.")
            assert path_exists(instance, "testfile.txt")

            # Pull
            pull_file = tmp / "testfile_pull.txt"
            assert not pull_file.exists()
            assert cinaseek("transfer", f"{instance}:testfile.txt", str(pull_file))
    ...
    def test_transfer_single_file_create_parents(self, instance):
        ...
        with TempDirectory() as tmp:
            ...
            # This fails.
            # assert cinaseek("transfer", "--parents", str(file), f"{instance}:bar/baz/")
            assert cinaseek(
    ...
    def test_transfer_directory_recursive(self, instance):
        ...
        with TempDirectory() as tmp:
            ...
            # Push
            assert not cinaseek("transfer", str(tmp), f"{instance}:dest")
            assert cinaseek("transfer", "--recursive", str(tmp), f"{instance}:dest")
            ...
            assert cinaseek("transfer", "--recursive", f"{instance}:dest", pull_folder)
