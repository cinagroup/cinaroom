...
from cli.cinaseek import cinaseek, state, launch, random_vm_name, get_boot_id
...
class TestVmLifecycle:
    ...
    def test_lifecycle_ops_on_nonexistent(self):
        ...
        for op in ops:
            with cinaseek(op, f"{name}", disable_hooks=True) as output:
                assert not output
                assert "does not exist" in output

    def test_stop(self, instance):
        assert state(f"{instance}") == "Running"
        assert cinaseek("stop", f"{instance}")
    ...
    def test_stop_start(self, instance):
        ...
        assert cinaseek("stop", f"{instance}")
        assert state(f"{instance}") == "Stopped"
        assert cinaseek("start", f"{instance}")
        assert state(f"{instance}") == "Running"
    ...
    def test_suspend_resume(self, instance):
        assert state(f"{instance}") == "Running"
        boot_id_before = get_boot_id(instance)
        assert cinaseek("suspend", f"{instance}")
    ...
    def test_launch_delete_recover_purge(self, instance):
        assert state(f"{instance}") == "Running"
        assert cinaseek("delete", f"{instance}")
        assert state(f"{instance}") == "Deleted"

        with cinaseek("start", f"{instance}") as output:
            assert not output
            assert f"Instance '{instance}' is deleted." in output

        assert cinaseek("recover", f"{instance}")
        assert state(f"{instance}") == "Stopped"
        ...
        assert cinaseek("delete", f"{instance}", "--purge")
        with cinaseek("info", f"{instance}") as output:
            assert not output
            assert "does not exist" in output
    ...
    def test_delete_purge(self):
        with (
            ...
            assert cinaseek("delete", f"{name1}")
            assert state(f"{name1}") == "Deleted"

            assert cinaseek("delete", f"{name2}")
            assert state(f"{name2}") == "Deleted"

            assert cinaseek("purge")
    ...
    def test_lifecycle_multiple(self):
        with (
            ...
            assert cinaseek("stop", name1, name2)
            assert state(name1) == "Stopped" and state(name2) == "Stopped"

            assert cinaseek("start", name1, name2)
            assert state(name1) == "Running" and state(name2) == "Running"

            assert cinaseek("suspend", name1, name2)
            ...
            assert cinaseek("restart", name1, name2)
            assert state(name1) == "Running" and state(name2) == "Running"
            ...
            assert cinaseek("delete", name1, name2)
            assert state(name1) == "Deleted" and state(name2) == "Deleted"

            assert cinaseek("recover", name1, name2)
            assert state(name1) == "Stopped" and state(name2) == "Stopped"
            ...
            with cinaseek("info", name1, name2) as output:
                assert not output
