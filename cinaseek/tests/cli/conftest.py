...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--bin-dir",
        action="store",
        help="Directory to look for the cinaseek binaries.",
    )

    parser.addoption(
        "--storage-dir",
        action="store",
        help="Storage directory for cinaseek.",
    )
...
def pytest_runtest_setup(item):
    ...
    cinaseek_version = get_cinaseek_version()
    ...
    current = version.parse(cinaseek_version)
...
def ensure_cinaseek_binaries_are_present():
    if not get_cinaseek_path():
        pytest.exit(
            "ERROR: Could not locate the `cinaseek` binary!",
            returncode=1,
        )

    if cfg.daemon_controller == "standalone" and not get_cinaseekd_path():
        pytest.exit(
            "ERROR: Could not locate the `cinaseekd` binary!",
            returncode=1,
        )
...
def set_driver(controller):
    if cinaseek("get", "local.driver") != cfg.driver:
        assert cinaseek("set", f"local.driver={cfg.driver}")
...
def cinaseekd_class_scoped(request):
    with cinaseekd_impl() as daemon:
        request.cls.cinaseekd = daemon
        yield daemon


@pytest.fixture
def windows_privileged_mounts(cinaseekd):
    if sys.platform == "win32":
        # Check if privileged mounts are already enabled
        if "true" in cinaseek("get", "local.privileged-mounts"):
            return
        assert cinaseek("set", "local.privileged-mounts=1")
