...
"""
Windows Service-based controller for cinaseekd
...
class WindowsServiceMultipassdController:
    """SCM-backed controller for cinaseekd on Windows."""

    def __init__(self, service_name: Optional[str] = "cinaseek"):
    ...
    async def follow_output(self) -> AsyncIterator[str]:
        ...
        - Uses `wevtutil qe` with XPath filter for provider name containing
          "cinaseek". Because there is no simple built-in "follow" mode,
