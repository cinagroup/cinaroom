...
from .cinaseekd_controller import MultipassdController


class MultipassdGovernor:
    """
    Orchestrates the lifecycle of the **cinaseekd** daemon during tests.
    """
    ...
    async def _read_stream(self):
        ...
        logging.debug("cinaseekd-governor :: read_stream start")
        try:
        ...
        logging.debug("cinaseekd-governor :: read_stream finish")
    ...
    async def _monitor(self):
        ...
        try:
            ...
            if returncode is None:
                ...
                logging.warning(
                    "cinaseekd-governor :: monitor task -- unable to determine daemon exit code")
                return
            ...
            if returncode == 1:
                raise TestSessionFailure(
                    f"FATAL: cinaseekd died with code {returncode}!{reasons}"
                )
    ...
    async def on_monitor_exit(self, task):
        logging.debug(
            f"cinaseekd-governor :: monitor task exited (cancelled: {task.cancelled()})"
        )
        if task.cancelled():
            return

        cinaseekd_settings_changed_code = 42
        daemon_exit_code = await self.controller.exit_code()
        needs_restarting = (
            not self.controller.supports_self_autorestart()
            and daemon_exit_code == cinaseekd_settings_changed_code
        )
        self._reset_state()
        if needs_restarting:
            logging.debug("cinaseekd-governor :: daemon auto-restarting")
            self.asyncio_loop.run_fn(
                lambda: asyncio.create_task(self.start_async()))

    async def _ensure_client_certs_are_created(self):
        # Call cinaseek cli to create the client certs
        async with SilentAsyncSubprocess(
            get_cinaseek_path(),
            "version",
            env=get_cinaseek_env(),
        ) as version_proc:
    ...
    async def start_async(self):
        """Start the cinaseekd daemon"""
        logging.debug("cinaseekd-governor :: start called")
        ...
        ready_task = asyncio.create_task(
            self.wait_for_cinaseekd_ready(), name="wait_for_ready_task"
        )
        ...
        if not await ready_task:
            logging.warning(
                "⚠️ cinaseekd not ready, attempting graceful shutdown...")
            await self.controller.stop()
            pytest.exit(
                "Tests cannot proceed, cinaseekd not responding in time.",
                returncode=12,
            )
    ...
    async def stop_async(self):
        """Stop the cinaseekd daemon"""
        logging.debug("cinaseekd-governor :: stop called")
    ...
    def wait_for_restart(self, timeout=60):
        ...
        if self.controller.supports_self_autorestart():
            ...
            logging.debug(
                "cinaseekd-governor :: daemon auto-restart detected")
            # Wait until daemon is ready
            self.asyncio_loop.run(self.wait_for_cinaseekd_ready()).result()
            return
        self.wait_for_shutdown(timeout=timeout)
        self.wait_for_start(timeout=timeout)

    @staticmethod
    async def wait_for_cinaseekd_ready(timeout=60):
        """
