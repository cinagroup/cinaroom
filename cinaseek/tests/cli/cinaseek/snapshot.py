...
def take_snapshot(vm_name, snapshot_name, expected_parent="", expected_comment=""):
    ...
    """
    assert cinaseek("exec", f"{vm_name}", "--", "touch", f"{snapshot_name}")
    assert cinaseek("exec", f"{vm_name}", "--", "ls", f"{snapshot_name}")
    assert cinaseek("stop", f"{vm_name}")

    with cinaseek("snapshot", f"{vm_name}", "--name", f"{snapshot_name}") as output:
        assert output.exitstatus == 0
        assert "Snapshot taken" in output

    with cinaseek("list", "--format=json", "--snapshots").json() as output:
        assert output.exitstatus == 0
...
def build_snapshot_tree(vm_name, tree, parent=""):
    ...
    for name, children in tree.items():
        ...
        if parent != "":
            assert cinaseek("restore", f"{vm_name}.{parent}", "--destructive")
