...
from .helpers import get_cinaseek_version
...
def skip_if_feature_not_supported(feature_name, version=None):
    if not cinaseek_version_has_feature(feature_name, version=version):
        pytest.skip(
            f"The version does not support `{feature_name}`, skipping.")


def requires_cinaseek(spec: str):
    """
    Decorator to skip tests unless Multipass version matches the specifier.

    Usage:
        @requires_cinaseek(">=1.14")
        def test_snapshot_feature():
            ...

        @requires_cinaseek(">=1.12,<1.15")
        def test_deprecated_feature():
            ...
    """

    return pytest.mark.skipif(
        get_cinaseek_version() not in SpecifierSet(spec),
