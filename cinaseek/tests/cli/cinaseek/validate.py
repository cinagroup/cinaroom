...
def validate_output(*args, properties, jq_filter=None):
    with cinaseek(*args).json() as output:
...
def validate_list_output(name, properties):
    """Validate properties of a specific VM from `cinaseek list`.

    Fetches all VM data via `cinaseek list --format=json`, locates the
...
def validate_info_snapshot_output(name, properties):
    """Validate properties of a specific VM from `cinaseek info --snapshots`.

    Fetches VM data via `cinaseek info --format=json --snapshots <name>`, locates the
...
def validate_info_output(name, properties):
    """
    Validate 'cinaseek info' JSON output for a VM.
    """
