...
"""Test helpers for running `cinaseek` commands and querying instance info."""
...
from .cinaseek_cmd import cinaseek


def _retrieve_info_field(name, key):
    """Run `cinaseek info` for the given instance and return the specified
...
def debug_interactive_shell(name):
    ...
    with cinaseek("shell", name, interactive=True) as sh:
        sh.interact()


def info(name):
    """
    Get 'cinaseek info' JSON for instance.
    """
    with cinaseek("info", "--format=json", f"{name}").json() as output:
...
def exec(name, *args, **kwargs):
    ...
    return cinaseek("exec", name, "--", *args, **kwargs)
...
def shell(name):
    ...
    if sys.platform == "win32":
        ...
        # On Unix hosts, 'cinaseek shell' is fine.
        return cinaseek("shell", name, interactive=True)


def vm_exists(name):
    with cinaseek("list", "--format=json").json() as list_json:
...
def get_ram_size(name):
    ...
    with cinaseek("exec", name, "--", "cat", "/proc/meminfo") as result:
...
def get_disk_size(name):
    ...
    with cinaseek("exec", name, "--", "df", "-m", "--output=size", "/") as result:
        assert result, f"Failed: {result.content} ({result.exitstatus})"
...
def get_core_count(name):
    ...
    with cinaseek("exec", name, "--", "nproc") as result:
        assert result, f"Failed: {result.content} ({result.exitstatus})"
...
def get_default_interface_name(name):
    ...
    """
    with cinaseek("exec", name, "--", "ip", "-o", "route", "get", "1") as result:
        assert result, f"Failed: {result.content} ({result.exitstatus})"
...
def get_cinaseek_version():
    with cinaseek("version") as version_output:
        match = re.search(r"cinaseek\s*(\d+\.\d+\.\d+)", version_output.content,re.MULTILINE)
