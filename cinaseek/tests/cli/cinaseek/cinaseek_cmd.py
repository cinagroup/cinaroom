...
from cli.cinaseek import get_cinaseek_env, get_cinaseek_path
from cli.cinaseek.cmd_output import Output
from cli.utilities import retry, wrap_call_if
from .cinaseek_spawn import spawn_cinaseek
...
def restart_epilogue(args, kwargs, result, prologue_result):
    ...
    for vm_name in filtered_args:
        ...
        def attempt_ssh(vm_name):
            with cinaseek("exec", vm_name, "--", "echo", "ok") as v:
...
def cinaseek(*args, **kwargs):
    ...
    Args:
        *args: Positional CLI arguments to pass to the `cinaseek` command.
    ...
    Example:
        >>> out = cinaseek("list", "--format", "json").jq(".instances")
        >>> with cinaseek("info", "test") as output:
        ...     print(output.exitstatus)

    Notes:
        - You can access output fields directly: `cinaseek("version").exitstatus`
    ...
    if retry_count and retry_count > 0:
        ...
        def retry_wrapper():
            ...
            return cinaseek(*args, **{**kwargs, "retry": 0})
    ...
    cmd_args = [get_cinaseek_path(), *map(str, args)]
    logging.debug(f"cmd: {cmd_args}")

    if kwargs.get("interactive"):
        return spawn_cinaseek(args, timeout=timeout, echo=echo, env=env, interactive=True)
    ...
    class RunToCompletion:
        ...
        def __init__(self):
            ...
            try:
                self.pexpect_child = spawn_cinaseek(
