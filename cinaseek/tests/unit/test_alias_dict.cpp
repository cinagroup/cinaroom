...
// `--no-working-directory` flag were not yet introduced. In case the file was generated by an old
...
TEST_P(WriteReadTestsuite, writesAndReadsFiles)
{
    ...
    ASSERT_EQ((size_t)std::distance(reader.get_active_context().cbegin(),
                                    reader.get_active_context().cend()),
              (size_t)aliases_vector.size());
    ASSERT_EQ((size_t)std::distance(reader.get_active_context().begin(),
                                    reader.get_active_context().end()),
              (size_t)aliases_vector.size());
...
struct DaemonAliasTestsuite
    : public mpt::DaemonTestFixture,
      ...
      public WithParamInterface<std::tuple<CmdList,
                                           std::string,
                                           std::vector<std::string> /* removed aliases */,
...
TEST_P(DaemonAliasTestsuite, purgeRemovesPurgedInstanceAliasesAndScripts)
{
    ...
    EXPECT_CALL(*mock_image_vault, fetch_image).WillRepeatedly(Return(mp::VMImage{}));
