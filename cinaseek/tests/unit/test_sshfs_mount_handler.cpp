...
const cinaseek::logging::Level default_log_level = cinaseek::logging::Level::debug;
...
TEST_F(SSHFSMountHandlerTest, throwsInstallSshfsNoSnapDirFails)
{
    ...
    REPLACE(ssh_channel_request_exec,
            make_exec_that_fails_for({"[ -e /snap ]", "sudo snap list cinaseek-sshfs"}, invoked));
...
TEST_F(SSHFSMountHandlerTest, throwsInstallSshfsSnapInstallFails)
{
    ...
    REPLACE(ssh_channel_request_exec,
            make_exec_that_fails_for(
                {"sudo snap list cinaseek-sshfs", "sudo snap install cinaseek-sshfs"},
                invoked));
...
TEST_F(SSHFSMountHandlerTest, installSshfsTimeoutLogsInfo)
{
    ...
    auto mocked_ssh_channel_request_exec = [&sleep, &exit_code](ssh_channel, const char* raw_cmd) {
        std::string cmd{raw_cmd};
        exit_code = SSH_OK;
        if (cmd == "sudo snap install cinaseek-sshfs")
            sleep = true;
        else if (cmd == "sudo snap list cinaseek-sshfs")
            exit_code = SSH_ERROR;
    ...
    EXPECT_CALL(*logger_scope.mock_logger,
                log(mpl::Level::error,
                    ...
                    AllOf(HasSubstr("Could not install 'cinaseek-sshfs' in 'stub'"),
                          HasSubstr("timed out"))));
