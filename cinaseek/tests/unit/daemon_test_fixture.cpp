...
#include <cinaseek/auto_join_thread.h>
#include <cinaseek/cli/argparser.h>
#include <cinaseek/cli/client_common.h>
...
namespace
{
class TestCreate final : public cinaseek::cmd::Command
{
public:
    using Command::Command;
    cinaseek::ReturnCodeVariant run(cinaseek::ArgParser* parser) override
    {
        auto on_success = [](cinaseek::CreateReply& /*reply*/) -> mp::ReturnCodeVariant {
            return cinaseek::ReturnCode::Ok;
        };
        auto on_failure = [this](grpc::Status& status) -> mp::ReturnCodeVariant {
            cinaseek::CreateError create_error;
            ...
            if (errors.size() == 1)
            {
                ...
                if (error == cinaseek::CreateError::INVALID_DISK_SIZE)
                    cerr << "disk";
                else if (error == cinaseek::CreateError::INVALID_MEM_SIZE)
                    cerr << "memory";
                else
                    cerr << "?";
            }

            return cinaseek::ReturnCode::CommandFail;
        };
        ...
        return ret == cinaseek::ParseCode::Ok ? dispatch(&mp::Rpc::StubInterface::create,
                                                          request,
                                                          on_success,
                                                          on_failure,
                                                          streaming_callback)
...
private:
    cinaseek::ParseCode parse_args(cinaseek::ArgParser* parser)
    {
        ...
        if (status == cinaseek::ParseCode::Ok)
        {
    ...
    cinaseek::CreateRequest request;
};
...
class TestClient : public cinaseek::Client
{
public:
    explicit TestClient(cinaseek::ClientConfig& context) : cinaseek::Client{context}
    {
                                           ...
                                           std::istream& cin)
{
    ...
    AutoJoinThread t([this, &commands, &cout, &cerr, &cin] {
        ...
        {
            QStringList args = QStringList() << "cinaseek_test";
    ...
    const std::unordered_map<std::string, mp::VMMount>& mounts)
{
    ...
    cinaseek::test::fake_vm_properties vm_properties{};
    ...
    const std::string& contents)
{
    ...
    QString filename(temp_dir->path() + "/cinaseekd-vm-instances.json");
