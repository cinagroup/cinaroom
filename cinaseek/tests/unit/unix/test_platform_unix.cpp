...
TEST_F(TestPlatformUnix, setServerSocketRestrictionsChownFailsThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        ...
        std::runtime_error,
        mpt::match_what(
            StrEq("Could not set ownership of the cinaseek socket: Operation not permitted")));
}
...
TEST_F(TestPlatformUnix, setServerSocketRestrictionsChmodFailsThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        MP_PLATFORM.Platform::set_server_socket_restrictions(fmt::format("unix:{}", file.name()),
                                                             false),
        std::runtime_error,
        mpt::match_what(StrEq("Could not set permissions for the cinaseek socket")));
}
...
TEST_F(TestPlatformUnix, cinaseekStorageLocationReturnsExpectedPath)
{
    mpt::SetEnvScope e(mp::cinaseek_storage_env_var, file.name().toUtf8());

    auto storage_path = MP_PLATFORM.cinaseek_storage_location();

    EXPECT_EQ(storage_path, file.name());
}

TEST_F(TestPlatformUnix, cinaseekStorageLocationNotSetReturnsEmpty)
{
    auto storage_path = MP_PLATFORM.cinaseek_storage_location();
