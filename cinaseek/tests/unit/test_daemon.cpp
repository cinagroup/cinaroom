...
using namespace cinaseek::utils;
...
struct Daemon : public mpt::DaemonTestFixture
{
    Daemon()
    {
        ...
        EXPECT_CALL(mock_platform, cinaseek_storage_location())
            .Times(AnyNumber())
...
TEST_F(Daemon, dataPathValid)
{
    ...
    EXPECT_CALL(mock_platform, cinaseek_storage_location()).WillOnce(Return(QString()));
...
TEST_F(Daemon, dataPathWithStorageValid)
{
    ...
    mpt::SetEnvScope storage(mp::cinaseek_storage_env_var, storage_dir.path().toUtf8());

    EXPECT_CALL(mock_platform, cinaseek_storage_location())
        .WillOnce(Return(mp::utils::get_cinaseek_storage()));
...
TEST_F(Daemon, rootCertPathDoesntChangeWithStorage)
{
    ...
    const auto base_storage = mp::utils::get_cinaseek_storage();

    mpt::SetEnvScope storage(mp::cinaseek_storage_env_var, storage_dir.path().toUtf8());

    EXPECT_EQ(base_location, MP_PLATFORM.get_root_cert_path());
    EXPECT_NE(base_storage, mp::utils::get_cinaseek_storage());
}
...
{
    auto mock_factory = use_a_mock_vm_factory();
    cinaseek::test::fake_vm_properties vm_props{};
    vm_props.default_mac = "52:54:00:73:76:28";
    vm_props.state = cinaseek::VirtualMachine::State::running;
...
TEST_P(DaemonCreateLaunchTestSuite, defaultCloudInitGrowsRootFs)
{
    ...
    EXPECT_CALL(*mock_factory, prepare_instance_image(_, _))
        .WillOnce([](const cinaseek::VMImage&, const mp::VirtualMachineDescription& desc) {
...
TEST_P(DaemonCreateLaunchTestSuite, addsSshKeysToCloudInitConfig)
{
    ...
    EXPECT_CALL(*mock_factory, prepare_instance_image(_, _))
        .WillOnce([&expected_key](const cinaseek::VMImage&,
...
TEST_P(DaemonCreateLaunchPollinateDataTestSuite, addsPollinateUserAgentToCloudInitConfig)
{
    ...
    std::vector<std::pair<std::string, std::string>> const& expected_pollinate_map{
        ...
        {"content",
         fmt::format("cinaseek/version/{} # written by Multipass\n"
                     "cinaseek/driver/mock-1234 # written by Multipass\n"
                     "cinaseek/host/{}-{} # written by Multipass\n"
                     "cinaseek/alias/{} # written by Multipass\n",
    ...
    EXPECT_CALL(*mock_factory, prepare_instance_image(_, _))
        .WillOnce([&expected_pollinate_map](const cinaseek::VMImage&,
                                            const mp::VirtualMachineDescription& desc) {
...
TEST_P(LaunchWithBridges, createsNetworkCloudInitIso)
{
    ...
    EXPECT_CALL(*mock_factory, prepare_instance_image(_, _))
        .WillOnce([&args, &forbidden_names](const cinaseek::VMImage&,
                                            const mp::VirtualMachineDescription& desc) {
...
TEST_F(Daemon, refusesLaunchBridgedWithoutSetting)
{
    ...
    EXPECT_THAT(err_stream.str(),
                HasSubstr("You have to `cinaseek set local.bridged-network=<name>` to use the "
                          "\"bridged\" shortcut."));
}
...
TEST_F(Daemon, refusesLaunchWithInvalidBridgedInterface)
{
    ...
    EXPECT_THAT(
        err_stream.str(),
        HasSubstr(
            "Invalid network 'invalid' set as bridged interface, use `cinaseek set "
            "local.bridged-network=<name>` to correct. See `cinaseek networks` for valid names."));
}
...
TEST_F(Daemon, addBridgedInterfaceThrowsOnBadBridgedNetworkSetting)
{
    ...
    std::string msg{"Invalid network 'eth8' set as bridged interface, use `cinaseek set "
                    "local.bridged-network=<name>` "
                    "to correct. See `cinaseek networks` for valid names."};
    MP_EXPECT_THROW_THAT(daemon.test_add_bridged_interface(instance_name, instance_ptr),
                         std::runtime_error,
                         mpt::match_what(msg));
}
...
TEST_F(Daemon, setsPermissionsOnProvidedStoragePath)
{
    ...
    EXPECT_CALL(mock_platform, cinaseek_storage_location()).WillOnce(Return(path));
