...
struct DaemonTestFixture : public ::Test
{
    ...
    void send_commands(std::vector<std::vector<std::string>> commands,
                       std::ostream& cout = trash_stream,
                       std::ostream& cerr = trash_stream,
                       std::istream& cin = trash_stream);
     ...
     *  (e.g. @c mp::Daemon::get). It takes care of promise/future boilerplate. This will generally
     * be given a
     ...
     * expects. The template needs to be instantiated with the correct reply type. Example: <tt>
     * grpc::ServerWriterInterface\<mp\::GetReply\>
     * </tt>
     ...
     * @param server The concrete @c grpc::ServerWriterInterface to call the slot with (see doc on
     * Server typename). This will generally be a @c mpt::MockServerWriter. Notice that this is a
     * <em>universal reference</em>, so it will bind to both lvalue and rvalue references.
...
#else
    std::string server_address{"unix:/tmp/test-cinaseekd.socket"};
#endif
