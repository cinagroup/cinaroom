...
namespace
{
const auto apparmor_output_file = "/tmp/cinaseek-apparmor-profile.txt";
...
TEST_F(ApparmoredProcessNoFactoryTest, snapEnablesCacheWithExpectedArgs)
{
    ...
    EXPECT_TRUE(
        input.contains(QString("args: -WL, %1/apparmor.d/cache/cinaseek, --abort-on-error, -r,")
                           .arg(cache_dir.path())
                           .toUtf8()));
    EXPECT_TRUE(input.contains(apparmor_profile_text));
}
...
TEST_F(ApparmoredProcessNoFactoryTest, logsAllExpectedMessagesOnStart)
{
    ...
    logger_scope.mock_logger->expect_log(mpl::Level::debug,
                                         "Applied AppArmor policy: cinaseek.mock_process");
    logger_scope.mock_logger->expect_log(
        mpl::Level::trace,
