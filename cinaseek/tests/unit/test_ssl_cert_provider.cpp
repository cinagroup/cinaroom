...
TEST_F(SSLCertProviderFixture, importsExistingCertAndKey)
{
    ...
    const auto key_path = dir.filePath("cinaseek_cert_key.pem");
    const auto cert_path = dir.filePath("cinaseek_cert.pem");
...
{
    QDir dir{cert_dir};
    auto key_file = dir.filePath("cinaseek_cert_key.pem");
    auto cert_file = dir.filePath("cinaseek_cert.pem");
...
TEST_F(SSLCertProviderFixture, createsDifferentCertsPerServerName)
{
    ...
    // move the cinaseek_root_cert.pem into the temporary directory so it will be deleted
    // automatically later
