...
namespace test
{
struct StubVirtualMachineFactory : public cinaseek::BaseVirtualMachineFactory
{
    ...
    StubVirtualMachineFactory(std::unique_ptr<TempDir> tmp_dir)
        : cinaseek::BaseVirtualMachineFactory{tmp_dir->path()}, tmp_dir{std::move(tmp_dir)}
    {
    }
    ...
    cinaseek::FetchType fetch_type() override
    {
        return cinaseek::FetchType::ImageOnly;
    }

    cinaseek::VMImage prepare_source_image(const cinaseek::VMImage& source_image) override
    {
        return source_image;
    }

    void prepare_instance_image(const cinaseek::VMImage& instance_image,
                                const cinaseek::VirtualMachineDescription& vm_desc) override
    {
    }
    ...
    cinaseek::VMImageVault::UPtr create_image_vault(std::vector<VMImageHost*> image_hosts,
